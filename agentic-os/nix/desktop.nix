# nix/desktop.nix — Agentic OS Desktop (Electron) app build + wrapper
#
...
# `AGENTIC_OS_DESKTOP_HERMES` override env var, so the desktop's resolver
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "agentic-os-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "agentic-os-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = pkgs.buildNpmPackage (
    npm
    // {
      pname = "agentic-os-desktop-renderer";
      inherit version;
      doCheck = true;
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/agentic-os-desktop $out/bin
    cp -r ${renderer}/* $out/share/agentic-os-desktop/
    ...
    substituteInPlace $out/share/agentic-os-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/agentic-os-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # AGENTIC_OS_DESKTOP_HERMES to the absolute path of the nix-built `hermes`
    # binary so the desktop's resolver step 4 ("existing Agentic OS CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/agentic-os-desktop \
      --add-flags "$out/share/agentic-os-desktop" \
      --set AGENTIC_OS_DESKTOP_HERMES "${lib.getExe hermesAgent}" \
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Agentic OS";
    homepage = "https://github.com/subhxroy/agentic-os";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "agentic-os-desktop";
  };
}
