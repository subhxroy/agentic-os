# nix/nixosModules.nix — NixOS module for agentic-os
#
# Two modes:
...
# container recreation. Environment variables are written to $AGENTIC_OS_HOME/.env
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.agentic-os;
    effectivePackage =
    ...
    agentic-os = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-agentic-os)
    ...
    # AGENTIC_OS_HOME via addToSystemPackages, so they can save settings through the
    ...
    containerEntrypoint = pkgs.writeShellScript "hermes-container-entrypoint" ''
      ...
      # Ensure AGENTIC_OS_HOME is owned by the target user.
      ...
      if [ -n "''${AGENTIC_OS_HOME:-}" ] && [ -d "$AGENTIC_OS_HOME" ]; then
        find "$AGENTIC_OS_HOME" \! -user "$HERMES_UID" -exec chown "$HERMES_UID:$HERMES_GID" {} +
      fi
    ...
    # so they can update without recreation. Env vars go through $AGENTIC_OS_HOME/.env.
  ...
  in {
    options.services.agentic-os = with lib; {
      enable = mkEnableOption "Agentic OS gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = agentic-os;
        description = "The agentic-os package to use.";
      };
      ...
      stateDir = mkOption {
        ...
        description = "State directory. Contains .hermes/ subdir (AGENTIC_OS_HOME).";
      };
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $AGENTIC_OS_HOME/.env at activation time.
          Hermes reads this file on every startup via load_agentic_os_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $AGENTIC_OS_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $AGENTIC_OS_HOME/mcp-tokens/.
              '';
            };
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          agentic_os.plugins entry-point group. Each package must be built
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in agentic-os's
      ...
      addToSystemPackages = mkOption {
        ...
        description = ''
          ...
          AGENTIC_OS_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.agentic-os symlink to the service
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.agentic-os.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the hermes CLI to system PATH and export AGENTIC_OS_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.agentic-os/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.AGENTIC_OS_HOME = "${cfg.stateDir}/.hermes";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.agentic-os.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.agentic-os: container.enable is true and container.hostUsers
      ...
      {
        system.activationScripts."agentic-os-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
    ...
    HERMES_CONTAINER_MODE_EOF
            ...
            ${lib.concatStringsSep "\n" (map (user:
              ...
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.hermes" ]; then
                  rm -f "${symlinkPath}"
                  echo "agentic-os: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.agentic-os -> stateDir/.hermes for each hostUser so the
            ...
            (lib.concatStringsSep "\n" (map (user:
              ...
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  ...
                  echo "agentic-os: backing up existing ${symlinkPath} to $_backup"
          ...
          # Hermes reads $AGENTIC_OS_HOME/.env at startup via load_agentic_os_dotenv(),
    ...
    HERMES_NIX_ENV_EOF
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.agentic-os = {
          description = "Agentic OS Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            AGENTIC_OS_HOME = "${cfg.stateDir}/.hermes";
            HERMES_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $AGENTIC_OS_HOME/.env by the activation script. load_agentic_os_dotenv()
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.agentic-os = {
          description = "Agentic OS Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ${containerBin} create \
                ...
                --env AGENTIC_OS_HOME=${containerDataDir}/.hermes \
