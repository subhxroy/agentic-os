import { forceRedraw, type MouseTrackingMode } from '@agentic-os/ink'
...
export const coreCommands: SlashCommand[] = [
  ...
  {
    help: 'update Agentic OS to the latest version (exits TUI)',
    name: 'update',
    run: (_arg, ctx) => {
      if (DASHBOARD_TUI_MODE) {
    ...
    run: async (arg, ctx) => {
      ...
      if (!arg && ctx.composer.hasSelection) {
        ...
        if (text) {
          ...
          return sys('clipboard copy failed — try AGENTIC_OS_TUI_FORCE_OSC52=1 to force the escape sequence')
        }
      }
