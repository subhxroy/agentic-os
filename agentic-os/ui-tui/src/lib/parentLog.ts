...
const logDir = join(process.env.AGENTIC_OS_HOME?.trim() || join(homedir(), '.hermes'), 'logs')
...
// a real ~/.agentic-os (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('agentic-os-tui: parent lifecycle log unavailable\n')
    }
  }
}
