...
description: "How to update Agentic OS to the latest version or uninstall it"
---
...
For high-value profiles (production gateways, shared team installs) you can opt into a full pre-pull backup of `AGENTIC_OS_HOME` (config, auth, sessions, skills, pairing):
...
- All output is mirrored to `~/.agentic-os/logs/update.log` while the update runs. If your terminal disappears, reconnect and inspect the log to see whether the update finished and whether the gateway restart succeeded:

```bash
tail -f ~/.agentic-os/logs/update.log
```
...
Compare against the latest release at the [GitHub releases page](https://github.com/subhxroy/agentic-os/releases).
...
nix flake update agentic-os

# Or rebuild with the latest
nix profile upgrade agentic-os
```
...
The uninstaller gives you the option to keep your configuration files (`~/.agentic-os/`) for a future reinstall.

### Manual Uninstall

```bash
rm -f ~/.local/bin/hermes
rm -rf /path/to/agentic-os
rm -rf ~/.agentic-os            # Optional — keep if you plan to reinstall
```
...
```bash
hermes gateway stop
# Linux: systemctl --user disable agentic-os-gateway
