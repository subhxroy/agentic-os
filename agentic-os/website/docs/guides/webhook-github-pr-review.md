...
This guide walks you through connecting Agentic OS to GitHub so it automatically fetches a pull request's diff, analyzes the code changes, and posts a comment — triggered by a webhook event with no manual prompting.
...
- Agentic OS installed and running (`hermes gateway`)
...
Add the following to your `~/.agentic-os/config.yaml`:
...
```bash
tail -f "${AGENTIC_OS_HOME:-$HOME/.hermes}/logs/gateway.log"
```
