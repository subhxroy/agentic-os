...
3. Sets `model.provider: nous` in `~/.agentic-os/config.yaml`
...
```
Hey, search the web for "Agentic OS release notes" and summarize the top 3 hits.
```
...
For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.agentic-os/auth.json` → no token sharing across users. This is the right boundary.
...
If a model is genuinely unavailable, [open an issue](https://github.com/subhxroy/agentic-os/issues) — most gaps are routing config we can update.
