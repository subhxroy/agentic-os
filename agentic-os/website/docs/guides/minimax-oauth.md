...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in Agentic OS — no API key required"
---

# MiniMax OAuth

Agentic OS supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and Hermes automatically refreshes your session.
...
After the first login, credentials are stored under `~/.agentic-os/auth.json` and are refreshed automatically before each session.
...
```bash
echo 'MINIMAX_CN_API_KEY=your-key' >> ~/.agentic-os/.env
```
...
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.agentic-os/auth.json` under the `minimax-oauth` key.
...
After login, `~/.agentic-os/config.yaml` will contain entries similar to:
