...
hermes cron create "0 2 * * *" \
  "You are a project manager triaging the subhxroy/agentic-os GitHub repo.

1. Run: gh issue list --repo subhxroy/agentic-os --state open --json number,title,labels,author,createdAt --limit 30
...
hermes cron create "0 9 * * 1" \
  "Scan the subhxroy/agentic-os repo for documentation drift.

1. Run: gh pr list --repo subhxroy/agentic-os --state merged --json number,title,files,mergedAt --limit 30
...
hermes cron create "0 6 * * *" \
  "Run a dependency security audit on the agentic-os project.

1. cd ~/.agentic-os/agentic-os && source .venv/bin/activate
...
```python title="~/.agentic-os/scripts/check-uptime.py"
...
hermes cron create "every 30m" \
  ...
  --script ~/.agentic-os/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
hermes cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Agentic OS development." \
...
hermes cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the agentic-os codebase.
