...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup to Agentic OS — what gets migrated, how config maps, and what to check after."
---
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.agentic-os/` (by default a single restore-point archive is written before apply, under `~/.agentic-os/backups/pre-migration-*.zip`; restorable with `hermes import`). |
...
|------|----------------|-------------------|-------|
| Persona | `workspace/SOUL.md` | `~/.agentic-os/SOUL.md` | Direct copy |
| Workspace instructions | `workspace/AGENTS.md` | `AGENTS.md` in `--workspace-target` | Requires `--workspace-target` flag |
| Long-term memory | `workspace/MEMORY.md` | `~/.agentic-os/memories/MEMORY.md` | Parsed into entries, merged with existing, deduped. Uses `§` delimiter. |
| User profile | `workspace/USER.md` | `~/.agentic-os/memories/USER.md` | Same entry-merge logic as memory. |
| Daily memory files | `workspace/memory/*.md` | `~/.agentic-os/memories/MEMORY.md` | All daily files merged into main memory. |
...
|--------|------------------|-------------------|
| Workspace skills | `workspace/skills/` | `~/.agentic-os/skills/openclaw-imports/` |
| Managed/shared skills | `~/.openclaw/skills/` | `~/.agentic-os/skills/openclaw-imports/` |
| Personal cross-project | `~/.agents/skills/` | `~/.agentic-os/skills/openclaw-imports/` |
| Project-level shared | `workspace/.agents/skills/` | `~/.agentic-os/skills/openclaw-imports/` |
...
| Provider API keys | `models.providers.*.apiKey` | `~/.agentic-os/.env` | Requires `--migrate-secrets`. See [API key resolution](#api-key-resolution) below. |
...
| TTS assets | `~/.agentic-os/tts/` (file copy) |
...
These are saved to `~/.agentic-os/migration/openclaw/<timestamp>/archive/` for manual review:
...
2. **Review archived files** — anything in `~/.agentic-os/migration/openclaw/<timestamp>/archive/` needs manual attention.
...
5. **Test messaging** — if you migrated platform tokens, restart the gateway: `systemctl --user restart agentic-os-gateway`
...
Imported skills land in `~/.agentic-os/skills/openclaw-imports/`. Start a new session for them to take effect, or run `/skills` to verify they're loaded.
