...
sidebar_position: 5
title: "WhatsApp"
description: "Set up Agentic OS as a WhatsApp bot via the built-in Baileys bridge"
---
...
The Baileys bridge saves its session under `~/.agentic-os/platforms/whatsapp/session`. This means:
...
- Agent responses are prefixed with "⚕ **Agentic OS**" by default. You can customize or disable this in `config.yaml`:
...
| **Session not persisting** | Check that `~/.agentic-os/platforms/whatsapp/session` exists and is writable. If containerized, mount it as a persistent volume. |
...
| **Bot replies to strangers with a pairing code** | Set `whatsapp.unauthorized_dm_behavior: ignore` in `~/.agentic-os/config.yaml` if you want unauthorized DMs to be silently ignored instead. |
...
- The `~/.agentic-os/platforms/whatsapp/session` directory contains full session credentials — protect it like a password
- Set file permissions: `chmod 700 ~/.agentic-os/platforms/whatsapp/session`
