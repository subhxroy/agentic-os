...
sidebar_position: 4
title: "Slack"
description: "Set up Agentic OS as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Agentic OS to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
...
1. Generate the manifest. New Slack apps must use Agent view:
   ...
   ```
   This writes `~/.agentic-os/slack-manifest.json` and prints paste-in
...
4. Enter an app name (e.g., "Agentic OS") and select your workspace
5. Click **Create App**
...
[`COMMAND_REGISTRY`](https://github.com/subhxroy/agentic-os/blob/main/hermes_cli/commands.py)
...
3. Paste the new contents of `~/.agentic-os/slack-manifest.json`
...
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Agentic OS what time is it?`). In channels, Hermes replies in a thread attached to that message. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.agentic-os/config.yaml`.
...
Make sure the bot has been **invited to the channel** (`/invite @Agentic OS`).
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Agentic OS` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Agentic OS` |
...
7. ✅ Bot was **invited** to the channel (`/invite @Agentic OS`)
...
- Tokens should be stored in `~/.agentic-os/.env` (file permissions `600`)
