...
- A strong shared secret to use as the `clientState` value. Generate with `openssl rand -hex 32` and put it in `~/.agentic-os/.env` as `MSGRAPH_WEBHOOK_CLIENT_STATE`.

## Quick Start

Minimum `~/.agentic-os/config.yaml`:
...
Or via env vars in `~/.agentic-os/.env` (auto-merged on startup):
