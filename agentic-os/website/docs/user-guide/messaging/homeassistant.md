...
description: Control your smart home with Agentic OS via Home Assistant integration.
...
Agentic OS integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Agentic OS"
5. Copy the token
...
Agentic OS registers four tools for smart home control:
...
Configure which events the agent sees in `~/.agentic-os/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Agentic OS".
...
The adapter reads credentials from `~/.agentic-os/.env` (auto-merged at startup) or
