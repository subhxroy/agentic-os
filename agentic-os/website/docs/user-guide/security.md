...
Agentic OS is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
The approval system supports three modes, configured via `approvals.mode` in `~/.agentic-os/config.yaml`:
...
Configure the timeout in `~/.agentic-os/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.agentic-os/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.agentic-os/.env` | Overwrite sensitive file via redirection |
...
The `AGENTIC_OS_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.agentic-os/config.yaml`:
...
| Hermes credential stores | `auth.json`, `.env`, `.anthropic_oauth.json`, `mcp-tokens/`, `pairing/` under AGENTIC_OS_HOME (active profile and global root) |
...
- **Do not add to `~/.agentic-os/.env` casually.** If you set it to a project directory, the agent cannot write to `~/.agentic-os/cron/jobs.json`, profile skills, or other Hermes state outside that prefix
...
Do not ask the agent to `patch` `~/.agentic-os/cron/jobs.json` directly. Use the `cronjob` tool, [`hermes cron`](./features/cron.md), or `/cron` — they update the job store through the supported API. The same applies to other Hermes control files when write safety blocks direct edits.
...
Set allowed user IDs as comma-separated values in `~/.agentic-os/.env`:
...
Set GATEWAY_ALLOW_ALL_USERS=true in ~/.agentic-os/.env to allow open access,
...
Control how unauthorized direct messages are handled in `~/.agentic-os/config.yaml`:
...
```bash
docker exec -u hermes agentic-os hermes pairing approve telegram ABC12DEF
```
...
**Storage:** Pairing data is stored in `~/.agentic-os/pairing/` with per-platform JSON files:
...
Container resources are configurable in `~/.agentic-os/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.agentic-os/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `AGENTIC_OS_HOME`. Skills declare these in frontmatter:
...
When loaded, Hermes checks if these files exist in the active profile's `AGENTIC_OS_HOME` and registers them for mounting:
...
4. **Store secrets securely** — keep API keys in `~/.agentic-os/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.agentic-os/logs/` for unauthorized access attempts
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.agentic-os/.env`:
...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. Hermes installs these **lazily** on first use rather than eagerly under `agentic-os[all]`. The implementation lives in `tools/lazy_deps.py`.
