...
| Version | `1.0.0` |
| Author | Ben Barclay (benbarclay), Agentic OS |
| License | MIT |
...
```json
{
  "client_name": "Agentic OS (manual OAuth)",
...
11. **Client info file is NOT optional.** Agentic OS needs `<server>.client.json` to know the `client_id` for refresh grants. Skipping it means the first refresh fails and the user has to re-auth — writing both files is the whole point of this skill.
