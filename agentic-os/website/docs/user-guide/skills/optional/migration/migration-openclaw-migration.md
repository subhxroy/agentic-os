---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Agentic OS"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Agentic OS"
---
...
Migrate a user's OpenClaw customization footprint into Agentic OS. Imports Hermes-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
...
| Version | `1.0.0` |
| Author | Agentic OS (Nous Research) |
| License | MIT |
...
Use this skill when a user wants to move their OpenClaw setup into Agentic OS with minimal manual cleanup.
...
It uses `scripts/openclaw_to_agentic_os.py` to:
...
- copy OpenClaw skills into `~/.agentic-os/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Hermes workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.agentic-os/tts/`
...
- `scripts/openclaw_to_agentic_os.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.agentic-os/skills/migration/openclaw-migration/scripts/openclaw_to_agentic_os.py`

Do not guess a shorter path like `~/.agentic-os/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.agentic-os/skills/migration/openclaw-migration/`.
...
```bash
python3 ~/.agentic-os/skills/migration/openclaw-migration/scripts/openclaw_to_agentic_os.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.hermes/skills/migration/openclaw-migration/scripts/openclaw_to_agentic_os.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.agentic-os/skills/migration/openclaw-migration/scripts/openclaw_to_agentic_os.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.agentic-os/skills/migration/openclaw-migration/scripts/openclaw_to_agentic_os.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.agentic-os/skills/migration/openclaw-migration/scripts/openclaw_to_agentic_os.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.agentic-os/skills/migration/openclaw-migration/scripts/openclaw_to_agentic_os.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
- OpenClaw skills available under `~/.agentic-os/skills/openclaw-imports/`
