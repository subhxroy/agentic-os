...
| Path | `skills/agentic-os-desktop-plugins` |
| Version | `1.0.0` |
...
system (`~/.agentic-os/plugins/`) is otherwise documented separately.
...
- Write access to `$AGENTIC_OS_HOME/desktop-plugins/` (usually
  `~/.agentic-os/desktop-plugins/`).

## How to Run

1. Create `$AGENTIC_OS_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (relative to this skill directory) — that's
   `~/.agentic-os/...` by default, or `~/.agentic-os/profiles/<profile>/...` under a
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.agentic-os/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
