...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Agentic OS terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Agentic OS's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.
...
hermes -s agentic-os-dev,github-auth
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Agentic OS CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Agentic OS CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
Every installed skill in `~/.agentic-os/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.agentic-os/config.yaml`:
...
```
Agentic OS has been suspended. Run `fg` to bring Agentic OS back.
```
...
CLI sessions are stored in Hermes's SQLite state database under `~/.agentic-os/state.db`. The database keeps:
