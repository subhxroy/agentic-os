...
sidebar_position: 10
title: "Skins & Themes"
description: "Customize the Agentic OS CLI with built-in and user-defined skins"
---

# Skins & Themes

Skins control the **visual presentation** of the Agentic OS CLI: banner colors, spinner faces and verbs, response-box labels, branding text, and the tool activity prefix.
...
/skin mytheme        # switch to a custom skin from ~/.agentic-os/skins/mytheme.yaml
```

Or set the default skin in `~/.agentic-os/config.yaml`:
...
|------|-------------|----------------|------------------|
| `default` | Classic Hermes — gold and kawaii | `Agentic OS` | Warm gold borders, cornsilk text, kawaii faces in spinners. The familiar caduceus banner. Clean and inviting. |
...
| `mono` | Monochrome — clean grayscale | `Agentic OS` | All grays — no color. Borders are `#555555`, text is `#c9d1d9`. Ideal for minimal terminal setups or screen recordings. |
| `slate` | Cool blue — developer-focused | `Agentic OS` | Royal blue borders (`#4169e1`), soft blue text. Calm and professional. No custom spinner — uses default faces. |
| `daylight` | Light theme for bright terminals with dark text and cool blue accents | `Agentic OS` | Designed for white or bright terminals. Dark slate text with blue borders, pale status surfaces, and a light completion menu that stays readable in light terminal profiles. |
| `warm-lightmode` | Warm brown/gold text for light terminal backgrounds | `Agentic OS` | Warm parchment tones for light terminals. Dark brown text with saddle-brown accents, cream-colored status surfaces. An earthy alternative to the cooler daylight theme. |
...
|-----|-------------|---------|
| `agent_name` | Name shown in banner title and status display | `Agentic OS` |
| `welcome` | Welcome message shown at CLI startup | `Welcome to Agentic OS! Type your message or /help for commands.` |
...
Create YAML files under `~/.agentic-os/skins/`. User skins inherit missing values from the built-in `default` skin, so you only need to specify the keys you want to change.

### Full custom skin YAML template

```yaml
# ~/.agentic-os/skins/mytheme.yaml
...
- Saves directly to `~/.agentic-os/skins/`
- Activates a skin by updating `~/.agentic-os/config.yaml`
...
6. Click **Save** to write the skin YAML to `~/.agentic-os/skins/`.
7. Click **Activate** to set it as the current skin (updates `display.skin` in `config.yaml`).

Hermes Mod respects the `AGENTIC_OS_HOME` environment variable, so it works with [profiles](/user-guide/profiles) too.
...
- User skins in `~/.agentic-os/skins/` take precedence over built-in skins with the same name.
