...
1. **Themes** — YAML files that repaint the dashboard's palette, typography, layout, and per-component chrome. Drop a file in `~/.agentic-os/dashboard-themes/`; it appears in the theme switcher.
...
Themes are YAML files stored in `~/.agentic-os/dashboard-themes/`. The file name doesn't matter (the theme's `name:` field is what the system uses), but convention is `<name>.yaml`. Every field is optional — missing keys fall back to the built-in `default` theme, so a theme can be as small as one color.

### Quick start — your first theme

```bash
mkdir -p ~/.agentic-os/dashboard-themes
```

```yaml
# ~/.agentic-os/dashboard-themes/neon.yaml
name: neon
label: Neon
...
```yaml
# ~/.agentic-os/dashboard-themes/ocean.yaml
name: ocean
label: Ocean Deep
...
A dashboard plugin is a directory with a `manifest.json`, a pre-built JS bundle, and optionally a CSS file and a Python file with FastAPI routes. Plugins live next to other Hermes plugins in `~/.agentic-os/plugins/<name>/` — the dashboard extension is a `dashboard/` subfolder inside that plugin directory, so one plugin can extend both the CLI/gateway and the dashboard from a single install.

Plugins don't bundle React or UI components. They use the **Plugin SDK** exposed on `window.__AGENTIC_OS_PLUGIN_SDK__`. This keeps plugin bundles tiny (typically a few KB) and avoids version conflicts.
...
```bash
mkdir -p ~/.agentic-os/plugins/my-plugin/dashboard/dist
```

Write the manifest:

```json
// ~/.agentic-os/plugins/my-plugin/dashboard/manifest.json
{
  "name": "my-plugin",
  "label": "My Plugin",
  "icon": "Sparkles",
...
```javascript
// ~/.agentic-os/plugins/my-plugin/dashboard/dist/index.js
(function () {
  "use strict";

  const SDK = window.__AGENTIC_OS_PLUGIN_SDK__;
  const { React } = SDK;
...
Everything a plugin needs is on `window.__AGENTIC_OS_PLUGIN_SDK__`. Plugins should never import React directly.
...
```json
// ~/.agentic-os/plugins/session-notes/dashboard/manifest.json
{
  "name": "session-notes",
  "label": "Session Notes",
...
```javascript
// ~/.agentic-os/plugins/session-notes/dashboard/dist/index.js
(function () {
  const SDK = window.__AGENTIC_OS_PLUGIN_SDK__;
  const { React } = SDK;
...
```python
# ~/.agentic-os/plugins/my-plugin/dashboard/plugin_api.py
...
Backend routes run inside the dashboard process, so they can import from the agentic-os codebase directly:
...
|----------|-----------|--------------|
| 1 (wins on conflict) | `~/.agentic-os/plugins/<name>/dashboard/` | `user` |
...
1. Dashboard loads. `main.tsx` exposes the SDK on `window.__AGENTIC_OS_PLUGIN_SDK__` and the registry on `window.__HERMES_PLUGINS__`.
...
cp hermes-example-plugins/strike-freedom-cockpit/theme/strike-freedom.yaml \
   ~/.agentic-os/dashboard-themes/

# Plugin
cp -r hermes-example-plugins/strike-freedom-cockpit ~/.agentic-os/plugins/
```
...
|--------|------|----------|
| `window.__AGENTIC_OS_PLUGIN_SDK__` | object | `registry.ts` — React, hooks, UI components, API client, utils. |
...
Check that the file is in `~/.agentic-os/dashboard-themes/` and ends in `.yaml` or `.yml`. Refresh the page. Run `curl http://127.0.0.1:9119/api/dashboard/themes` — your theme should be in the response. If the YAML has a parse error, the dashboard logs to `errors.log` under `~/.agentic-os/logs/`.

**My plugin's tab doesn't show up.**
1. Check the manifest is at `~/.agentic-os/plugins/<name>/dashboard/manifest.json` (note the `dashboard/` subdirectory).
...
4. Tail `~/.agentic-os/logs/errors.log` for `Failed to load plugin <name> API routes` — import errors are logged there.
...
Dashboard plugins are installed by directory layout, not by pip entry point. The cleanest distribution path today is a git repo the user clones into `~/.agentic-os/plugins/`. A pip-based installer for dashboard plugins is not currently wired up.
