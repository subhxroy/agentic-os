...
1. loads jobs from `~/.agentic-os/cron/jobs.json`
...
A file lock at `~/.agentic-os/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.

### Execution history

Hermes records each claimed cron attempt in the profile-local
`~/.agentic-os/cron/executions.db` before executor or provider dispatch. Attempts
...
| `"local"` | Save to local files only (`~/.agentic-os/cron/output/`) | Default on CLI |
...
If the agent's final response contains `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.agentic-os/cron/output/`), but no message is sent to the delivery target.
...
`.sh` / `.bash` files run under `/bin/bash`; anything else under the current Python interpreter (`sys.executable`). Scripts must live in `~/.agentic-os/scripts/` (same sandboxing rule as the pre-run script gate).
...
Hermes will write the check script to `~/.agentic-os/scripts/` via `write_file`, then call:
...
cronjob(
    action="create",
    prompt="Fetch the top 10 AI/ML stories from Hacker News. Save them to ~/.agentic-os/data/briefs/raw.md in markdown format with title, URL, and score.",
    schedule="0 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.agentic-os/data/briefs/raw.md. Score each story 1–10 for engagement potential and novelty. Output the top 5 to ~/.agentic-os/data/briefs/ranked.md.",
    schedule="30 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.agentic-os/data/briefs/ranked.md. Write 3 tweet drafts (hook + body + hashtags). Deliver to telegram:7976161601.",
    schedule="0 8 * * *",
...
- When Job 2 fires, Hermes reads Job 1's most recent output from `~/.agentic-os/cron/output/{job1_id}/*.md`
...
```bash
#!/bin/bash
# ~/.agentic-os/scripts/feed-changed.sh
...
```bash
#!/bin/bash
# ~/.agentic-os/scripts/flag-ready.sh
...
```python
#!/usr/bin/env python
# ~/.agentic-os/scripts/new-rows.py
import json, sqlite3
...
:::tip
Hermes's own `~/.agentic-os/state.db` is an internal schema that changes between releases. Don't query it from a pre-run gate — point at your own database or feed instead.
:::

Credit: this recipe set was prompted by @iankar8's exploration in [#2654](https://github.com/subhxroy/agentic-os/pull/2654), which proposed adding sql/file/command triggers as a parallel mechanism. The `script` + `wakeAgent` gate already covers all three cases at $0, so the work landed as documentation instead.
...
Jobs are stored in `~/.agentic-os/cron/jobs.json`. Output from job runs is saved to `~/.agentic-os/cron/output/{job_id}/{timestamp}.md`.
