...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Agentic OS — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Agentic OS supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.
...
1. **Agentic OS installed** — via the install script (see [Installation](/getting-started/installation))
2. **An LLM provider configured** — run `hermes model` or set your preferred provider credentials in `~/.agentic-os/.env`
3. **A working base setup** — run `hermes` to verify the agent responds to text before enabling voice

:::tip
The `~/.agentic-os/` directory and default `config.yaml` are created automatically the first time you run `hermes`. You only need to create `~/.agentic-os/.env` manually for API keys.
:::
...
cd ~/.agentic-os/agentic-os && uv pip install -e ".[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
cd ~/.agentic-os/agentic-os && uv pip install -e ".[messaging]"

# Premium TTS (ElevenLabs)
cd ~/.agentic-os/agentic-os && uv pip install -e ".[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
cd ~/.agentic-os/agentic-os && uv pip install -e ".[all]"
```
...
Voice mode is available in both the **classic CLI** (`hermes chat`) and the **TUI** (`hermes --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.agentic-os/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
:::tip
The record key is configurable via `voice.record_key` in `~/.agentic-os/config.yaml` (default: `ctrl+b`).
:::
...
:::tip
To disable the mention requirement in server channels, add to `~/.agentic-os/.env`:
```bash
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.agentic-os/.env`
...
- Review gateway logs: `tail -f ~/.agentic-os/logs/gateway.log`
