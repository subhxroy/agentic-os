...
description: "Overview of Agentic OS's tools — what's available, how toolsets work, and terminal backends"
---
...
```
```bash
# Set credentials in ~/.agentic-os/.env
...
If a command needs sudo, you'll be prompted for your password (cached for the session). Or set `SUDO_PASSWORD` in `~/.agentic-os/.env`.

:::warning
On messaging platforms, if sudo fails, the output includes a tip to add `SUDO_PASSWORD` to `~/.agentic-os/.env`.
:::
