...
The web dashboard is a browser-based UI for managing your Agentic OS installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.
...
child with the selected profile's `AGENTIC_OS_HOME`, so the conversation runs
...
The default `agentic-os` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
cd ~/.agentic-os/agentic-os && uv pip install -e ".[web,pty]"
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `cd ~/.agentic-os/agentic-os && uv pip install -e ".[all]"` includes both extras and is the easiest path if you also want messaging/voice/etc.
...
- `ptyprocess` — installed by the `pty` extra (`cd ~/.agentic-os/agentic-os && uv pip install -e ".[web,pty]"`, or `[all]` covers both)
...
ExecStart=/path/to/venv/bin/python -m agentic_os_cli.main dashboard \
    --host 0.0.0.0 --port 9119 --no-open
```

with `~/.agentic-os/.env` containing:
...
Browse, search, and toggle installed skills and toolsets, and install new ones from the hub. Skills are loaded from `~/.agentic-os/skills/` and grouped by category.
...
- **Restart gateway** — credentials are written to `~/.agentic-os/.env` and the enabled flag to `config.yaml`; the gateway connects each enabled channel on its next restart, which you can trigger right from the page.
...
This re-reads `~/.agentic-os/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
`AGENTIC_OS_HOME`. Omitted = the dashboard's own profile. Unknown profile names
...
When the dashboard is bound to a public or non-loopback address — anything other than `127.0.0.1` / `localhost` — Agentic OS engages an auth gate. Every request must carry a verified session cookie or it's bounced to the login page. Three providers ship in the box:
...
- **CLI — `hermes dashboard register`.** Run it on the host where the dashboard lives. It resolves your existing Nous login (run `hermes setup` first if you're not logged in), registers a self-hosted OAuth client with the Portal, and writes `HERMES_DASHBOARD_OAUTH_CLIENT_ID` into `~/.agentic-os/.env` for you. Optional flags: `--name` (a human-readable label, otherwise auto-generated) and `--redirect-uri` (a public HTTPS callback URL for an internet-facing host).

  ```bash
  hermes dashboard register
  # ✓ Registered dashboard "swift_falcon"
  # …writes HERMES_DASHBOARD_OAUTH_CLIENT_ID to ~/.agentic-os/.env
  ```
...
Bundled providers reported these issues:
  • nous: HERMES_DASHBOARD_OAUTH_CLIENT_ID is not set (and
    ...
    deploys a Agentic OS instance — set it to your provisioned
...
**1. Log in and register the dashboard.** `hermes dashboard register` uses your existing Nous login to provision an OAuth client and writes `HERMES_DASHBOARD_OAUTH_CLIENT_ID` into `~/.agentic-os/.env` for you:
...
**1. Set credentials in `~/.agentic-os/.env`.** Hash the password so no plaintext sits at rest, and set a stable signing secret so sessions survive restarts:
...
Every login start, success, failure, and session-verify failure is written as a JSON line to `$AGENTIC_OS_HOME/logs/dashboard-auth.log`. Sensitive fields (`access_token`, `refresh_token`, `code`, `code_verifier`, `state`, `Authorization` header) are redacted before logging.
...
```python
# ~/.agentic-os/plugins/dashboard-auth-myidp/__init__.py
from agentic_os_cli.dashboard_auth import DashboardAuthProvider, Session, LoginStart
...
Agentic OS Desktop can drive a Hermes backend running on another machine (a VPS, a home server, a Mini behind Tailscale). In the app this lives under **Settings → Gateway → Remote gateway**, which asks for a **Remote URL** and a way to **Sign in**. (For the desktop app itself — install, settings, chat — see the [Agentic OS Desktop](/user-guide/desktop) page.)
...
```bash
# 1. Set the dashboard login credentials in ~/.agentic-os/.env (secrets file, 0600).
cat >> ~/.agentic-os/.env <<'EOF'
HERMES_DASHBOARD_BASIC_AUTH_USERNAME=admin
HERMES_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
...
If you run the dashboard as a systemd service, `~/.agentic-os/.env` is picked up automatically when the unit has `EnvironmentFile=%h/.hermes/.env`, so the credentials are in the environment at boot.
...
Instead of the in-app setting, you can point the desktop at a backend with an env var before launching it. When `AGENTIC_OS_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app URL (the Gateway settings panel shows an "env override" badge and disables editing); you still **Sign in** with your username and password from the panel.

| Env var | Value |
|---------|-------|
| `AGENTIC_OS_DESKTOP_REMOTE_URL` | `http://<backend-host>:9119` |
