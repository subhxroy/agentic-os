...
The Zed ACP Registry path launches the same adapter through `uvx --from 'agentic-os[acp]==<version>' hermes-acp`, pointed at the `agentic-os` PyPI release.
...
- ACP sessions are persisted to the shared `~/.agentic-os/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
