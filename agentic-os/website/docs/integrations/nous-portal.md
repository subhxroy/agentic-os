...
sidebar_position: 1
title: "Nous Portal"
description: "One subscription, 300+ frontier models, and the Tool Gateway — the recommended way to run Agentic OS"
---
...
The same subscription unlocks the [Tool Gateway](/user-guide/features/tool-gateway), which routes Agentic OS's tool calls through Nous-managed infrastructure. Five backends, one login:
...
If a model is genuinely missing, [open an issue](https://github.com/subhxroy/agentic-os/issues) — we surface the Portal's catalog to Hermes and gaps usually mean a routing config we can update.
