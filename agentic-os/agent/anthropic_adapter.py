"""Anthropic Messages API adapter for Agentic OS.
...
def _get_hermes_oauth_file() -> Path:
    return get_agentic_os_home() / ".anthropic_oauth.json"
...
def run_hermes_oauth_login_pure() -> Optional[Dict[str, Any]]:
    ...
    print()
    print("Authorize Agentic OS with your Claude Pro/Max subscription.")
    print()
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Agentic OS", "Claude Code")
                text = text.replace("Agentic OS agent", "Claude Code")
                text = text.replace("agentic-os", "claude-code")
