...
Nous can attribute usage to Agentic OS and bucket it by client release.
...
The version is sourced live from ``agentic_os_cli.__version__`` so it auto-aligns
...
``agentic_os_cli.__version__`` is the canonical source of truth.
"""
...
def _hermes_version() -> str:
    ...
    Falls back to ``"unknown"`` if ``agentic_os_cli`` cannot be imported (should
