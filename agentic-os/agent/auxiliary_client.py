...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.agentic-os/auth.json active provider)
...
# in lockstep with agentic_os_cli.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    ...
    Computed at call time so a hot-reloaded ``agentic_os_cli.__version__`` is
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Agentic OS)",
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    if getattr(entry, "provider", None) == "nous":
        ...
        from agentic_os_cli.auth import _nous_inference_env_override
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``agentic_os_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.agentic-os/auth.json for an active Nous provider.
...
def _nous_api_key(provider: dict) -> str:
    ...
    from agentic_os_cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from agentic_os_cli.auth import _agent_key_is_usable
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    Falls back to ``agentic_os_cli.auth``'s singleton runtime resolver for older
...
def _read_codex_access_token() -> Optional[str]:
    ...
    try:
        from agentic_os_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from agentic_os_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
...
def _try_azure_foundry(
    *,
    ...
    :func:`agentic_os_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from agentic_os_cli.runtime_provider import _resolve_azure_foundry_runtime
...
def _resolve_auto(
    ...
    #    old OPENAI_BASE_URL lingers in ~/.agentic-os/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "OPENAI_BASE_URL from ~/.agentic-os/.env",
...
def resolve_provider_client(
    ...
    try:
        ...
        logger.debug("agentic_os_cli.auth not available for provider %s", provider)
        return None, None
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`agentic_os_cli.plugins.PluginContext.register_auxiliary_task`) the
