...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$AGENTIC_OS_HOME`` (which is profile-scoped) alongside the other
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$AGENTIC_OS_HOME`` just means the notice
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .hermes.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / AGENTIC_OS_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.agentic-os/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from agentic_os_cli.config import load_config as _load_pc_cfg
    ...
    if agent.verbose_logging:
        ...
        # on the fact that agentic_os_logging.setup_logging() does not
        ...
        # handler level inside agentic_os_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        try:
            from agentic_os_cli.config import load_config as _load_br_cfg
    ...
    try:
        from agentic_os_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from agentic_os_cli.config import load_config as _load_agent_config
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Agentic OS.  Choose a model with at least "
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from agentic_os_cli.model_switch import _check_hermes_model_warning
