...
try:
    ...
    # shadowing Agentic OS's own modules — ``hermes acp`` can be started from any
...
def _load_env() -> None:
    ...
    loaded = load_agentic_os_dotenv(agentic_os_home=agentic_os_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", agentic_os_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="hermes-acp",
        description="Run Agentic OS as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Agentic OS version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Agentic OS provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
...
def _print_version() -> None:
    from agentic_os_cli import __version__ as hermes_version
...
def _run_check() -> None:
    ...
    print("Agentic OS ACP check OK")


def _run_setup() -> None:
    from agentic_os_cli.main import main as hermes_main
