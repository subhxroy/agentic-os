...
class QQAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # to ~/.agentic-os/.update_response. Set here so the cross-adapter gateway
    ...
    async def _default_interaction_dispatch(
        ...
        - ``update_prompt:<answer>`` →
          writes the answer to ``~/.agentic-os/.update_response`` for the
    ...
    async def send_update_prompt(
        ...
        ``~/.agentic-os/.update_response`` so the detached update process
        can read it.
        """
