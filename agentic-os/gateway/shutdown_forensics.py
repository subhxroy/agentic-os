    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        ...
        if agentic_os_home_str:
            takeover_path = Path(agentic_os_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(agentic_os_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded agentic-os but never re-ran ``hermes setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../agentic-os-gateway.service"
    ...
    # constant in agentic_os_cli/gateway.py.
    headroom = 30.0
