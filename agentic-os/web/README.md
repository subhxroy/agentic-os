...
Browser-based dashboard for managing Agentic OS configuration, API keys, and monitoring active sessions.
...
cd ../
python -m agentic_os_cli.main web --no-open
...
- **Do not introduce new `uppercase`** (the literal Tailwind class) in `agentic-os/web/src`. Prefer `text-display` for new brand chrome. Legacy `uppercase` call sites (e.g. `components/ui/label.tsx`, `card.tsx`) remain until migrated.
...
| Wordmark | `Typography` + size/tracking only | Sidebar/mobile “Agentic OS” — mixed case, no Mondwest, no `text-display` |
