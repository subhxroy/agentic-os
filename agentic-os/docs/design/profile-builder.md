...
needs, and a profile is just a AGENTIC_OS_HOME directory — so the dashboard is the
right home for a full-featured builder, and it can reuse everything that
already exists.

A profile = a full `~/.agentic-os/profiles/<name>/` directory with its own:
...
| Model + Provider | ModelsPage | `_write_profile_model(profile_dir, …)` | yes — AGENTIC_OS_HOME override, already wired into create endpoint |
| MCPs | McpPage | `mcp_config._save_mcp_server` + `/api/mcp/catalog` | yes — wrap with AGENTIC_OS_HOME override |
...
### Seam #1 — hub-skill install cannot use the AGENTIC_OS_HOME override

`tools/skills_hub.py` binds `SKILLS_DIR = AGENTIC_OS_HOME / "skills"` at **module
...
runs `python -m agentic_os_cli.main <subcommand>`, and `_apply_profile_override()`
...
A fresh subprocess re-imports `skills_hub` with the profile's AGENTIC_OS_HOME bound
...
2. write model (synchronous, AGENTIC_OS_HOME override)
3. write selected MCP servers (synchronous, AGENTIC_OS_HOME override)
...
class ProfileCreate(BaseModel):
    ...
    mcp_servers: List[MCPServerCreate] = []      # synchronous, AGENTIC_OS_HOME override
...
page afterward. Mirror `_write_profile_model`'s AGENTIC_OS_HOME-override helper for
...
- Backend E2E with isolated AGENTIC_OS_HOME: POST a full create body
