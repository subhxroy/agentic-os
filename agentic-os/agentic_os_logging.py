"""Centralized logging setup for Agentic OS.
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``agentic_os_cli.web_server``,
    ``agentic_os_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
from agentic_os_constants import get_config_path, get_agentic_os_home
...
COMPONENT_PREFIXES = {
    ...
    "tools": ("tools",),
    "cli": ("agentic_os_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "agentic_os_cli.web_server",
        "agentic_os_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
