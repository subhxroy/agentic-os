...
function createSandbox() {
  ...
  const agenticOSHome = path.join(root, 'hermes-home')
  const userDataDir = path.join(root, 'electron-user-data')
  fs.mkdirSync(agenticOSHome, { recursive: true })
  fs.mkdirSync(userDataDir, { recursive: true })
  return { root, agenticOSHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(agenticOSHome, mockUrl) {
  fs.writeFileSync(
    path.join(agenticOSHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(agenticOSHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.agenticOSHome, mock.url)
  console.log(`  AGENTIC_OS_HOME: ${sandbox.agenticOSHome}`)

  const electronBin = findElectron()

  const env = {
    ...process.env,
    AGENTIC_OS_HOME: sandbox.agenticOSHome,
    AGENTIC_OS_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    AGENTIC_OS_DESKTOP_IGNORE_EXISTING: '1',
    AGENTIC_OS_DESKTOP_AGENTIC_ROOT: REPO_ROOT,
    AGENTIC_OS_DESKTOP_APP_NAME: `HermesDevMock-${Date.now()}`,
  }
