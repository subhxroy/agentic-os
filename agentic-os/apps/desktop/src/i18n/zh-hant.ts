...
export const zhHant = defineLocale({
  ...
  boot: {
    ...
    steps: {
      ...
      startingHermesDesktop: '正在啟動 Agentic OS Desktop…'
    },
    errors: {
  ...
  notifications: {
    ...
    errors: {
      ...
      methodNotAllowed: '桌面後端拒絕了該請求 (405 Method Not Allowed)。請嘗試重新啟動 Agentic OS Desktop。',
  ...
  settings: {
    ...
    gateway: {
      ...
      envOverride: '環境變數覆寫',
      intro:
        'Agentic OS Desktop 預設會啟動自己的本機閘道。如果您希望此應用程式控制另一台機器或可信代理後面已執行的 Hermes 後端，請使用遠端閘道。在下方按設定檔指定各自的遠端主機。',
      appliesTo: '套用至',
      allProfiles: '全部設定檔',
      ...
      envOverrideDesc: '取消設定 AGENTIC_OS_DESKTOP_REMOTE_URL 和 AGENTIC_OS_DESKTOP_REMOTE_TOKEN 後才會使用下方儲存的設定。',
      localTitle: '本機閘道',
      ...
      savedTitle: '閘道設定已儲存',
      restartingMessage: 'Agentic OS Desktop 將使用已儲存的設定重新連線。',
  ...
  install: {
    ...
    failedTitle: '安裝失敗',
    settingUpTitle: '正在設定 Agentic OS',
    finishingTitle: '正在收尾',
    failedDesc:
      '某個安裝步驟失敗。在 Windows 上，如果另一個 Agentic OS CLI 或桌面執行個體正在執行，可能會出現這種情況。請停止正在執行的 Hermes 執行個體後重試。可查看下方的詳細資訊或 desktop 記錄中的完整記錄。',
  ...
  assistant: {
    ...
    approval: {
      ...
      alwaysDescription: pattern =>
        `這會將「${pattern}」模式加入永久允許清單（~/.agentic-os/config.yaml）。Hermes 對類似指令將不再詢問，包括目前工作階段和未來工作階段。`,
      alwaysAllow: '一律允許'
    },
    clarify: {
      notReady: '澄清請求尚未就緒',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '重新啟動 Agentic OS Desktop 後可使用儲存圖片。',
    restartToSaveImages: '重新啟動 Agentic OS Desktop 以儲存圖片',
