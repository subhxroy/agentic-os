 ...
 */
function writeMockProviderConfig(agenticOSHome: string, mockUrl: string): void {
  const configPath = path.join(agenticOSHome, 'config.yaml')
 ...
 */
function writeEnvFile(agenticOSHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(agenticOSHome, '.env')
 ...
 */
function writeEmptyConfig(agenticOSHome: string): void {
  const configPath = path.join(agenticOSHome, 'config.yaml')
 ...
 *  - AGENTIC_OS_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - AGENTIC_OS_DESKTOP_IGNORE_EXISTING=1 → don't pick up `hermes` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - AGENTIC_OS_DESKTOP_AGENTIC_ROOT → REPO_ROOT (dev checkout resolution)
 *  - AGENTIC_OS_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...
    AGENTIC_OS_DESKTOP_APP_NAME: `HermesE2E-${Date.now()}`,
...
export async function setupMockBackend(): Promise<MockBackendFixture> {
  ...
  writeMockProviderConfig(sandbox.agenticOSHome, mock.url)
  writeEnvFile(sandbox.agenticOSHome)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.agenticOSHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via AGENTIC_OS_DESKTOP_BOOT_FAKE_ERROR
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.agenticOSHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { AGENTIC_OS_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Hermes backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  ...
  if (process.platform === 'darwin') {
    ...
    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Hermes.app', 'Contents', 'MacOS', 'Agentic OS')
  }
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    AGENTIC_OS_DESKTOP_BOOT_FAKE: '1',
    AGENTIC_OS_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + hermes-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).AGENTIC_OS_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).AGENTIC_OS_DESKTOP_HERMES
  delete (env as Record<string, string | undefined>).AGENTIC_OS_DESKTOP_AGENTIC_ROOT
