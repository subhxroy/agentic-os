...
1. ``post_tool_call`` hook — inspects ``write_file`` and ``terminal``
   ...
   under ``AGENTIC_OS_HOME`` and tracks them silently.  Zero agent
   compliance required.
...
2. ``on_session_end`` hook — when any test files were auto-tracked
   ...
   logs a single line to ``$AGENTIC_OS_HOME/disk-cleanup/cleanup.log``.
...
All operations are scoped to AGENTIC_OS_HOME and /tmp/hermes-*.
...
def _handle_slash(raw_args: str) -> Optional[str]:
    ...
    if sub == "track":
        ...
        return (
            f"Not tracked (already present, missing, or outside AGENTIC_OS_HOME): "
            f"{path_arg}"
        )
