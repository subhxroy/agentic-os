...
Mounted at /api/plugins/agentic-os-achievements/ by Hermes dashboard.
"""
...
try:
    ...
    def get_agentic_os_home() -> Path:  # type: ignore[misc]
        val = (_os.environ.get("AGENTIC_OS_HOME") or "").strip()
...
def state_path() -> Path:
    return get_agentic_os_home() / "plugins" / "agentic-os-achievements" / "state.json"


def snapshot_path() -> Path:
    return get_agentic_os_home() / "plugins" / "agentic-os-achievements" / "scan_snapshot.json"


def checkpoint_path() -> Path:
    return get_agentic_os_home() / "plugins" / "agentic-os-achievements" / "scan_checkpoint.json"
...
def _start_background_scan() -> None:
    ...
    with _BACKGROUND_SCAN_LOCK:
        ...
        thread = threading.Thread(
            ...
            name="agentic-os-achievements-scan",
            daemon=True,
        )
