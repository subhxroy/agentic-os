...
Credential storage mirrors every other Hermes channel:
    ...
    * runtime SDK creds  -> ``~/.agentic-os/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.agentic-os/auth.json`` under
...
DEFAULT_PROJECT_NAME = "Agentic OS"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of agentic-os.

def _auth_json_path() -> Path:
    """Resolve ``~/.agentic-os/auth.json`` honouring the active Hermes profile."""
    try:
        from agentic_os_constants import get_agentic_os_home
        return Path(get_agentic_os_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.agentic-os")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.agentic-os/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.agentic-os/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.agentic-os/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: agentic_os_cli.config unavailable — skipping .env write")
        return
    try:
