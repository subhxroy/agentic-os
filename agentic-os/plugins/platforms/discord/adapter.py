...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.agentic-os/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.agentic-os/skills/ for new or removed skills")
        ...
        @tree.command(name="update", description="Update Agentic OS to the latest version")
        ...
        # agentic_os_cli/commands.py automatically appear as Discord slash
        ...
        dropped_over_cap = 0
        try:
            from agentic_os_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from agentic_os_cli.commands import _iter_plugin_command_entries
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from agentic_os_cli.commands import discord_skill_commands_by_category
    ...
    async def send_clarify(
        ...
        try:
            ...
            embed = discord.Embed(
                title="❓ Agentic OS needs your input",
                description=body,
                color=discord.Color.orange(),
            )
            ...
            content = self._self_contained_prompt_content(
                "❓ **Agentic OS needs your input**", str(question or "").strip(),
                tail=clarify_tail,
            )
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``agentic_os_cli.gateway.get_env_value`` at call time (not via
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'agentic-os[messaging]'",
        # Interactive setup wizard — replaces the central
        # agentic_os_cli/setup.py::_setup_discord function.  Same shape as Teams.
