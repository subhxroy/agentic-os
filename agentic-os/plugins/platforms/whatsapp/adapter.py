...
class WhatsAppAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # hardcodes ~/.agentic-os/{image,audio,document}_cache, which diverges
            # under AGENTIC_OS_HOME overrides, profiles, and the new cache/ layout.
            from gateway.platforms.base import (
...
# dict in agentic_os_cli/gateway.py, and the _send_whatsapp dispatch in
...
def interactive_setup() -> None:
    ...
    Replaces the central _setup_whatsapp in agentic_os_cli/gateway.py and the
...
def _is_connected(config) -> bool:
    ...
    if config is not None and getattr(config, "enabled", False) and extra:
        ...
        return True
    # Read via agentic_os_cli.gateway.get_env_value (not os.getenv) so setup-status
