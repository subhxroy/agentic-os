...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Agentic OS and the User"
    ...
    def save_config(self, values, agentic_os_home):
        ...
        import json
        from pathlib import Path
        config_dir = Path(agentic_os_home) / "hindsight"
    ...
    def post_setup(self, agentic_os_home: str, config: dict) -> None:
        ...
        self.save_config(provider_config, agentic_os_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(agentic_os_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(agentic_os_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Agentic OS and the User"},
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Agentic OS and the User")
