...
def _local_config_path() -> Path:
    ...
    Always returns $AGENTIC_OS_HOME/honcho.json so each profile/instance gets
...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom agentic_os_cli already uses for gateway refs).
    """
    try:
...
def cmd_setup(args) -> None:
    ...
    try:
        ...
        agentic_os_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(agentic_os_config)
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        from agentic_os_cli.memory_setup import cmd_setup_provider
