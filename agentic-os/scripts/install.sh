...
# Configuration
REPO_URL_SSH="git@github.com:subhxroy/agentic-os.git"
REPO_URL_HTTPS="https://github.com/subhxroy/agentic-os.git"
AGENTIC_OS_HOME="${AGENTIC_OS_HOME:-$HOME/.hermes}"
...
#   code at /usr/local/lib/agentic-os, command at /usr/local/bin/hermes,
#   data still at /root/.hermes (AGENTIC_OS_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$AGENTIC_OS_HOME/.no-bundled-skills so future"
            ...
            echo "                   default (non-root):  ~/.agentic-os/agentic-os"
            echo "                   default (root, Linux): /usr/local/lib/agentic-os"
            echo "  --hermes-home PATH  Data directory (default: ~/.agentic-os, or \$AGENTIC_OS_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Hermes installs the code under"
            echo "  /usr/local/lib/agentic-os and links the command into"
            echo "  /usr/local/bin/hermes (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$AGENTIC_OS_HOME"
            echo "  (default /root/.hermes).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$AGENTIC_OS_HOME/agentic-os are preserved in-place."
...
emit_manifest() {
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Agentic OS","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install hermes command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $AGENTIC_OS_HOME/agentic-os
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $AGENTIC_OS_HOME/agentic-os
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/agentic-os
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $AGENTIC_OS_HOME/agentic-os — then preserve it)
#
...
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in AGENTIC_OS_HOME.
    if is_termux; then
        INSTALL_DIR="$AGENTIC_OS_HOME/agentic-os"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$AGENTIC_OS_HOME/agentic-os/.git" ]; then
            INSTALL_DIR="$AGENTIC_OS_HOME/agentic-os"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/agentic-os)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/agentic-os"
        ...
        log_info "  Data:    $AGENTIC_OS_HOME (unchanged)"
...
# so global package binaries land in $AGENTIC_OS_HOME/node/bin — which is NOT on
...
configure_managed_node_npm_prefix() {
    [ -x "$AGENTIC_OS_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$AGENTIC_OS_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$AGENTIC_OS_HOME/node/etc/npmrc"
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://agentic-os.nousresearch.com/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Hermes owns its own uv at $AGENTIC_OS_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (hermes_cli/managed_uv.py) looks in the same
    # place, so install.sh and `hermes update` stay in sync.
    local _managed_uv="$AGENTIC_OS_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $AGENTIC_OS_HOME/bin ..."
    mkdir -p "$AGENTIC_OS_HOME/bin"
    ...
    # directly into $AGENTIC_OS_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$AGENTIC_OS_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
check_node() {
    ...
    if [ -x "$AGENTIC_OS_HOME/node/bin/node" ] && node_satisfies_build "$("$AGENTIC_OS_HOME/node/bin/node" --version)"; then
        export PATH="$AGENTIC_OS_HOME/node/bin:$PATH"
        log_success "Node.js $("$AGENTIC_OS_HOME/node/bin/node" --version) found (Hermes-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.agentic-os/node/..."
    ...
    # Place into ~/.agentic-os/node/ and symlink binaries into the same bin dir
    # the hermes command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$AGENTIC_OS_HOME/node"
    mkdir -p "$AGENTIC_OS_HOME"
    mv "$extracted_dir" "$AGENTIC_OS_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$AGENTIC_OS_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$AGENTIC_OS_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$AGENTIC_OS_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$AGENTIC_OS_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.agentic-os/node/"
    HAS_NODE=true
}
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Agentic OS itself does not require or retain root access."
...
EOF
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        PATH_COMMENT='# Agentic OS — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Agentic OS — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Agentic OS — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
...
copy_config_templates() {
    ...
    # Create ~/.agentic-os directory structure (config at top level, code in subdir)
    mkdir -p "$AGENTIC_OS_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.agentic-os/.env (top level, easy to find)
    if [ ! -f "$AGENTIC_OS_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$AGENTIC_OS_HOME/.env"
            log_success "Created ~/.agentic-os/.env from template"
        else
            touch "$AGENTIC_OS_HOME/.env"
            log_success "Created ~/.agentic-os/.env"
        fi
    else
        log_info "~/.agentic-os/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$AGENTIC_OS_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.agentic-os/config.yaml (top level, easy to find)
    if [ ! -f "$AGENTIC_OS_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$AGENTIC_OS_HOME/config.yaml"
            log_success "Created ~/.agentic-os/config.yaml from template"
        fi
    else
        log_info "~/.agentic-os/config.yaml already exists, keeping it"
    fi
    ...
    if [ ! -f "$AGENTIC_OS_HOME/SOUL.md" ]; then
        cat > "$AGENTIC_OS_HOME/SOUL.md" << 'SOUL_EOF'
You are Agentic OS, an intelligent AI assistant created by Nous Research. You are helpful, knowledgeable, and direct. You assist users with a wide range of tasks including answering questions, writing and editing code, analyzing information, creative work, and executing actions via your tools. You communicate clearly, admit uncertainty when appropriate, and prioritize being genuinely useful over being verbose unless otherwise directed below. Be targeted and efficient in your exploration and investigations.
SOUL_EOF
        log_success "Created ~/.agentic-os/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.agentic-os/"

    # Seed bundled skills into ~/.agentic-os/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        ...
        printf '%s\n' \
            ...
            > "$AGENTIC_OS_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $AGENTIC_OS_HOME/.no-bundled-skills"
        log_info "  Future 'hermes update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.agentic-os/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.agentic-os/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$AGENTIC_OS_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$AGENTIC_OS_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.agentic-os/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local env_file="$AGENTIC_OS_HOME/.env"
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Agentic OS browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Agentic OS browser tools — explicit browser override."
...
run_setup_wizard() {
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m agentic_os_cli.main setup < /dev/tty
    else
        python -m agentic_os_cli.main setup < /dev/tty
    fi
}
...
maybe_start_gateway() {
    ...
    WHATSAPP_SESSION="$AGENTIC_OS_HOME/whatsapp/session/creds.json"
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            nohup $HERMES_CMD gateway > "$AGENTIC_OS_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.agentic-os/logs/gateway.log"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $AGENTIC_OS_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $AGENTIC_OS_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $AGENTIC_OS_HOME/cron/, sessions/, logs/"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$AGENTIC_OS_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$AGENTIC_OS_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$AGENTIC_OS_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$AGENTIC_OS_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$AGENTIC_OS_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    if [ -n "$HERMES_CMD" ]; then
        ...
        log_info "Try: python -m agentic_os_cli.main setup"
    fi
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            # provisioned during prerequisites/node-deps (at $AGENTIC_OS_HOME/node/bin)
        ...
        complete)
            ...
            # $AGENTIC_OS_HOME. $AGENTIC_OS_HOME is a shared data dir (it can be
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $AGENTIC_OS_HOME.
    # $AGENTIC_OS_HOME is a shared data dir (it can be bind-mounted into a Docker
