---
name: agentic-os
description: "Configure, extend, or contribute to Agentic OS."
version: 2.3.0
author: Agentic OS + Teknium
license: MIT
...
Agentic OS is an open-source AI agent framework by Nous Research that runs in your terminal, a native desktop app, messaging platforms, and IDEs. It's in the same category as Claude Code (Anthropic), Codex (OpenAI), and OpenClaw — autonomous coding and task-execution agents that use tool calling to interact with your system. Hermes works with any LLM provider (OpenRouter, Anthropic, OpenAI, Google, DeepSeek, xAI, local models, and 20+ others) and runs on Linux, macOS, Windows, and WSL.
...
- User documentation: https://agentic-os.nousresearch.com/docs/
- Source tree: https://github.com/subhxroy/agentic-os
...
pip install agentic-os       # or: uv pip install agentic-os
...
/reload-skills       Re-scan ~/.agentic-os/skills/ for added/removed skills
...
```
~/.agentic-os/config.yaml       Main configuration
~/.agentic-os/.env              API keys and secrets (under $AGENTIC_OS_HOME if set)
$AGENTIC_OS_HOME/skills/        Installed skills
~/.agentic-os/sessions/         Gateway routing index, request dumps, *.jsonl transcripts (and optional per-session JSON snapshots when sessions.write_json_snapshots: true)
~/.agentic-os/state.db          Canonical session store (SQLite + FTS5)
~/.agentic-os/logs/             Gateway and error logs
~/.agentic-os/auth.json         OAuth tokens and credential pools
~/.agentic-os/agentic-os/     Source code (if git-installed)
```

Profiles use `~/.agentic-os/profiles/<name>/` with the same layout.
...
`SOUL.md` (in `$AGENTIC_OS_HOME`) is independent and always loaded when present — it sets the agent's identity, not project rules.
...
- **Don't put project rules in `~/.agentic-os/AGENTS.md`** (or any other home-level location). When Hermes runs with that directory as cwd, the file loads — but only for that one directory. For cross-project context, use `SOUL.md` (in `$AGENTIC_OS_HOME`, identity-only) or install a skill via `hermes skills install`.
...
Some shell-hook integrations require explicit allowlisting before they fire. Managed via `~/.agentic-os/shell-hooks-allowlist.json` — prompted interactively the first time a hook wants to run.
...
- **Cost:** the deterministic inactivity/prune sweep runs for free. The
  ...
  **off by default** — opt in with `curator.consolidate: true` or
  ...
  zero tokens.
- **Telemetry:** sidecar at `~/.agentic-os/skills/.usage.json` holds
...
Check logs first:
```bash
grep -i "failed to send\|error" ~/.agentic-os/logs/gateway.log | tail -20
```

Common gateway problems:
- **Gateway dies on SSH logout**: Enable linger: `sudo loginctl enable-linger $USER`
- **Gateway dies on WSL2 close**: WSL2 requires `systemd=true` in `/etc/wsl.conf` for systemd services to work. Without it, gateway falls back to `nohup` (dies when session closes).
- **Gateway crash loop**: Reset the failed state: `systemctl --user reset-failed agentic-os-gateway`
...
| Gateway logs | `~/.agentic-os/logs/gateway.log` |
| Session files | `hermes sessions browse` (reads state.db) |
| Source code | `~/.agentic-os/agentic-os/` |
...
├── agentic_os_state.py       # SQLite session store
...
Config: `~/.agentic-os/config.yaml` (settings), `~/.agentic-os/.env` (API keys) — both under `$AGENTIC_OS_HOME` when it is set.
...
All handlers must return JSON strings. Use `get_agentic_os_home()` for paths,
never hardcode `~/.agentic-os`. For custom/local-only tools, write a plugin in
`~/.agentic-os/plugins/` instead of editing core — see the developer docs.
...
- Tests auto-redirect `AGENTIC_OS_HOME` to temp dirs — never touch real `~/.agentic-os/`.
...
- Use `get_agentic_os_home()` from `agentic_os_constants` for all paths (profile-safe)
