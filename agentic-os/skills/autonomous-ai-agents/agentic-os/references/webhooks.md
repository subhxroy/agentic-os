...
Add to `${AGENTIC_OS_HOME:-~/.agentic-os}/.env`:
```bash
...
- **Route scripts** (`--script` on subscribe, or `script:` on a config route): a script under `~/.agentic-os/scripts/` receives the payload as JSON on stdin. JSON stdout replaces the payload before prompt templating; empty stdout, `[SILENT]`, or a nonzero exit ignores the webhook. `.sh`/`.bash` run with bash, everything else with Python. Scripts cannot live outside `~/.agentic-os/scripts/` (path traversal is blocked).
...
Full filter syntax: https://agentic-os.nousresearch.com/docs/user-guide/messaging/webhooks#payload-filters
...
- Subscriptions persist to `~/.agentic-os/webhook_subscriptions.json`

## How It Works

1. `hermes webhook subscribe` writes to `~/.agentic-os/webhook_subscriptions.json`
...
1. **Is the gateway running?** Check with `systemctl --user status agentic-os-gateway` or `ps aux | grep gateway`
2. **Is the webhook server listening?** `curl http://localhost:8644/health` should return `{"status": "ok"}`
3. **Check gateway logs:** `grep webhook ~/.agentic-os/logs/gateway.log | tail -20`
