...
4. Store the token in `${AGENTIC_OS_HOME:-~/.agentic-os}/.env` (or via `hermes setup`):
   ```
...
- **`AIRTABLE_API_KEY` flows from `${AGENTIC_OS_HOME:-~/.agentic-os}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
