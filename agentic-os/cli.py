#!/usr/bin/env python3
"""
Agentic OS CLI - Interactive Terminal Interface

A beautiful command-line interface for Agentic OS.
...
from agentic_os_cli.cli_agent_setup_mixin import CLIAgentSetupMixin
from agentic_os_cli.cli_commands_mixin import CLICommandsMixin
from agentic_os_cli.cli_billing_mixin import CLIBillingMixin
...
try:
    from agentic_os_cli.pt_input_extras import (
...
from agentic_os_cli.banner import _format_context_length, format_banner_version_label
...
def _load_prefill_messages(file_path: str) -> List[Dict[str, Any]]:
    ...
    Relative paths are resolved from ~/.agentic-os/.
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.agentic-os/config.yaml (user config - preferred)
    ...
    the user config at ``~/.agentic-os/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({AGENTIC_OS_HOME}/config.yaml)
    user_config_path = _agentic_os_home / 'config.yaml'
    ...
    defaults = {
        ...
        "display": {
            "compact": False,
            "resume_display": "full",
            # Recap tuning for /resume — see agentic_os_cli/config.py DEFAULT_CONFIG.
            "resume_exchanges": 10,
            ...
            # agentic_os_cli/config.py DEFAULT_CONFIG (display.show_reasoning).
            "show_reasoning": True,
    ...
    if config_path.exists():
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                from agentic_os_cli.config import _normalize_root_model_keys
    ...
    # agentic_os_cli.config._load_config_impl (which has its own managed merge), so
    ...
    # Normalize config key: the new config system (agentic_os_cli/config.py) and all
...
# Initialize centralized logging early — agent.log + errors.log in ~/.agentic-os/logs/.
# This ensures CLI sessions produce a log trail even before AIAgent is instantiated.
try:
    from agentic_os_logging import setup_logging
...
try:
    from agentic_os_cli.config import print_config_warnings
...
try:
    from agentic_os_cli.skin_engine import init_skin_from_config
...
from agentic_os_cli.banner import build_welcome_banner
from agentic_os_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from agentic_os_cli.callbacks import prompt_for_secret
...
def _resolve_worktree_base(repo_root: str) -> tuple:
    ...
    (the ``~/.agentic-os/agentic-os`` clone is updated only by ``hermes update``,
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`agentic_os_cli.config.load_config` (the authoritative loader that
    ...
    try:
        ...
        _agentic_os_home_maint = _get_agentic_os_home()
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_agentic_os_home_maint / "sessions"
                )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`agentic_os_cli.config.load_config`. Honours ``auto_prune`` /
...
#
# Detection priority:
#   1. HERMES_LIGHT / AGENTIC_OS_TUI_LIGHT env (true/false) — explicit override
#   2. AGENTIC_OS_TUI_THEME=light|dark — explicit theme
#   3. AGENTIC_OS_TUI_BACKGROUND=#RRGGBB — explicit bg hint
...
def _detect_light_mode() -> bool:
    ...
    try:
        ...
        for var in ("HERMES_LIGHT", "AGENTIC_OS_TUI_LIGHT"):
            ...
            if _FALSE_RE.match(v):
                ...
                return result
        # 2. Theme hint
        theme = (os.environ.get("AGENTIC_OS_TUI_THEME") or "").strip().lower()
        if theme == "light":
            result = True
            _LIGHT_MODE_CACHE = result
            return result
        if theme == "dark":
            ...
            return result
        # 3. Explicit bg hex
        bg_hint = os.environ.get("AGENTIC_OS_TUI_BACKGROUND") or ""
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from agentic_os_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
...
class _SkinAwareAnsi:
    ...
    def __str__(self) -> str:
        if self._cached is None:
            try:
                from agentic_os_cli.skin_engine import get_active_skin
...
from agentic_os_constants import is_termux as _is_termux_environment
...
def _strip_leaked_bracketed_paste_wrappers(text: str) -> str:
    from agentic_os_cli.input_sanitize import strip_leaked_bracketed_paste_wrappers
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default" or skin_name == "agentic-os":
        line1 = "AGENTIC OS - Personal AI Agent"
        tiny_line = "AGENTIC OS"
    else:
        agent_name = _skin.get_branding("agent_name", "Agentic OS") if _skin else "Agentic OS"
        line1 = f"{agent_name} - AI Agent Framework"
        tiny_line = agent_name

    if os.environ.get("HERMES_FAST_STARTUP_BANNER") == "1":
        from agentic_os_cli import __release_date__ as _release_date
        from agentic_os_cli import __version__ as _version

        version_line = f"Agentic OS v{_version} ({_release_date})"
    else:
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from agentic_os_cli.plugins import get_plugin_commands
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.agentic-os/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.agentic-os/config.yaml on first use)
...
class HermesCLI(CLIAgentSetupMixin, CLICommandsMixin, CLIBillingMixin):
    """
    Interactive CLI for Agentic OS.
    ...
    def __init__(
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from agentic_os_cli.runtime_provider import _auto_detect_local_model
        ...
        # by `hermes chat --ignore-rules` in agentic_os_cli/main.py. When true we
        ...
        # shared chokepoint in agentic_os_constants (Closes #21256).
        ...
        # it's shared across all Hermes processes for this AGENTIC_OS_HOME.
        # Never blocks startup on failure.
        _run_state_db_auto_maintenance(self._session_db)

        # Opportunistic shadow-repo cleanup — deletes orphan/stale
        # checkpoint repos under ~/.agentic-os/checkpoints/.  Opt-in via
        ...
        self._history_file = _agentic_os_home / ".hermes_history"
    ...
    def _recover_terminal_after_interrupt(self) -> None:
        ...
        """
        try:
            from agentic_os_cli.curses_ui import flush_stdin
            flush_stdin()
        except Exception:
            pass
    ...
    def _get_status_bar_snapshot(self) -> Dict[str, Any]:
        ...
        if model_short == model_name:
            ...
            from agentic_os_cli.model_switch import format_model_for_display
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from agentic_os_cli.voice import format_voice_record_key_for_status
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from agentic_os_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from agentic_os_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from agentic_os_cli.models import normalize_opencode_model_id, opencode_model_api_mode
        ...
        if self._model_is_default:
            ...
            try:
                from agentic_os_cli.codex_models import get_codex_model_ids
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from agentic_os_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", " agentic ")
    ...
    def _show_security_advisories(self):
        ...
        """
        try:
            from agentic_os_cli.security_advisories import (
                detect_compromised,
                startup_banner,
            )
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Agentic OS needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        from agentic_os_cli.model_switch import is_nous_hermes_non_agentic
        ...
        if is_nous_hermes_non_agentic(model_name):
            ...
            self._console_print(
                ...
                "designed for use with Agentic OS.[/]"
            )
            self._console_print(
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.agentic-os/images/ and appends the path to
    ...
    def _fast_command_available(self) -> bool:
        try:
            from agentic_os_cli.models import model_supports_fast_mode
        except Exception:
            return False
        agent = getattr(self, "agent", None)
    ...
    def show_help(self):
        ...
        try:
            from agentic_os_cli.skin_engine import get_active_help_header
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Agentic OS!{_RST}")
    ...
    def _list_recent_sessions(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        if not self._session_db:
            return []
        try:
            from agentic_os_cli.session_listing import query_session_listing
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from agentic_os_cli.main import _relative_time
    ...
    def new_session(self, silent=False, title=None):
        ...
        if _config_model and _config_model != getattr(self, "model", None):
            ...
            try:
                from agentic_os_cli.model_switch import switch_model as _switch_model
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.agentic-os/sessions/saved/.
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from agentic_os_cli.curses_ui import curses_single_select
    ...
    def _confirm_expensive_model_switch(self, result) -> bool:
        ...
        choices = [
            ("once", "Switch anyway", "Use this model for the current Agentic OS session."),
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        try:
            from agentic_os_cli.model_switch import resolve_display_context_length
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            if not model_list:
                try:
                    from agentic_os_cli.models import provider_model_ids
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in agentic_os_cli/commands.py instead of touching every dispatch site.
        from agentic_os_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            if self._app:
                ...
                try:
                    from agentic_os_cli.tips import get_random_tip
                    _tip = get_random_tip()
                    try:
                        from agentic_os_cli.skin_engine import get_active_skin
            ...
            if len(parts) > 1:
                ...
                if raw_title:
                    if self._session_db:
                        ...
                        from agentic_os_state import format_session_db_unavailable
            ...
            from agentic_os_cli.main import _print_version_info
            ...
            from agentic_os_cli.moa_config import (
                moa_usage,
    ...
    def _toggle_verbose(self):
        ...
        from agentic_os_cli.colors import Colors as _Colors
        labels = {
    ...
    def _manual_compress(self, cmd_original: str = ""):
        ...
        * ``/compress here [N]`` — boundary-aware compression. Summarize
        ...
        from agentic_os_cli.partial_compress import (
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by agentic_os_logging not
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        * When opted out (``mcp.auto_reload_on_config_change: false``) it
        ...
        from agentic_os_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.agentic-os/skills/ and queue a note for the
        next user turn.
                ...
                try:
                    if (
                        ...
                        if not is_seen(CLI_CONFIG, TOOL_PROGRESS_FLAG):
                            ...
                            mark_seen(_agentic_os_home / "config.yaml", TOOL_PROGRESS_FLAG)
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _agentic_os_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self, clear_screen: bool = True):
        ...
        if msg_count > 0:
            ...
            # profile names use the standard AGENTIC_OS_HOME, so no -p needed.
            try:
            ...
            try:
                from agentic_os_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from agentic_os_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from agentic_os_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Agentic OS ready. Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Agentic OS! Type your message or /help for commands."
        ...
        try:
            from agentic_os_cli.model_switch import prewarm_picker_cache_async
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from agentic_os_cli.config import get_config_path as _get_cfg_path_resid
        ...
        def handle_ignored_terminal_sequence(event):
            ...
            install_ignored_terminal_sequences() in agentic_os_cli.pt_input_extras
        ...
        def handle_enter(event):
            ...
            if self._model_picker_state:
                try:
                    ...
                    from agentic_os_cli.model_switch import resolve_persist_behavior
                ...
                if self._agent_running and not (text and _looks_like_slash_command(text)):
                    ...
                    try:
                        ...
                        if not is_seen(CLI_CONFIG, BUSY_INPUT_FLAG):
                            ...
                            mark_seen(_agentic_os_home / "config.yaml", BUSY_INPUT_FLAG)
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from agentic_os_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Agentic OS")
        ...
        try:
            from agentic_os_cli.config import load_config
            from agentic_os_cli.voice import (
        ...
        def handle_paste(event):
            ...
            if pasted_text:
                ...
                if (lines_hit or chars_hit) and not buf.text.strip().startswith('/'):
                    _paste_counter[0] += 1
                    paste_dir = _agentic_os_home / "pastes"
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Agentic OS needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Agentic OS needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Agentic OS needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
...
def _run_kanban_goal_loop_q(cli: "HermesCLI", first_response: str) -> None:
    ...
    from agentic_os_cli.goals import run_kanban_goal_loop as _run_loop, DEFAULT_MAX_TURNS as _DEF_TURNS
...
def main(
    ...
    """
    Agentic OS CLI - Interactive AI Assistant
    ...
    if gateway:
        ...
        print("Starting Agentic OS Gateway (messaging platforms)...")
                    ...
                    if cli._init_agent(
                        ...
                        if isinstance(result, dict) and result.get("failed"):
                            ...
                            if os.environ.get("HERMES_KANBAN_TASK") and result.get(
                                ...
                                try:
                                    from agentic_os_cli.kanban_db import (
