#!/usr/bin/env python3
"""
Hyperliquid CLI Tool for Agentic OS
-------------------------------------
...
def _agentic_os_home() -> Path:
    return Path(os.environ.get("AGENTIC_OS_HOME", "~/.agentic-os")).expanduser()
...
def _resolve_user(user: Optional[str]) -> str:
    ...
    sys.exit(
        ...
        f"{DEFAULT_USER_ENV} in your environment or {_agentic_os_home() / '.env'}."
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Hyperliquid CLI Tool for Agentic OS")
