...
def _write_auth_store(tmp_path, payload: dict) -> None:
    ...
    (agentic_os_home / "auth.json").write_text(json.dumps(payload, indent=2))
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    ...
    config_text = (agentic_os_home / "config.yaml").read_text()
...
def test_logout_clears_stale_active_codex_without_provider_credentials(tmp_path, monkeypatch, capsys):
    ...
    out = capsys.readouterr().out
    assert "Logged out of OpenAI Codex." in out
    auth_payload = json.loads((agentic_os_home / "auth.json").read_text())
...
def test_auth_remove_claude_code_suppresses_reseed(tmp_path, monkeypatch):
    ...
    updated = json.loads((agentic_os_home / "auth.json").read_text())
...
def test_seed_from_singletons_respects_codex_suppression(tmp_path, monkeypatch):
    ...
    after = json.loads((agentic_os_home / "auth.json").read_text())
...
def test_auth_remove_env_seeded_suppresses_shell_exported_var(tmp_path, monkeypatch, capsys):
    ...
    by the shell (not written into ~/.agentic-os/.env).  Before PR for #13371 the
    ...
    (agentic_os_home / ".env").write_text("")
...
def test_auth_remove_env_seeded_dotenv_only_no_shell_hint(tmp_path, monkeypatch, capsys):
    """When the env var lives only in ~/.agentic-os/.env (not the shell), the
    ...
    (agentic_os_home / ".env").write_text("DEEPSEEK_API_KEY=sk-ds-only\n")
    ...
    assert (agentic_os_home / ".env").read_text().strip() == ""
...
def test_seed_from_singletons_respects_hermes_pkce_suppression(tmp_path, monkeypatch):
    """anthropic hermes_pkce must not re-seed from ~/.agentic-os/.anthropic_oauth.json when suppressed."""
    agentic_os_home = tmp_path / "hermes"
    agentic_os_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("AGENTIC_OS_HOME", str(agentic_os_home))

    import yaml
    (agentic_os_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (agentic_os_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
...
def test_seed_custom_pool_respects_config_suppression(tmp_path, monkeypatch):
    ...
    import yaml
    (agentic_os_home / "config.yaml").write_text(yaml.dump({
        "model": {},
        "custom_providers": [
...
def test_credential_sources_registry_has_expected_steps():
    ...
    required = {
        ...
        "~/.agentic-os/.anthropic_oauth.json",
