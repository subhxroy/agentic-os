...
These tests run against a fake $AGENTIC_OS_HOME under tmp_path; no real
...
def test_missing_profiles_root_still_registers_default_slot(
    ...
    """When $AGENTIC_OS_HOME/profiles doesn't exist (fresh install), the
...
def test_default_slot_always_registered_on_empty_home(tmp_path: Path) -> None:
    """Bare AGENTIC_OS_HOME with nothing under it still produces a
...
def test_default_slot_run_script_omits_profile_flag(tmp_path: Path) -> None:
    ...
    that would resolve to $AGENTIC_OS_HOME/profiles/default/ instead of
...
def test_default_slot_autostarts_when_root_state_running(tmp_path: Path) -> None:
    """gateway_state.json at the AGENTIC_OS_HOME root with state=running
...
def test_default_slot_cleans_up_stale_runtime_files_at_root(
    ...
    """gateway.pid and processes.json at the AGENTIC_OS_HOME root (left
