...
class TestSystemdServiceRefresh:
    def test_systemd_install_repairs_outdated_unit_without_force(self, tmp_path, monkeypatch):
        unit_path = tmp_path / "agentic-os-gateway.service"
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``AGENTIC_OS_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../hermes_test`` AGENTIC_OS_HOME — silently breaking
        ...
        # Realistic generated unit referencing a pytest tmpdir AGENTIC_OS_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="AGENTIC_OS_HOME=/tmp/pytest-of-alice/pytest-42/'
    ...
    def test_refresh_refuses_to_bake_any_tempdir_home_into_real_user_unit(
        ...
        """Structural guard: a manual E2E AGENTIC_OS_HOME like
        ...
        ``hermes gateway restart`` with a /tmp AGENTIC_OS_HOME exported; the
        ...
        must refuse ANY temp-dir AGENTIC_OS_HOME, not just pytest-shaped ones.
        """
        unit_path = tmp_path / "agentic-os-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
        ...
        polluted_unit = (
            "[Service]\n"
            'Environment="AGENTIC_OS_HOME=/tmp/hermes-e2e-41264"\n'
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_var_tmp_home(self):
        unit = '[Service]\nEnvironment="AGENTIC_OS_HOME=/var/tmp/hermes-x"\n'
    ...
    def test_detects_tempdir_env_home(self, monkeypatch, tmp_path):
        ...
        unit = f'[Service]\nEnvironment="AGENTIC_OS_HOME={tmp_path}/hermes-home"\n'
        assert gateway_cli._temp_home_in_service_definition(unit) is not None
    ...
    def test_accepts_real_home(self):
        unit = '[Service]\nEnvironment="AGENTIC_OS_HOME=/home/alice/.hermes"\n'
    ...
    def test_accepts_unit_without_agentic_os_home(self):
    ...
    def test_tmp_prefixed_non_temp_path_is_accepted(self):
        ...
        unit = '[Service]\nEnvironment="AGENTIC_OS_HOME=/tmpfs-data/.hermes"\n'
        assert gateway_cli._temp_home_in_service_definition(unit) is None
...
class TestGeneratedSystemdUnits:
    ...
    def test_system_unit_includes_wsl_windows_interop_paths(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "_agentic_os_home_for_target_user", lambda home: "/home/alice/.hermes")
        monkeypatch.setenv("PATH", "/usr/local/bin:/mnt/c/WINDOWS/system32")
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
...
class TestLaunchdServiceRecovery:
    ...
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        ...
        # AGENTIC_OS_HOME resolves under the (pytest tmp) test AGENTIC_OS_HOME.
        monkeypatch.setattr(
            gateway_cli,
    ...
    def test_launchd_install_falls_back_to_detached_on_bootstrap_5(self, tmp_path, monkeypatch, capsys):
        ...
        # (which would trip on the pytest-tmp test AGENTIC_OS_HOME) stays out of
        # the way — this test exercises the bootstrap-error fallback.
        monkeypatch.setattr(
            gateway_cli,
            "generate_launchd_plist",
            lambda: (
                "<plist><key>AGENTIC_OS_HOME</key>"
                "<string>/Users/alice/.hermes</string></plist>"
            ),
        )
...
class TestSystemUnitHermesHome:
    """AGENTIC_OS_HOME in system units must reference the target user, not root."""
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'AGENTIC_OS_HOME=/home/alice/.hermes' in unit
        assert '/root/.hermes' not in unit

    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: AGENTIC_OS_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("AGENTIC_OS_HOME", "/root/.hermes/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'AGENTIC_OS_HOME=/home/alice/.hermes/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_agentic_os_home(self, monkeypatch):
        # Custom AGENTIC_OS_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("AGENTIC_OS_HOME", "/opt/hermes-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'AGENTIC_OS_HOME=/opt/hermes-shared' in unit
...
class TestSystemUnitRefreshSyncsHermesHome:
    ...
    def test_refresh_adopts_unit_agentic_os_home_before_rewriting(self, tmp_path, monkeypatch):
        ...
        # Correct installed unit (operator's AGENTIC_OS_HOME + drain timeout).
        monkeypatch.setenv("AGENTIC_OS_HOME", str(alice_hermes))
        ...
        # Simulate sudo without inherited AGENTIC_OS_HOME (falls back to root).
        monkeypatch.setenv("AGENTIC_OS_HOME", str(root_hermes))
        ...
        assert os.environ["AGENTIC_OS_HOME"] == str(alice_hermes)
    ...
    def test_is_current_syncs_before_reading_unit(self, tmp_path, monkeypatch):
        ...
        unit's pinned AGENTIC_OS_HOME *before* it reads/compares the unit.
        ...
        monkeypatch.setattr(gateway_cli, "_sync_agentic_os_home_from_systemd_unit", tracking_sync)
    ...
    def test_start_and_restart_delegate_sync_to_chokepoint(self, monkeypatch):
        ...
        for entry in ("systemd_start", "systemd_restart"):
            ...
            monkeypatch.setattr(
                gateway_cli,
                "_sync_agentic_os_home_from_systemd_unit",
...
class TestHermesHomeForTargetUser:
    """Unit tests for _agentic_os_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("AGENTIC_OS_HOME", raising=False)

        result = gateway_cli._agentic_os_home_for_target_user("/home/alice")
    ...
    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("AGENTIC_OS_HOME", "/opt/hermes")
...
class TestProfileArg:
    ...
    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.agentic-os/profiles/mybot should return '--profile mybot'."""
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile AGENTIC_OS_HOME should return empty string."""
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.agentic-os/profiles/mybot/subdir should NOT match — too deep."""
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("AGENTIC_OS_HOME", str(root_profile))
        ...
        assert f'AGENTIC_OS_HOME={target_home / ".hermes" / "profiles" / "mybot"}' in unit
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("AGENTIC_OS_HOME", str(root_home / ".hermes"))
        ...
        # WorkingDirectory is anchored at the target user's AGENTIC_OS_HOME (stable,
        ...
        assert "WorkingDirectory=/home/alice/.hermes/agentic-os" not in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "agentic-os-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "agentic-os-gateway"])
...
class TestLegacyHermesUnitDetection:
    ...
    renamed to ``agentic-os-gateway.service``. After PR #5646 (signal recovery
    ...
    (``agentic-os-gateway-<profile>.service``) or unrelated third-party services.
    """
    ...
    def test_ignores_profile_unit_hermes_gateway_coder(self, tmp_path, monkeypatch):
        ...
        Teknium's concern — ``agentic-os-gateway-coder.service`` is our standard
        ...
        for base in (user_dir, system_dir):
            (base / "agentic-os-gateway-coder.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "agentic-os-gateway-orcha.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "agentic-os-gateway.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
...
class TestRemoveLegacyHermesUnits:
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (agentic-os-gateway-coder.service)
        ...
        profile_unit = user_dir / "agentic-os-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "agentic-os-gateway.service"
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "agentic-os-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "agentic-os-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "agentic-os-gateway.service",
        )
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "agentic-os-gateway")
        ...
        assert "sudo systemctl start agentic-os-gateway" in out
    ...
    def test_restart_remediation_uses_systemctl_restart(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl restart agentic-os-gateway" in out
    ...
    def test_stop_remediation_uses_systemctl_stop(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl stop agentic-os-gateway" in out
...
class TestServiceWorkingDirIsStable:
    ...
    (AGENTIC_OS_HOME), never the source checkout / worktree, so a relocated or
    deleted checkout can't crash-loop the unit on CHDIR (status=200).
    """

    def test_stable_working_dir_uses_agentic_os_home(self, tmp_path, monkeypatch):
    ...
    def test_stable_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        # AGENTIC_OS_HOME points somewhere that does not exist -> fall back.
    ...
    def test_user_unit_workingdirectory_is_agentic_os_home_not_checkout(self, tmp_path, monkeypatch):
        home = tmp_path / ".hermes"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_agentic_os_home", lambda: home)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_workingdirectory_is_agentic_os_home(self, tmp_path, monkeypatch):
        import re
