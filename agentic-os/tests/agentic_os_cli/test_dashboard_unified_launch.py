...
class TestUnifiedDashboardRouting:
    def test_profile_launch_attaches_to_running_dashboard(self, main_mod, monkeypatch):
        monkeypatch.setattr(
            "agentic_os_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        # AGENTIC_OS_HOME.  For a standard install (AGENTIC_OS_HOME unset) that root is
        # the platform-native default (~/.agentic-os), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from agentic_os_constants import get_default_agentic_os_root
        assert env.get("AGENTIC_OS_HOME") == str(get_default_agentic_os_root())

    def test_reexec_pins_docker_machine_root(self, main_mod, monkeypatch):
        """In the Docker layout (AGENTIC_OS_HOME=/opt/data, profiles under
        /opt/data/profiles/<name>) the reroute must pin the child to the
        machine root /opt/data — NOT drop AGENTIC_OS_HOME.
        ...
        """
        monkeypatch.setenv("AGENTIC_OS_HOME", "/opt/data/profiles/oracle")
        monkeypatch.setattr(
            "agentic_os_cli.profiles.get_active_profile_name", lambda: "oracle"
        )
        monkeypatch.setattr(main_mod, "_dashboard_listening", lambda host, port: False)
        execs = []
        ...
        assert env.get("AGENTIC_OS_HOME") == "/opt/data"
