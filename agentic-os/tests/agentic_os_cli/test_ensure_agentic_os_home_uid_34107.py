"""Regression tests for #34107 — Docker UID/GID handling in ensure_agentic_os_home.

When Hermes runs in Docker with ``HERMES_UID=1000`` / ``HERMES_GID=911``,
the entrypoint chowns the top-level ``AGENTIC_OS_HOME`` once at startup. But
...
class TestResolveHermesUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        ...
        from agentic_os_cli.config import _resolve_hermes_uid_gid
