"""Regression tests for TOCTOU-safe credential file writers in ``agentic_os_cli.auth``.
...
# ---------------------------------------------------------------------------
# _save_auth_store  (~/.agentic-os/auth.json — every native OAuth provider)
# ---------------------------------------------------------------------------
...
def test_shared_nous_store_writes_0o600_with_0o700_parent(tmp_path, monkeypatch):
    ...
    # home" reference — which currently tracks AGENTIC_OS_HOME via
    # get_default_agentic_os_root() — can't collide with our override and
