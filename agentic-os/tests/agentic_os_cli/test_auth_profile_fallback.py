...
When ``AGENTIC_OS_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Profile     -> tmp_path/.hermes/profiles/coder   (active, AGENTIC_OS_HOME points here)
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (AGENTIC_OS_HOME == global root), no fallback path runs.
    ...
    # to the AGENTIC_OS_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    agentic_os_home = tmp_path / "classic"
    ...
    # Classic mode: AGENTIC_OS_HOME is set to a custom path that is NOT under
    # ~/.agentic-os/profiles/ — get_default_agentic_os_root() returns AGENTIC_OS_HOME
