...
""".lstrip(),
    ...
    assert pid == 4242
    assert captured["env"]["AGENTIC_OS_HOME"] == str(profile)
...
def test_default_spawn_never_boots_the_tui(monkeypatch, tmp_path):
    """Workers are headless: an inherited AGENTIC_OS_TUI=1 (or a TUI-default
    ...
    flag) and strips AGENTIC_OS_TUI from the child env."""
    root = tmp_path / ".hermes"
    ...
    assert "AGENTIC_OS_TUI" not in captured["env"]
