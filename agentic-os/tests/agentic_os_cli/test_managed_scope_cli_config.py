...
agentic_os_cli.config._load_config_impl, so the managed-scope merge has to be
...
def _load_cli_config(home):
    ...
    cli.py binds ``_agentic_os_home = get_agentic_os_home()`` at import time (module
    singleton), so monkeypatching AGENTIC_OS_HOME after import doesn't move it.
    ...
    this only matters for tests that swap AGENTIC_OS_HOME.)
    """
    import cli

    cli._agentic_os_home = home
