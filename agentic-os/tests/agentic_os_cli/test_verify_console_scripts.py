...
def temp_pyproject(tmp_path, monkeypatch):
    ...
    pyproject.write_text(
        ...
        [project.scripts]
        hermes = "agentic_os_cli.main:main"
        agentic-os = "run_agent:main"
...
class TestVerifyConsoleScriptsInstalled:
    def test_no_action_when_all_shims_present(self, temp_pyproject, fake_scripts_dir):
        ...
        with patch("agentic_os_cli.main._is_windows", return_value=True), \
             patch("agentic_os_cli.main._venv_scripts_dir", return_value=fake_scripts_dir), \
             patch("agentic_os_cli.main._run_quarantined_install") as mock_install:
            from agentic_os_cli.main import _verify_console_scripts_installed
    ...
    def test_triggers_reinstall_when_hermes_exe_missing(
        ...
        (fake_scripts_dir / "agentic-os.exe").write_bytes(b"fake")
    ...
    def test_skips_off_windows(self, temp_pyproject, fake_scripts_dir):
        with patch("agentic_os_cli.main._is_windows", return_value=False), \
             patch("agentic_os_cli.main._run_quarantined_install") as mock_install:
            from agentic_os_cli.main import _verify_console_scripts_installed
    ...
    def test_primary_install_success_still_verifies_scripts(self):
        ...
        with patch("agentic_os_cli.main._is_windows", return_value=False), \
             patch("agentic_os_cli.main._run_quarantined_install") as mock_install, \
             patch("agentic_os_cli.main._verify_console_scripts_installed") as mock_verify:
    ...
    def test_quarantine_shims_include_declared_console_scripts(
        ...
        assert {"hermes.exe", "agentic-os.exe", "hermes-acp.exe"} <= names
        assert "agentic-os-gateway.exe" in names
