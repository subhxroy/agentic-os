...
from agentic_os_cli.console_engine import HermesConsoleEngine, run_console_repl
...
def test_console_parses_bare_and_hermes_prefixed_commands(_isolate_agentic_os_home):
...
def test_console_status_hides_cli_next_step_footer(
    ...
    import agentic_os_cli.status as status_mod
...
def test_config_set_requires_confirmation_then_writes(_isolate_agentic_os_home):
    engine = HermesConsoleEngine()
...
def test_sessions_list_and_stats_use_isolated_session_store(_isolate_agentic_os_home):
...
def test_cron_pause_resume_and_run_require_confirmation(_isolate_agentic_os_home):
...
def test_repl_runs_non_interactive_lines_without_prompts(_isolate_agentic_os_home):
...
def test_repl_refuses_non_interactive_confirmation(_isolate_agentic_os_home):
...
def test_main_console_subcommand_smoke(_isolate_agentic_os_home):
    import subprocess

    result = subprocess.run(
        [sys.executable, "-m", "agentic_os_cli.main", "console"],
