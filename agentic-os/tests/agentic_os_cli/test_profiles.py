...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and AGENTIC_OS_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * AGENTIC_OS_HOME  -> tmp_path/.hermes  (so get_agentic_os_home() agrees)
    * Creates the bare-minimum ~/.agentic-os directory.
    """
...
class TestGetProfileDir:
    ...
    def test_default_returns_agentic_os_home(self, profile_env):
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.agentic-os must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".hermes"
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes agentic-os, .worktrees,
...
class TestGetActiveProfileName:
    ...
    def test_default_agentic_os_home_returns_default(self, profile_env):
        # AGENTIC_OS_HOME points to tmp_path/.hermes which is the default
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom AGENTIC_OS_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("AGENTIC_OS_HOME", str(custom))
        # With Docker-aware roots, a custom AGENTIC_OS_HOME is the default —
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        excluded_prefixes = [
            "default/agentic-os", "default/.worktrees", "default/profiles",
    ...
    def test_export_default_uses_allowlist_for_unrelated_dirs(self, profile_env, tmp_path):
        """Unrelated directories under AGENTIC_OS_HOME are excluded by allow-list (#58394).

        Docker/custom deployments often set AGENTIC_OS_HOME to a working
...
class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_agentic_os_home()."""
    ...
    def test_default_agentic_os_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_agentic_os_home()
        assert home == tmp_path / ".hermes"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (AGENTIC_OS_HOME outside ~/.agentic-os), profiles go under AGENTIC_OS_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("AGENTIC_OS_HOME", str(docker_home))
        root = _get_profiles_root()
        assert root == docker_home / "profiles"

    def test_default_agentic_os_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_agentic_os_home() returns AGENTIC_OS_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("AGENTIC_OS_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (AGENTIC_OS_HOME under ~/.agentic-os), profiles root is still ~/.agentic-os/profiles."""
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under AGENTIC_OS_HOME."""
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under AGENTIC_OS_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("AGENTIC_OS_HOME", str(docker_home))
...
class TestProfilesToServe:
    ...
    def test_off_returns_only_active_default(self, profile_env):
        ...
        name, home = serve[0]
        assert name == "default"
        assert home == _get_default_agentic_os_home()

    def test_off_returns_only_active_named(self, profile_env, monkeypatch):
        # A named profile's gateway runs with AGENTIC_OS_HOME pointing at the
        # profile dir; get_active_profile_name() infers the name from there.
        create_profile("coder", no_alias=True)
        monkeypatch.setenv("AGENTIC_OS_HOME", str(get_profile_dir("coder")))
        serve = profiles_to_serve(multiplex=False)
        assert len(serve) == 1
    ...
    def test_on_returns_default_plus_all_named(self, profile_env):
        ...
        assert serve["default"] == _get_default_agentic_os_home()
