...
from agentic_os_cli.cron import (
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "systemctl restart agentic-os-gateway",
        "systemctl stop agentic-os-gateway.service",
        "systemctl start agentic-os-gateway",
    ])
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # AGENTIC_OS_HOME/scripts so the scheduler — and the guard — resolve it.
...
class TestTerminalToolGatewayLifecycleGuard:
    ...
    Issue #37453: systemctl --user restart agentic-os-gateway runs as a child of the
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart agentic-os-gateway", force=True
        ))
    ...
    def test_guard_inactive_outside_gateway(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(command="systemctl restart agentic-os-gateway"))

        # Outside the gateway the lifecycle guard doesn't block — the normal
        # approval flow handles it (here mocked as approved).
        assert result["exit_code"] == 0
        assert calls == ["systemctl restart agentic-os-gateway"]
...
class TestLifecycleGuardModule:
    ...
    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under AGENTIC_OS_HOME/scripts (the
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    agentic_os_cli.cron.cron_create. Enforcing at create_job covers both."""
