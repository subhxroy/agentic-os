...
1. ``agentic_os_cli.web_server._discover_dashboard_plugins`` opted into
...
2. ``agentic_os_cli.web_server._mount_plugin_api_routes`` then imported
...
class TestProjectPluginsEnvGate:
    ...
    def project_plugin(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("AGENTIC_OS_HOME", str(tmp_path / "home"))
        (tmp_path / "home").mkdir()
