...
Complements tests/agentic_os_cli/test_kanban_db.py (schema + CAS atomicity)
and tests/agentic_os_cli/test_kanban_cli.py (end-to-end run_slash).  The
...
def kanban_home(tmp_path, monkeypatch):
    ...
    # tests in tests/agentic_os_cli/test_kanban_db.py.
    monkeypatch.setenv("HERMES_KANBAN_CRASH_GRACE_SECONDS", "0")
    kb.init_db()
    return home
...
def test_list_profiles_on_disk(tmp_path, monkeypatch):
    ...
    named profiles under ~/.agentic-os/profiles/ that contain a config.yaml."""
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
...
def test_list_profiles_on_disk_custom_root(tmp_path, monkeypatch):
    """list_profiles_on_disk respects a custom AGENTIC_OS_HOME root."""
...
def test_parse_duration_accepts_formats():
    from agentic_os_cli.kanban import _parse_duration
...
def test_cli_bulk_complete_with_summary_rejects(kanban_home):
    ...
    # Note: agentic_os_cli.main doesn't propagate sub-command exit codes
    ...
    env = os.environ.copy()
    r = _run(
        [sys.executable, "-m", "agentic_os_cli.main", "kanban",
...
def test_cli_create_on_fresh_home_auto_inits(tmp_path, monkeypatch):
    """First CLI action on an empty AGENTIC_OS_HOME must not error with
    ...
    env = {**os.environ, "AGENTIC_OS_HOME": str(home),
           "PYTHONPATH": str(worktree_root)}
    r = _sp.run(
        [_sys.executable, "-m", "agentic_os_cli.main", "kanban",
...
def test_connect_auto_inits_fresh_db(tmp_path, monkeypatch):
    """Calling connect() on a fresh AGENTIC_OS_HOME must create the
...
def _drive_worker_exit(conn, tid, fake_pid, raw_status):
    ...
    Deliberately resolves ``agentic_os_cli.kanban_db`` fresh and uses that single
