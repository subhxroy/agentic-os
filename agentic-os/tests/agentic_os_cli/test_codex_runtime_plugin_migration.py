...
from agentic_os_cli.codex_runtime_plugin_migration import (
    MIGRATION_MARKER,
...
class TestMigrate:
    ...
    def test_plugin_discovery_writes_plugin_blocks(self, tmp_path, monkeypatch):
        ...
        from agentic_os_cli import codex_runtime_plugin_migration as crpm
    ...
    def test_plugin_discovery_skips_unavailable_plugins(self):
        ...
        from agentic_os_cli.codex_runtime_plugin_migration import _query_codex_plugins
    ...
    def test_expose_hermes_tools_writes_callback_mcp_entry(self, tmp_path):
        ...
        assert "agentic_os_tools_mcp_server" in text
    ...
    def test_expose_hermes_tools_disabled_skips_entry(self, tmp_path):
        ...
        assert "agentic_os_tools_mcp_server" not in text
...
class TestStripUnmanagedPluginTables:
    ...
    def test_migrate_dedups_codex_owned_plugin_tables(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "agentic_os_cli.codex_runtime_plugin_migration._query_codex_plugins",
            fake_query,
        )
...
# ---- Bug C: AGENTIC_OS_HOME tempdir leak into ~/.codex/config.toml ----


class TestHermesHomeLeakGuard:
    """Regression tests for issue #26250 Bug C.

    Previously ``_build_hermes_tools_mcp_entry()`` read ``AGENTIC_OS_HOME``
    ...
    def test_tempdir_detector_accepts_real_agentic_os_home(self):
    ...
    def test_pytest_tempdir_not_burned_into_mcp_env(self, monkeypatch):
        """The headline regression: even when AGENTIC_OS_HOME points at a pytest
        ...
        env = entry.get("env", {})
        assert "AGENTIC_OS_HOME" not in env, (
            f"pytest-tempdir AGENTIC_OS_HOME leaked into codex MCP entry: "
            f"{env.get('AGENTIC_OS_HOME')!r}"
        )

    def test_real_agentic_os_home_propagates(self, monkeypatch, tmp_path):
        """A legitimate AGENTIC_OS_HOME (not a tempdir path) DOES propagate so the
        ...
        monkeypatch.setenv("AGENTIC_OS_HOME", real_path)
        entry = _build_hermes_tools_mcp_entry()
        env = entry.get("env", {})
        assert env.get("AGENTIC_OS_HOME") == real_path

    def test_unset_agentic_os_home_omits_env_key(self, monkeypatch):
        """When AGENTIC_OS_HOME is unset in the environment, the MCP entry MUST
        NOT bake in a resolved-default path. The codex subprocess should
        inherit whatever AGENTIC_OS_HOME its launcher (systemd, gateway, shell)
        ...
        env = entry.get("env", {})
        assert "AGENTIC_OS_HOME" not in env, (
            f"AGENTIC_OS_HOME should not be set when env var is unset, got: "
            f"{env.get('AGENTIC_OS_HOME')!r}"
        )
