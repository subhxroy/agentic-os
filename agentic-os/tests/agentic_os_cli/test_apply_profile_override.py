"""Regression tests for _apply_profile_override AGENTIC_OS_HOME guard (issue #22502).

When AGENTIC_OS_HOME is set to the hermes root (e.g. systemd hardcodes
AGENTIC_OS_HOME=/root/.hermes), _apply_profile_override must still read
active_profile and update AGENTIC_OS_HOME to the profile directory.

When AGENTIC_OS_HOME is already a profile directory (.../profiles/<name>),
...
def _run_apply_profile_override(
    ...
    Returns the value of os.environ["AGENTIC_OS_HOME"] after the call,
    or None if unset.
    """
    ...
    from agentic_os_cli.main import _apply_profile_override
    _apply_profile_override()

    return os.environ.get("AGENTIC_OS_HOME")


class TestApplyProfileOverrideHermesHomeGuard:
    """Regression guard for issue #22502.

    Verifies that AGENTIC_OS_HOME pointing to the hermes root does NOT suppress
    the active_profile check, while AGENTIC_OS_HOME already pointing to a
    profile directory IS trusted as-is.
    """

    def test_agentic_os_home_at_root_with_active_profile_is_redirected(
        self, tmp_path, monkeypatch
    ):
        """AGENTIC_OS_HOME=/root/.hermes + active_profile=coder must redirect
        AGENTIC_OS_HOME to .../profiles/coder.

        Bug scenario from #22502: systemd sets AGENTIC_OS_HOME to the hermes root
        ...
        assert result is not None, "AGENTIC_OS_HOME must be set after profile redirect"
        assert "profiles" in result, (
            f"Expected AGENTIC_OS_HOME to point into profiles/ dir, got: {result!r}"
        )
        assert result.endswith("coder"), (
            f"Expected AGENTIC_OS_HOME to end with 'coder', got: {result!r}"
        )

    def test_agentic_os_home_already_profile_dir_is_trusted(self, tmp_path, monkeypatch):
        """AGENTIC_OS_HOME=.../profiles/coder must not be overridden even when
        active_profile says something different.

        Preserves the child-process inheritance contract: a subprocess spawned
        with AGENTIC_OS_HOME already set to a specific profile must stay in that
        profile.
        """
        hermes_root = tmp_path / ".hermes"
        ...
        assert os.environ.get("AGENTIC_OS_HOME") == str(profile_dir), (
            "AGENTIC_OS_HOME must remain unchanged when already pointing to a profile dir"
        )

    def test_agentic_os_home_unset_reads_active_profile(self, tmp_path, monkeypatch):
        """Classic case: AGENTIC_OS_HOME unset + active_profile=coder must set
        AGENTIC_OS_HOME to the profile directory (existing behaviour must not regress).
        """
        result = _run_apply_profile_override(
            tmp_path,
            monkeypatch,
    ...
    def test_agentic_os_home_unset_default_profile_no_redirect(self, tmp_path, monkeypatch):
        """active_profile=default must not redirect AGENTIC_OS_HOME."""
        hermes_root = tmp_path / ".hermes"
        hermes_root.mkdir(parents=True, exist_ok=True)
        ...
        assert os.environ.get("AGENTIC_OS_HOME") is None
...
class TestSupervisedChildIgnoresStickyProfile:
    ...
    bare ``hermes gateway run`` (no ``-p``) to mean "the root AGENTIC_OS_HOME
    ...
    def test_supervised_child_does_not_follow_active_profile(
        ...
        AGENTIC_OS_HOME=/opt/data (the container root, whose parent is NOT
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Container root AGENTIC_OS_HOME: parent dir is NOT "profiles", so the
        # #22502 guard does not short-circuit — step 2 (active_profile) runs.
        monkeypatch.setenv("AGENTIC_OS_HOME", str(hermes_root))
        ...
        assert os.environ.get("AGENTIC_OS_HOME") == str(hermes_root), (
            "Supervised default gateway must stay on the root profile, not be "
            f"hijacked by active_profile; got {os.environ.get('AGENTIC_OS_HOME')!r}"
        )
    ...
    def test_supervised_named_profile_flag_still_wins(self, tmp_path, monkeypatch):
        ...
        result = os.environ.get("AGENTIC_OS_HOME")
        assert result is not None
        assert result.endswith("coder")
