"""Tests for agentic_os_cli.web_server and related config utilities."""
...
@pytest.fixture
def _install_example_plugin(_isolate_agentic_os_home):
    """Drop the example-dashboard fixture into the per-test AGENTIC_OS_HOME
    ...
    real bundled plugins — kanban, agentic-os-achievements, model providers
    ...
    #      and start serving requests against a torn-down AGENTIC_OS_HOME.
    app = web_server.app
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_agentic_os_home):
        """Create a TestClient and isolate the state DB under the test AGENTIC_OS_HOME."""
        try:
    ...
    def test_status_active_session_count_uses_read_only_db(self, monkeypatch, tmp_path):
        ...
        fake_db_path.touch()
        monkeypatch.setattr(agentic_os_state, "DEFAULT_DB_PATH", fake_db_path)
        ...
        monkeypatch.setattr("agentic_os_state.SessionDB", _FakeDB)
    ...
    def test_status_active_session_count_fresh_install_returns_zero(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(agentic_os_state, "DEFAULT_DB_PATH", tmp_path / "absent.db")

        def _boom(*a, **k):
            raise AssertionError("SessionDB must not be constructed when db file is absent")

        monkeypatch.setattr("agentic_os_state.SessionDB", _boom)
    ...
    def _isolate_honcho_config(self):
        # Honcho tests write the suite-wide AGENTIC_OS_HOME honcho.json; snapshot and
    ...
    def test_get_media_requires_auth(self):
        from agentic_os_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_config_schema_merges_custom_command_tts_provider(self):
        ...
        from agentic_os_cli.config import load_config, save_config
        from agentic_os_cli.web_server import CONFIG_SCHEMA
    ...
    def test_get_env_vars_marks_channel_managed_keys(self):
        from agentic_os_cli.web_server import _channel_managed_env_keys
    ...
    def test_ops_backup_uses_hosted_agentic_os_home(self, tmp_path, monkeypatch):
        from pathlib import Path

        import agentic_os_cli.web_server as ws

        hosted_home = tmp_path / "opt-data"
        monkeypatch.setenv("AGENTIC_OS_HOME", str(hosted_home))
        captured = {}
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from agentic_os_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from agentic_os_cli.config import save_env_value
        from agentic_os_cli.web_server import _SESSION_TOKEN
    ...
    def test_teams_messaging_metadata_links_setup_guide(self):
        ...
        from agentic_os_cli.web_server import _build_catalog_entry
    ...
    def test_telegram_onboarding_worker_request_uses_httpx(self, monkeypatch):
        ...
        assert kwargs["json"] == {"bot_name": "Agentic OS"}
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/agentic_os_cli/web_server.py")
        assert resp.status_code in {200, 404}
        if resp.status_code == 200:
    ...
    def test_set_model_main_nous_applies_gateway_defaults(self, monkeypatch):
        ...
        import agentic_os_cli.nous_subscription as ns
    ...
    def test_apply_main_model_assignment_base_url_and_context_reconcile(self):
        ...
        from agentic_os_cli.web_server import _apply_main_model_assignment
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from agentic_os_cli.web_server import _parse_model_ids
...
class TestBuildSchemaFromConfig:
    ...
    def test_approvals_mode_options_match_config_values(self):
        ...
        are 'manual', 'smart', and 'off' (see agentic_os_cli/config.py).
    ...
    def test_empty_prefix_produces_correct_keys(self):
        from agentic_os_cli.web_server import _build_schema_from_config
...
class TestConfigRoundTrip:
    ...
    def test_round_trip_preserves_schema_invisible_nested_keys(self):
        ...
        from agentic_os_cli.config import load_config, read_raw_config, save_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_agentic_os_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import agentic_os_cli.skills_config as skills_config
        import agentic_os_cli.web_server as web_server
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from agentic_os_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from agentic_os_cli.web_server import _denormalize_config_from_web
        from agentic_os_cli.config import save_config
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_missing_name(self):
        from agentic_os_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.agentic-os/dashboard-themes/."""
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``agentic_os_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in agentic_os_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_agentic_os_home):
        ...
        # Pin the SessionDB to the isolated AGENTIC_OS_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_agentic_os_home, _install_example_plugin):
        ...
        installs it into the per-test ``AGENTIC_OS_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into AGENTIC_OS_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, agentic-os-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (agentic-os-achievements is loaded by default).
        resp = self.client.get("/api/plugins/agentic-os-achievements/overview")
        assert resp.status_code == 401
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_user_plugins_ignore_profile_home_override(self, tmp_path, monkeypatch):
        ...
        AGENTIC_OS_HOME override scopes a request to another profile."""
        from agentic_os_constants import (
            reset_AGENTIC_OS_HOME_OVERRIDE,
            set_AGENTIC_OS_HOME_OVERRIDE,
        )
        ...
        monkeypatch.setenv("AGENTIC_OS_HOME", str(launch_home))
        from agentic_os_cli import web_server
        token = set_AGENTIC_OS_HOME_OVERRIDE(str(other))
        try:
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_uses_dashboard_scroll_env(self, monkeypatch):
        ...
        assert env["AGENTIC_OS_TUI_DASHBOARD"] == "1"
        assert env["AGENTIC_OS_TUI_INLINE"] == "1"
        assert env["AGENTIC_OS_TUI_DISABLE_MOUSE"] == "1"
    ...
    def test_resolve_chat_argv_applies_terminal_backend_config(
        self, monkeypatch, _isolate_agentic_os_home
    ):
        import agentic_os_cli.main as main_mod

        config_path = Path(os.environ["AGENTIC_OS_HOME"]) / "config.yaml"
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from agentic_os_cli.pty_bridge import PtyUnavailableError
        ...
        import agentic_os_cli.web_server as ws_mod
    ...
    def test_channel_param_propagates_sidecar_url(self, monkeypatch):
        ...
        AGENTIC_OS_TUI_SIDECAR_URL env var pointing back at /api/pub on the
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from agentic_os_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    ...
    assert env is not None
    gateway_url = env.get("AGENTIC_OS_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_agentic_os_home, _install_example_plugin):
        ...
        the per-test ``AGENTIC_OS_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_agentic_os_home):
        import threading
    ...
    def test_ticker_skipped_without_desktop(self, monkeypatch, _isolate_agentic_os_home):
        import threading
        import cron.scheduler as sched
