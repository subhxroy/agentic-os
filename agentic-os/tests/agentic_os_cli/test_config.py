"""Tests for agentic_os_cli configuration management."""
...
class TestGetHermesHome:
    ...
    def test_env_override(self):
        with patch.dict(os.environ, {"AGENTIC_OS_HOME": "/custom/path"}):
            home = get_agentic_os_home()
...
class TestEnsureHermesHome:
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from agentic_os_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
    ...
    def test_preserves_legacy_template_with_user_persona(self, tmp_path):
        ...
        from agentic_os_cli.default_soul import _LEGACY_TEMPLATE_SOULS
    ...
    def test_existing_named_profile_still_bootstraps_subdirs(self, tmp_path):
        ...
        with patch.dict(os.environ, {"AGENTIC_OS_HOME": str(profile_home)}):
            ensure_agentic_os_home()
...
class TestLoadConfigParseFailure:
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        with patch.dict(os.environ, {"AGENTIC_OS_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="agentic_os_cli.config"):
                config = load_config()
...
class TestEmptyConfigSections:
    ...
    def test_null_override_of_non_dict_default_still_applies(self, tmp_path):
        ...
        from agentic_os_cli.config import _deep_merge
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"AGENTIC_OS_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"AGENTIC_OS_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"AGENTIC_OS_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"AGENTIC_OS_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"AGENTIC_OS_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"AGENTIC_OS_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from agentic_os_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
...
class TestEnvWriteDenylist:
    ...
    def _agentic_os_home(self, tmp_path, monkeypatch):
...
"""
    ...
    def test_persist_migration_writes_full_read_raw_config(self, tmp_path):
        from agentic_os_cli.config import _persist_migration, read_raw_config
