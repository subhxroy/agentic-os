...
def test_tui_finds_bundled_entry_js(tmp_path):
    ...
    tui_dist = tmp_path / "agentic_os_cli" / "tui_dist"
    tui_dist.mkdir(parents=True)
    entry = tui_dist / "entry.js"
    entry.write_text("// bundled TUI", encoding="utf-8")

    from agentic_os_cli.main import _find_bundled_tui
    result = _find_bundled_tui(agentic_os_cli_dir=tmp_path / "agentic_os_cli")
