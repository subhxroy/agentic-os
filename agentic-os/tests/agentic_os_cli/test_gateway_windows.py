...
def test_build_gateway_argv_uses_base_pythonw_for_uv_venv_launcher(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "_profile_arg", lambda agentic_os_home: "")
    monkeypatch.setattr("agentic_os_cli.config.get_agentic_os_home", lambda: str(agentic_os_home))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(base_pythonw), "-m", "agentic_os_cli.main"]
    assert cwd == str(agentic_os_home.resolve())
...
class TestStableWindowsGatewayWorkingDir:
    def test_stable_gateway_working_dir_uses_agentic_os_home(self, tmp_path, monkeypatch):
...
def test_write_task_script_anchors_cmd_cd_at_agentic_os_home(monkeypatch, tmp_path):
    project = tmp_path / "project"
    agentic_os_home = tmp_path / "hermes-home"
    agentic_os_home.mkdir()
    ...
    assert f"cd /d {gateway_windows._quote_cmd_script_arg(str(agentic_os_home.resolve()))}" in content
...
def test_gateway_cmd_script_uses_pythonw_without_replace_or_start_churn(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda exe: (exe.replace("python.exe", "pythonw.exe"), r"C:\\Hermes\\agentic-os\\venv", []),
    )

    content = gateway_windows._build_gateway_cmd_script(
        r"C:\\Hermes\\agentic-os\\venv\\Scripts\\python.exe",
        r"C:\\Hermes\\agentic-os",
...
def test_gateway_vbs_script_is_console_less(monkeypatch):
    ...
    for var in ("AGENTIC_OS_HOME", "PYTHONIOENCODING", "HERMES_GATEWAY_DETACHED", "VIRTUAL_ENV", "PYTHONPATH"):
