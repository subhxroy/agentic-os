...
``uv tool install agentic-os`` lives outside any venv, so the previous
...
that also has ``uv tool install agentic-os`` does not get misclassified.
"""
...
class TestIsUvToolInstall:
    def test_returns_true_when_sys_prefix_matches_uv_tool_layout(self):
        from agentic_os_cli import config

        with patch.object(config.sys, "prefix", "/home/user/.local/share/uv/tools/agentic-os"):
    ...
    def test_returns_true_when_sys_executable_matches_uv_tool_layout(self):
        ...
        with patch.object(config.sys, "prefix", "/some/unrelated/venv"), \
             patch.object(
                 config.sys,
                 "executable",
                 "/home/user/.local/share/uv/tools/agentic-os/bin/python",
             ):
            assert config.is_uv_tool_install() is True
    ...
    def test_does_not_consult_uv_tool_list(self):
        ...
        ``uv tool install agentic-os`` somewhere on disk. Copilot review on
    ...
    def test_case_insensitive_match(self):
        ...
        (e.g. ``...AppData\\Local\\UV\\Tools\\agentic-os``) and a case-sensitive
        ...
        with patch.object(
            config.sys, "prefix", "/HOME/USER/.local/share/UV/Tools/agentic-os"
        ):
            assert config.is_uv_tool_install() is True
...
class TestRecommendedUpdateCommandForUvTool:
    def test_uv_tool_install_recommends_uv_tool_upgrade(self):
        ...
        with patch("shutil.which", return_value="/usr/local/bin/uv"), \
            ...
            assert cmd == "uv tool upgrade agentic-os"
    ...
    def test_uv_pip_install_keeps_legacy_recommendation(self):
        ...
        with patch("shutil.which", return_value="/usr/local/bin/uv"), \
            ...
            cmd = config.recommended_update_command_for_method("pip")
            assert cmd == "uv pip install --upgrade agentic-os"
    ...
    def test_no_uv_falls_back_to_plain_pip(self):
        ...
        with patch("shutil.which", return_value=None), \
             patch.object(config, "is_uv_tool_install", return_value=False):
            cmd = config.recommended_update_command_for_method("pip")
            assert cmd == "pip install --upgrade agentic-os"
...
class TestCmdUpdatePipUsesUvTool:
    ...
    def test_runs_uv_tool_upgrade_when_uv_tool_install(self, mock_run):
        ...
        from agentic_os_cli.main import _cmd_update_pip
...
class TestCmdUpdatePipInstallLayouts:
    ...
    def test_pipx_managed_uses_pipx_upgrade(self, mock_run, monkeypatch):
        ...
        monkeypatch.setattr(hm.sys, "prefix", "/home/u/.local/pipx/venvs/agentic-os")
    ...
    def test_venv_exports_virtualenv_and_omits_system(self, mock_run, monkeypatch):
        ...
        monkeypatch.setattr(hm.sys, "prefix", "/home/u/.hermes/agentic-os/venv")
        monkeypatch.setattr(hm.sys, "base_prefix", "/usr")
        ...
        assert mock_run.call_args.kwargs["env"]["VIRTUAL_ENV"] == "/home/u/.hermes/agentic-os/venv"
