...
def isolated_kanban_home(monkeypatch):
    """Spin up a fresh AGENTIC_OS_HOME with a clean kanban DB."""
    test_home = tempfile.mkdtemp(prefix="kanban_default_assignee_test_")
    monkeypatch.setenv("AGENTIC_OS_HOME", test_home)
    # Force-reimport so the fresh AGENTIC_OS_HOME is picked up.
    ...
    # gives each test its own monkeypatched AGENTIC_OS_HOME so no cross-test
    # contamination.
...
def test_dispatch_result_has_auto_assigned_default_field():
    ...
    from agentic_os_cli.kanban_db import DispatchResult
    r = DispatchResult()
