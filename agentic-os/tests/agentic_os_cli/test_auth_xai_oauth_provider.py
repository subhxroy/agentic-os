"""Tests for xAI Grok OAuth — tokens stored in Hermes auth store (~/.agentic-os/auth.json)."""
...
def _patch_httpx_client(monkeypatch, response):
    ...
    monkeypatch.setattr("agentic_os_cli.auth.httpx.Client", _factory)
    return holder
...
def test_xai_oauth_poll_device_token_waits_until_authorized(monkeypatch):
    ...
    monkeypatch.setattr("agentic_os_cli.auth.time.sleep", lambda _: None)
...
def test_read_xai_oauth_tokens_missing_refresh_token(tmp_path, monkeypatch):
    agentic_os_home = tmp_path / "hermes"
    _setup_hermes_auth(agentic_os_home, refresh_token="")
    monkeypatch.setenv("AGENTIC_OS_HOME", str(agentic_os_home))
...
def test_resolve_xai_runtime_credentials_returns_singleton_state(tmp_path, monkeypatch):
    agentic_os_home = tmp_path / "hermes"
    fresh = _jwt_with_exp(int(time.time()) + 2 * 60 * 60)
    _setup_hermes_auth(agentic_os_home, access_token=fresh)
    monkeypatch.setenv("AGENTIC_OS_HOME", str(agentic_os_home))
...
def test_resolve_xai_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("agentic_os_cli.auth._refresh_xai_oauth_tokens", _fake_refresh)
...
def _seed_xai_oauth_state(
    agentic_os_home: Path, state: dict, *, active_provider: str = "xai-oauth"
) -> None:
    agentic_os_home.mkdir(parents=True, exist_ok=True)
    auth_store = {
        "version": 1,
    ...
    (agentic_os_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    """
    agentic_os_home = tmp_path / "hermes"
    _seed_xai_oauth_state(agentic_os_home, dict(_STALE_XAI_OAUTH_STATE), active_provider="nous")
    monkeypatch.setenv("AGENTIC_OS_HOME", str(agentic_os_home))
    ...
    monkeypatch.setattr("agentic_os_cli.auth._refresh_xai_oauth_tokens", _terminal_refresh)
    ...
    raw = json.loads((agentic_os_home / "auth.json").read_text())
...
def test_resolve_credentials_does_not_quarantine_on_transient_refresh_failure(
    ...
    """
    agentic_os_home = tmp_path / "hermes"
    _seed_xai_oauth_state(agentic_os_home, dict(_STALE_XAI_OAUTH_STATE))
    monkeypatch.setenv("AGENTIC_OS_HOME", str(agentic_os_home))
    ...
    monkeypatch.setattr("agentic_os_cli.auth._refresh_xai_oauth_tokens", _transient_refresh)
...
def test_xai_oauth_discovery_raises_typed_error_on_malformed_json(monkeypatch):
    ...
    from agentic_os_cli.auth import _xai_oauth_discovery
...
def test_xai_oauth_discovery_validates_endpoints(monkeypatch):
    ...
    monkeypatch.setattr("agentic_os_cli.auth.httpx.get", _fake_get)
    with pytest.raises(AuthError) as exc:
        _xai_oauth_discovery()
    assert exc.value.code == "xai_discovery_invalid"
...
def test_credential_pool_seeds_xai_oauth_from_singleton(tmp_path, monkeypatch):
    ...
    agentic_os_home = tmp_path / "hermes"
    fresh = _jwt_with_exp(int(time.time()) + 2 * 60 * 60)
    _setup_hermes_auth(agentic_os_home, access_token=fresh, refresh_token="rt-1")
    monkeypatch.setenv("AGENTIC_OS_HOME", str(agentic_os_home))
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    ...
    # nothing to import.
    raw_after = json.loads((agentic_os_home / "auth.json").read_text())
...
def test_login_xai_oauth_relogin_clears_suppression_and_reseeds(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agentic_os_cli.auth._update_config_for_provider",
...
def test_pool_sync_back_writes_to_singleton(tmp_path, monkeypatch):
    ...
    _setup_hermes_auth(agentic_os_home, access_token=expired, refresh_token="rt-old")
    monkeypatch.setenv("AGENTIC_OS_HOME", str(agentic_os_home))
    ...
    monkeypatch.setattr("agentic_os_cli.auth.refresh_xai_oauth_pure", _fake_refresh)
...
def test_pool_entry_needs_refresh_when_jwt_within_skew(tmp_path, monkeypatch):
    ...
    from agent.credential_pool import load_pool, AUTH_TYPE_OAUTH, PooledCredential
    from agentic_os_cli.auth import XAI_ACCESS_TOKEN_REFRESH_SKEW_SECONDS
    import uuid
...
def test_pool_refresh_marks_entry_exhausted_on_failure(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("agentic_os_cli.auth.refresh_xai_oauth_pure", _fake_refresh_fail)
...
def test_pool_seeded_entry_sync_back_after_refresh(tmp_path, monkeypatch):
    ...
    agentic_os_home = tmp_path / "hermes"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_hermes_auth(agentic_os_home, access_token=near_expiry, refresh_token="rt-singleton")
    monkeypatch.setenv("AGENTIC_OS_HOME", str(agentic_os_home))
...
def test_pool_refresh_adopts_singleton_tokens_when_consumed_elsewhere(tmp_path, monkeypatch):
    ...
    _setup_hermes_auth(agentic_os_home, access_token=in_memory_at, refresh_token="rt-stale")
    monkeypatch.setenv("AGENTIC_OS_HOME", str(agentic_os_home))
    ...
    (agentic_os_home / "auth.json").write_text(json.dumps(raw))
...
def test_pool_refresh_recovers_when_other_process_already_refreshed(tmp_path, monkeypatch):
    ...
    _setup_hermes_auth(agentic_os_home, access_token=in_memory_at, refresh_token="rt-shared")
    monkeypatch.setenv("AGENTIC_OS_HOME", str(agentic_os_home))
...
def test_pool_exhausted_xai_entry_recovers_after_singleton_refresh(tmp_path, monkeypatch):
    ...
    _setup_hermes_auth(agentic_os_home, access_token=stale_at, refresh_token="rt-stale")
    monkeypatch.setenv("AGENTIC_OS_HOME", str(agentic_os_home))
...
def test_pool_manual_xai_entry_not_synced_from_singleton(tmp_path, monkeypatch):
    ...
    _setup_hermes_auth(agentic_os_home, access_token=singleton_at, refresh_token="rt-singleton")
    monkeypatch.setenv("AGENTIC_OS_HOME", str(agentic_os_home))
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    agentic_os_home = tmp_path / "hermes"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_hermes_auth(agentic_os_home, access_token=near_expiry, refresh_token="rt-xai")
    monkeypatch.setenv("AGENTIC_OS_HOME", str(agentic_os_home))
