"""Tests for agentic_os_cli.doctor."""
...
import agentic_os_cli.doctor as doctor
import agentic_os_cli.gateway as gateway_cli
from agentic_os_cli import doctor as doctor_mod
from agentic_os_cli.doctor import _has_provider_env_config
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("AGENTIC_OS_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_agentic_os_home(self, tmp_path, provider=""):
        """Create a minimal AGENTIC_OS_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_agentic_os_home(tmp_path, provider)
        monkeypatch.setattr(doctor_mod, "AGENTIC_OS_HOME", home)
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        """Point doctor at the temp AGENTIC_OS_HOME.

        ``run_doctor`` reads the module-level ``AGENTIC_OS_HOME`` constant (cached
        at import time), NOT the env var — so ``setenv("AGENTIC_OS_HOME")`` alone
        leaves doctor probing the REAL ~/.agentic-os. On a dev machine with a
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from agentic_os_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("agentic_os_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "agentic_os_cli.doctor", raising=False)
        from agentic_os_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
        assert _has_healthy_oauth_fallback_for_apikey_provider("xai") is False
