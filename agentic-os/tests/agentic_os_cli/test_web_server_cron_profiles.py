...
def test_fire_cron_job_scopes_store_and_runtime_home_together(
    ...
    monkeypatch.setattr(scheduler, "_agentic_os_home", None)
    captured = {}
...
async def test_create_cron_job_without_profile_uses_backend_own_profile(
    ...
    ``~/.agentic-os`` when the request carries no explicit ``profile`` (the
    Desktop app's pre-profileScoped clients sent none)."""
    from agentic_os_cli import web_server

    monkeypatch.setenv(
        "AGENTIC_OS_HOME", str(isolated_profiles["worker_alpha"])
    )
...
async def test_create_cron_job_without_profile_defaults_when_unscoped(
    ...
    """AGENTIC_OS_HOME at the default home (or unrecognized) keeps the legacy
    ``default`` fallback."""
    from agentic_os_cli import web_server

    monkeypatch.setenv("AGENTIC_OS_HOME", str(isolated_profiles["default"]))
