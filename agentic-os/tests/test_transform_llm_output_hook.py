...
from agentic_os_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(agentic_os_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <agentic_os_home>/plugins/<name> and opt it in."""
    plugin_dir = agentic_os_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("AGENTIC_OS_HOME", str(tmp_path / "hermes_empty"))
