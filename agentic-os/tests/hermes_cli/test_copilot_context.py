...
def _clear_cache():
    ...
    import agentic_os_cli.models as mod
...
class TestGetCopilotModelContext:
    ...
    @patch("agentic_os_cli.models.fetch_github_model_catalog", return_value=_SAMPLE_CATALOG)
    ...
    @patch("agentic_os_cli.models.fetch_github_model_catalog", return_value=None)
    def test_returns_none_when_catalog_unavailable(self, mock_fetch):
        assert get_copilot_model_context("gpt-4.1") is None

    @patch("agentic_os_cli.models.fetch_github_model_catalog", return_value=[])
