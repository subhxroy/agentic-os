...
class TestGetDefaultHermesRoot:
    ...
    def test_no_agentic_os_home_returns_native(self, tmp_path, monkeypatch):
        """When AGENTIC_OS_HOME is not set, returns ~/.agentic-os."""
        monkeypatch.delenv("AGENTIC_OS_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_agentic_os_root() == tmp_path / ".hermes"

    def test_agentic_os_home_is_native(self, tmp_path, monkeypatch):
        """When AGENTIC_OS_HOME = ~/.agentic-os, returns ~/.agentic-os."""
        native = tmp_path / ".hermes"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("AGENTIC_OS_HOME", str(native))
        assert get_default_agentic_os_root() == native

    def test_agentic_os_home_is_profile(self, tmp_path, monkeypatch):
        """When AGENTIC_OS_HOME is a profile under ~/.agentic-os, returns ~/.agentic-os."""
        native = tmp_path / ".hermes"
    ...
    def test_agentic_os_home_is_docker(self, tmp_path, monkeypatch):
        """When AGENTIC_OS_HOME points outside ~/.agentic-os (Docker), returns AGENTIC_OS_HOME."""
    ...
    def test_agentic_os_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any AGENTIC_OS_HOME outside ~/.agentic-os is treated as the root."""
    ...
    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (AGENTIC_OS_HOME=<root>/profiles/<name>),
    ...
    def test_no_agentic_os_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\hermes, not ~/.agentic-os."""
    ...
    def test_no_agentic_os_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
...
class TestGetHermesHome:
    ...
    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When AGENTIC_OS_HOME is unset on Windows, use %LOCALAPPDATA%\\hermes."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("AGENTIC_OS_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
