...
def agentic_os_home(tmp_path, monkeypatch):
    ...
    if hasattr(agentic_os_constants, "_AGENTIC_OS_HOME_CACHE"):
        agentic_os_constants._AGENTIC_OS_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(agentic_os_home, monkeypatch):
...
def test_install_bws_happy_path(agentic_os_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(agentic_os_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(agentic_os_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from agentic_os_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under agentic_os_home/cache/."""
    home = tmp_path / ".hermes"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
