...
def isolated_home(tmp_path, monkeypatch):
    ...
    _saved = {
        ...
        if k.startswith(("agentic_os_cli", "gateway"))
    }
    ...
    def fresh_gateway():
        for mod in list(sys.modules.keys()):
            if mod.startswith(("agentic_os_cli", "gateway")):
                del sys.modules[mod]
...
def test_lift_helper_accepts_alias_and_rejects_garbage(isolated_home):
    ...
    write_cfg("model:\n  provider: openrouter\n")
    for mod in list(sys.modules.keys()):
        if mod.startswith("agentic_os_cli"):
            del sys.modules[mod]
    rp = importlib.import_module("agentic_os_cli.runtime_provider")
