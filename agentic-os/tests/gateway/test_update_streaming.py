...
def _make_runner(agentic_os_home=None):
...
class TestGatewayPrompt:
    ...
    def test_writes_prompt_file_and_reads_response(self, tmp_path):
        ...
        def write_response():
            time.sleep(0.3)
            (agentic_os_home / ".update_response").write_text("y")

        thread = threading.Thread(target=write_response)
        thread.start()

        with patch.dict(os.environ, {"AGENTIC_OS_HOME": str(agentic_os_home)}):
            from agentic_os_cli.main import _gateway_prompt
            result = _gateway_prompt("Restore? [Y/n]", "y", timeout=5.0)

        thread.join()
        assert result == "y"
        # Both files should be cleaned up
        assert not (agentic_os_home / ".update_prompt.json").exists()
        assert not (agentic_os_home / ".update_response").exists()
    ...
    def test_prompt_file_content(self, tmp_path):
        ...
        def capture_and_respond():
            ...
            prompt_path = agentic_os_home / ".update_prompt.json"
            for _ in range(20):
                if prompt_path.exists():
                    prompt_data = json.loads(prompt_path.read_text())
                    (agentic_os_home / ".update_response").write_text("n")
                    return
                time.sleep(0.1)
    ...
    def test_empty_response_returns_default(self, tmp_path):
        ...
        agentic_os_home = tmp_path / ".hermes"
        agentic_os_home.mkdir()
        (agentic_os_home / ".update_response").write_text("")
...
class TestRestoreStashWithInputFn:
    ...
    def test_uses_input_fn_when_provided(self, tmp_path):
        ...
        from agentic_os_cli.main import _restore_stashed_changes
...
class TestUpdateCommandGatewayFlag:
    ...
    async def test_spawns_with_gateway_flag(self, tmp_path):
        ...
        with patch("gateway.run._agentic_os_home", agentic_os_home), \
...
class TestWatchUpdateProgress:
    ...
    async def test_streams_output_to_adapter(self, tmp_path):
        ...
        (agentic_os_home / ".update_pending.json").write_text(json.dumps(pending))
        # Write output
        (agentic_os_home / ".update_output.txt").write_text("→ Fetching updates...\n", encoding="utf-8")
        ...
        async def write_exit_code():
            ...
            (agentic_os_home / ".update_output.txt").write_text(
                "→ Fetching updates...\n✓ Code updated!\n"
            , encoding="utf-8")
            (agentic_os_home / ".update_exit_code").write_text("0")
    ...
    async def test_detects_and_forwards_prompt(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222",
                   "session_key": "agent:main:telegram:dm:111"}
        (agentic_os_home / ".update_pending.json").write_text(json.dumps(pending))
        (agentic_os_home / ".update_output.txt").write_text("output\n")
        ...
        async def simulate_prompt_cycle():
            ...
            (agentic_os_home / ".update_prompt.json").write_text(json.dumps(prompt))
            # Simulate user responding
            await asyncio.sleep(0.5)
            (agentic_os_home / ".update_response").write_text("y")
            (agentic_os_home / ".update_prompt.json").unlink(missing_ok=True)
            await asyncio.sleep(0.3)
            (agentic_os_home / ".update_exit_code").write_text("0")
    ...
    async def test_prompt_forwarding_preserves_thread_metadata(self, tmp_path):
        ...
        (agentic_os_home / ".update_pending.json").write_text(json.dumps(pending))
        (agentic_os_home / ".update_output.txt").write_text("")
        (agentic_os_home / ".update_prompt.json").write_text(json.dumps({
    ...
    async def test_cleans_up_on_completion(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222",
                   "session_key": "agent:main:telegram:dm:111"}
        pending_path = agentic_os_home / ".update_pending.json"
        output_path = agentic_os_home / ".update_output.txt"
        exit_code_path = agentic_os_home / ".update_exit_code"
    ...
    async def test_failure_exit_code(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222",
                   "session_key": "agent:main:telegram:dm:111"}
        (agentic_os_home / ".update_pending.json").write_text(json.dumps(pending))
        (agentic_os_home / ".update_output.txt").write_text("error occurred\n")
        (agentic_os_home / ".update_exit_code").write_text("1")
    ...
    async def test_falls_back_and_delivers_after_reconnect(self, tmp_path):
        ...
        pending_path = agentic_os_home / ".update_pending.json"
        pending_path.write_text(json.dumps(pending))
        (agentic_os_home / ".update_output.txt").write_text("done\n")
        (agentic_os_home / ".update_exit_code").write_text("0")
        ...
        assert not pending_path.exists()
        assert not (agentic_os_home / ".update_exit_code").exists()
    ...
    async def test_prompt_is_recovered_after_watcher_restart(self, tmp_path):
        ...
        with patch("gateway.run._agentic_os_home", agentic_os_home):
            ...
            assert (agentic_os_home / ".update_prompt.json").exists()
...
class TestUpdatePromptInterception:
    ...
    async def test_intercepts_response_when_prompt_pending(self, tmp_path):
        ...
        (agentic_os_home / ".update_prompt.json").write_text(json.dumps({"prompt": "test"}))
        ...
        response_path = agentic_os_home / ".update_response"
