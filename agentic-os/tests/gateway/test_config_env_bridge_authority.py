...
def _run_gateway_import(agentic_os_home: Path, initial_env: dict[str, str]) -> dict[str, str]:
    ...
    env["AGENTIC_OS_HOME"] = str(agentic_os_home)
...
@pytest.fixture
def agentic_os_home(tmp_path: Path) -> Path:
    home = tmp_path / ".hermes"
    home.mkdir()
    return home


def test_config_max_turns_wins_over_stale_env(agentic_os_home: Path) -> None:
    """Regression: config.yaml:agent.max_turns=500 must beat .env=60."""
    _write_config(agentic_os_home, agent_cfg={"max_turns": 500})
    _write_env(agentic_os_home, {"HERMES_MAX_ITERATIONS": "60"})

    env = _run_gateway_import(agentic_os_home, initial_env={})
...
def test_config_gateway_timeout_wins_over_stale_env(agentic_os_home: Path) -> None:
    """Every agent.* bridge key must be config-authoritative, not .env-authoritative."""
    _write_config(agentic_os_home, agent_cfg={
        "gateway_timeout": 1800,
        "gateway_timeout_warning": 900,
    })
    _write_env(agentic_os_home, {
...
def test_config_display_busy_input_mode_wins_over_stale_env(agentic_os_home: Path) -> None:
    _write_config(agentic_os_home, display_cfg={"busy_input_mode": "interrupt"})
    _write_env(agentic_os_home, {"HERMES_GATEWAY_BUSY_INPUT_MODE": "queue"})
...
def test_config_display_busy_text_mode_wins_over_stale_env(agentic_os_home: Path) -> None:
    _write_config(agentic_os_home, display_cfg={"busy_text_mode": "queue"})
    _write_env(agentic_os_home, {"HERMES_GATEWAY_BUSY_TEXT_MODE": "interrupt"})
...
def test_config_timezone_wins_over_stale_env(agentic_os_home: Path) -> None:
    _write_config(agentic_os_home, timezone="America/Los_Angeles")
    _write_env(agentic_os_home, {"agentic_os_timeZONE": "UTC"})
...
def test_env_value_survives_when_config_omits_key(agentic_os_home: Path) -> None:
    ...
    """
    _write_config(agentic_os_home, agent_cfg={})  # no max_turns
    _write_env(agentic_os_home, {"HERMES_MAX_ITERATIONS": "123"})
...
def test_config_platform_connect_timeout_supplies_env_when_unset(agentic_os_home: Path) -> None:
    ...
    _write_config(agentic_os_home, gateway_cfg={"platform_connect_timeout": 90})
...
def test_env_platform_connect_timeout_wins_over_config(agentic_os_home: Path) -> None:
