...
class TestHandleUpdateCommand:
    ...
    async def test_managed_install_returns_package_manager_guidance(self, monkeypatch):
        ...
        assert "brew upgrade agentic-os" in result
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        runner = _make_runner()
        event = _make_event()
        # Point _agentic_os_home to tmp_path and project_root to a dir without .git
        ...
        with patch("gateway.run._agentic_os_home", tmp_path):
            ...
            # project_root will be the real agentic-os dir (which HAS .git).
    ...
    async def test_no_hermes_binary(self, tmp_path):
        """Returns error when hermes is not on PATH and agentic_os_cli is not importable."""
        runner = _make_runner()
        event = _make_event()
    ...
    async def test_fallback_to_sys_executable(self, tmp_path):
        """Falls back to sys.executable -m agentic_os_cli.main when hermes not on PATH."""
        runner = _make_runner()
        event = _make_event()
        ...
        # The update_cmd uses sys.executable -m agentic_os_cli.main
        joined = " ".join(call_args) if isinstance(call_args, list) else call_args
        assert "agentic_os_cli.main" in joined or "bash" in call_args[0]
    ...
    async def test_resolve_hermes_bin_fallback(self):
        ...
        assert result == [sys.executable, "-m", "agentic_os_cli.main"]
    ...
    async def test_writes_pending_marker_with_thread_id(self, tmp_path):
        ...
        data = json.loads((agentic_os_home / ".update_pending.json").read_text())
    ...
    async def test_popen_failure_cleans_up(self, tmp_path):
        ...
        assert not (agentic_os_home / ".update_pending.json").exists()
        assert not (agentic_os_home / ".update_exit_code").exists()
...
class TestSendUpdateNotification:
    ...
    async def test_defers_notification_while_update_still_running(self, tmp_path):
        ...
        (agentic_os_home / ".update_output.txt").write_text("still running")
    ...
    async def test_recovers_from_claimed_pending_file(self, tmp_path):
        ...
        claimed_path = agentic_os_home / ".update_pending.claimed.json"
    ...
    async def test_truncates_long_output(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (agentic_os_home / ".update_pending.json").write_text(json.dumps(pending))
        (agentic_os_home / ".update_output.txt").write_text("x" * 5000)
        (agentic_os_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_failure_message_when_update_fails(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (agentic_os_home / ".update_pending.json").write_text(json.dumps(pending))
        (agentic_os_home / ".update_output.txt").write_text("Traceback: boom")
    ...
    async def test_no_adapter_for_platform_preserves_markers(self, tmp_path):
        ...
        assert not (agentic_os_home / ".update_pending.claimed.json").exists()
