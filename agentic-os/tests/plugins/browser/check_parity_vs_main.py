...
Run from the PR worktree:
    ...
    cd ~/.agentic-os/agentic-os/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.agentic-os/agentic-os``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.agentic-os/agentic-os
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a agentic-os checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a agentic-os checkout"
)
...
# Isolated AGENTIC_OS_HOME for the config write.
