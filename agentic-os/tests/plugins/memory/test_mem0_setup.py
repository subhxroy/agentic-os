...
def _inject_fake_agentic_os_cli(monkeypatch):
    """Inject fake agentic_os_cli modules so yaml/curses aren't required."""
    fake_config_mod = types.ModuleType("agentic_os_cli.config")
    fake_config_mod.save_config = lambda c: None

    fake_setup_mod = types.ModuleType("agentic_os_cli.memory_setup")
    fake_setup_mod._curses_select = lambda *a, **kw: 0
    fake_setup_mod._prompt = lambda label, default=None, secret=False: default or ""

    fake_agentic_os_cli = types.ModuleType("agentic_os_cli")
    fake_agentic_os_cli.config = fake_config_mod
    fake_agentic_os_cli.memory_setup = fake_setup_mod

    monkeypatch.setitem(sys.modules, "agentic_os_cli", fake_agentic_os_cli)
    monkeypatch.setitem(sys.modules, "agentic_os_cli.config", fake_config_mod)
    monkeypatch.setitem(sys.modules, "agentic_os_cli.memory_setup", fake_setup_mod)
