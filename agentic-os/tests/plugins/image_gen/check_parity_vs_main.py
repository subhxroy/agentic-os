...
# worktree), ``MAIN_DIR`` falls back to a sibling ``agentic-os-main``
...
def _resolve_main_dir() -> Path:
    ...
    if (candidate / "tools" / "image_generation_tool.py").exists() and candidate != REPO_ROOT:
        return candidate
    sibling = REPO_ROOT.parent / "agentic-os-main"
...
# Isolated AGENTIC_OS_HOME so the config write is hermetic.
...
def main() -> int:
    ...
    if MAIN_DIR == PR_DIR:
        print(
            ...
            "      Set up a sibling 'agentic-os-main' checkout pinned to "
