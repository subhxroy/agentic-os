...
($INSTALL_DIR), not into the shared $AGENTIC_OS_HOME.

Background (shared-$AGENTIC_OS_HOME bug)
------------------------------------
$AGENTIC_OS_HOME is a data directory users frequently bind-mount into a Docker
gateway as well (``~/.agentic-os:/opt/data``). The published image stamps 'docker'
there on boot, so if install.sh had written its 'git' marker into the same
$AGENTIC_OS_HOME the two installs would fight over one slot — and the container,
...
``~/.agentic-os/agentic-os``), which is unique to this install and immune to the
shared data dir.
"""
...
def test_install_sh_stamps_code_tree_not_home() -> None:
    ...
    assert not re.search(r'>\s*"\$AGENTIC_OS_HOME/\.install_method"', text), (
        "install.sh must not stamp $AGENTIC_OS_HOME/.install_method — that data "
