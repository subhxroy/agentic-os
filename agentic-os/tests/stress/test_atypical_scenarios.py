...
normal tests assume away:
  ...
  - Filesystem: AGENTIC_OS_HOME with spaces / unicode / symlinks.
...
def scenario(name):
    """Decorator: run `fn` in its own AGENTIC_OS_HOME, collect failures.
    ...
    def wrap(fn):
        def run():
            ...
            for m in list(sys.modules.keys()):
                if m.startswith(("agentic_os_cli", "plugins", "gateway")):
                    del sys.modules[m]
            sys.path.insert(0, str(WT))
            from agentic_os_cli import kanban_db as kb  # noqa: F401
            print(f"\n═══ {name} ═══")
            try:
                fn(home, kb)
                print(f"  ✔ {name}")
...
def _(home, kb):
    ...
    env = {**os.environ, "PYTHONPATH": str(WT), "AGENTIC_OS_HOME": home, "HOME": home}
    bad_metas = [
        "not-json",
...
def _(home, kb):
    ...
    try:
        ...
        # allow escape.
        try:
            from agentic_os_cli.kanban_db import resolve_workspace
            ...
            home_abs = str(Path(os.environ["AGENTIC_OS_HOME"]).resolve())
...
def _(home, kb):
    ...
    os.environ["AGENTIC_OS_HOME"] = weird
    ...
    try:
        ...
        print(f"  AGENTIC_OS_HOME with spaces: OK at {weird}")
    finally:
        conn.close()
...
def _(home, kb):
    ...
    try:
        ...
        print(f"  AGENTIC_OS_HOME with unicode path: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)
...
def _(home, kb):
    ...
    try:
        os.environ["AGENTIC_OS_HOME"] = link1
        ...
        os.environ["AGENTIC_OS_HOME"] = link2
        ...
        conn2.close()
        print("  symlinks to same AGENTIC_OS_HOME share DB correctly")
    finally:
...
def _(home, kb):
    ...
    try:
        from agentic_os_cli import web_server as ws  # noqa
    except Exception:
        pass
