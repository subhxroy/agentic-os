...
def test_agentic_os_client_tag_includes_current_version():
    """The client tag must reflect agentic_os_cli.__version__ verbatim."""
    from agentic_os_cli import __version__
    from agent.portal_tags import agentic_os_client_tag

    assert agentic_os_client_tag() == f"client=hermes-client-v{__version__}"


def test_agentic_os_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import agentic_os_client_tag

    tag = agentic_os_client_tag()
