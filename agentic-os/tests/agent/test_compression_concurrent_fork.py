...
def _make_legacy_session_db_class() -> type:
    ...
    imports the same still-loaded ``agentic_os_state`` module, whose ``SessionDB``
    ...
    namespace = {"__name__": "agentic_os_state"}
    source = '''
class SessionDB:
...
_NominalSessionDBImpostor.__module__ = "agentic_os_state"
...
def test_missing_lock_subsystem_fails_open_not_infinite_loop(tmp_path: Path, monkeypatch) -> None:
    ...
    ``agentic_os_state.SessionDB`` class remains pre-lock. The compatibility path is
    ...
    real_session_db_type = agentic_os_state.SessionDB
    monkeypatch.setattr(agentic_os_state, "SessionDB", legacy_type)
    try:
        ...
        monkeypatch.setattr(agentic_os_state, "SessionDB", real_session_db_type)
