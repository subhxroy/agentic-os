...
Also verifies that ``agentic_os_cli.main._launch_tui`` correctly handles exit
...
def test_managed_install_refuses_and_does_not_set_pending_relaunch(capsys):
    ...
    with (
        patch("agentic_os_cli.config.is_managed", return_value=True),
        patch(
            "agentic_os_cli.config.format_managed_message",
            return_value="Use `brew upgrade agentic-os` to update.",
        ),
    ):
        result = _call(self_)

    out = capsys.readouterr().out
    assert "brew upgrade agentic-os" in out
...
def test_affirmative_answer_sets_pending_relaunch_and_returns_true(answer, capsys):
    ...
    with patch("agentic_os_cli.config.is_managed", return_value=False):
        result = _call(self_)
