    ...
    with patch.dict(
        "sys.modules",
        {
            "agentic_os_constants": MagicMock(
                get_agentic_os_home=MagicMock(return_value="/tmp/hermes_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'agentic-os-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'agentic-os-release' patched"
    }
