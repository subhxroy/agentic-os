...
class TestConfigureWindowsStdio:
    """``agentic_os_cli.stdio.configure_windows_stdio`` wiring.
    ...
    def _reset_configured(self, monkeypatch):
        ...
        sys.modules.pop("agentic_os_cli.stdio", None)
        # Fresh import now; tests import from agentic_os_cli.stdio themselves,
        # but this guarantees the module they get is a brand-new copy.
        import agentic_os_cli.stdio as _s
        _s._CONFIGURED = False
        yield
        sys.modules.pop("agentic_os_cli.stdio", None)

    def test_no_op_on_posix(self):
        from agentic_os_cli import stdio
...
class TestSigkillFallback:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("agentic_os_cli.kanban_db", 'getattr(signal, "SIGKILL", signal.SIGTERM)'),
        ],
    )
...
class TestWebServerPtyBridgeGuard:
    ...
    def test_import_guard_present_in_source(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "agentic_os_cli" / "web_server.py").read_text(encoding="utf-8")
...
class TestEntryPointsConfigureStdio:
    """cli.py, agentic_os_cli/main.py, gateway/run.py must call configure_windows_stdio."""

    @pytest.mark.parametrize(
        "relpath",
        ["cli.py", "agentic_os_cli/main.py", "gateway/run.py"],
    )
    def test_entry_point_calls_configure_stdio(self, relpath):
        root = Path(__file__).resolve().parents[2]
        source = (root / relpath).read_text(encoding="utf-8")
        assert "configure_windows_stdio" in source, (
            f"{relpath} must call agentic_os_cli.stdio.configure_windows_stdio() "
...
class TestSubprocessCompatHelpers:
    """agentic_os_cli/_subprocess_compat.py POSIX + Windows behaviour."""

    def test_is_windows_matches_sys_platform(self):
        from agentic_os_cli import _subprocess_compat as sc
    ...
    def test_resolve_node_command_returns_absolute_on_posix(self):
        ...
        from agentic_os_cli._subprocess_compat import resolve_node_command
...
# ---------------------------------------------------------------------------
# agentic_os_cli/kanban_db.py waitpid guard
# ---------------------------------------------------------------------------
...
class TestKanbanWaitpidWindowsGuard:
    ...
    def test_source_gates_waitpid_loop(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "agentic_os_cli" / "kanban_db.py").read_text(encoding="utf-8")
...
class TestNpmBareSpawnsResolved:
    ...
    shutil.which / agentic_os_cli._subprocess_compat.resolve_node_command
    so Windows can execute the .cmd batch shims."""

    @pytest.mark.parametrize(
        "relpath",
        [
            "agentic_os_cli/tools_config.py",
            "agentic_os_cli/doctor.py",
...
class TestGatewayDetachedWatcherWindowsFlags:
    ...
    def test_agentic_os_cli_gateway_uses_compat_kwargs(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "agentic_os_cli" / "gateway.py").read_text(encoding="utf-8")
        assert "windows_detach_popen_kwargs" in source, (
            "agentic_os_cli/gateway.py must use the platform-aware detach helper"
        )
    ...
    def test_launch_detached_profile_gateway_restart_inlined_watcher_uses_breakaway(self):
        ...
        agentic_os_cli._subprocess_compat; the watcher must call that helper from
        ...
        """
        root = Path(__file__).resolve().parents[2]
        text = (root / "agentic_os_cli" / "gateway.py").read_text(encoding="utf-8")
        ...
        block = text[idx:end]
        assert "from agentic_os_cli._subprocess_compat import" in block
...
class TestWindowlessGatewayRestartSpec:
    ...
    def test_noop_on_non_windows(self):
        import agentic_os_cli.gateway_windows as gw

        argv = ["/path/venv/bin/python", "-m", "agentic_os_cli.main", "gateway", "run"]
    ...
    def test_windows_rewrites_to_pythonw_and_preserves_tail(self):
        ...
        # ``from agentic_os_cli.gateway import PROJECT_ROOT`` resolves from
        ...
        import agentic_os_cli.config  # noqa: F401
