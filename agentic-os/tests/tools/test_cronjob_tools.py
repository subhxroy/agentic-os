...
class TestScanCronSkillAssembled:
    ...
    def test_descriptive_attack_command_prose_allowed(self):
        ...
        Real example: the `agentic-os-dev` skill contains a postmortem
        section saying 'the attacker could just cat ~/.agentic-os/.env'.
        """
        assert _scan_cron_skill_assembled(
            "the attacker could just cat ~/.agentic-os/.env to steal credentials"
        )[1] == ""
        assert _scan_cron_skill_assembled(
...
class TestCronjobRequirements:
    ...
    def test_accepts_gateway_session(self, monkeypatch):
        ...
        monkeypatch.setenv("AGENTIC_OS_GATEWAY_SESSION", "1")
        monkeypatch.delenv("AGENTIC_OS_EXEC_ASK", raising=False)
    ...
    def test_rejects_false_like_interactive_env(self, monkeypatch, false_like_value):
        monkeypatch.setenv("AGENTIC_OS_INTERACTIVE", false_like_value)
        monkeypatch.delenv("AGENTIC_OS_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("AGENTIC_OS_EXEC_ASK", raising=False)
        assert check_cronjob_requirements() is False
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["AGENTIC_OS_INTERACTIVE", "AGENTIC_OS_GATEWAY_SESSION", "AGENTIC_OS_EXEC_ASK"],
    )
    @pytest.mark.parametrize("false_like_value", ["0", "false", "no", "off"])
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("AGENTIC_OS_INTERACTIVE", "AGENTIC_OS_GATEWAY_SESSION", "AGENTIC_OS_EXEC_ASK"):
