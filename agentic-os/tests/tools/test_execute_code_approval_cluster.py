...
def gw_session(monkeypatch):
    """A clean gateway session: AGENTIC_OS_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_logs_dropped_hermes_vars(caplog):
    ...
    env = {
        "AGENTIC_OS_HOME": "/h",          # allowlisted → kept, not logged
    ...
    assert "AGENTIC_OS_HOME" in out and "PATH" in out
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    ...
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        _scrub_child_env(
            {"AGENTIC_OS_HOME": "/h", "PATH": "/usr/bin"},
