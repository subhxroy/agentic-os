...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_hermes_bin(self):
        """After _INSTALL_FAILED, manual install in AGENTIC_OS_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
...
# ---------------------------------------------------------------------------
# AGENTIC_OS_HOME isolation
# ---------------------------------------------------------------------------

class TestHermesHomeIsolation:
    def test_hermes_bin_dir_respects_agentic_os_home(self):
        """_hermes_bin_dir must use AGENTIC_OS_HOME, not hardcoded ~/.agentic-os."""
        from tools.tirith_security import _hermes_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"AGENTIC_OS_HOME": tmpdir}):
            result = _hermes_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_agentic_os_home(self):
        """_failure_marker_path must use AGENTIC_OS_HOME, not hardcoded ~/.agentic-os."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"AGENTIC_OS_HOME": "/custom/hermes"}):
    ...
    def test_get_agentic_os_home_fallback(self):
        ...
        from tools.tirith_security import _get_agentic_os_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove AGENTIC_OS_HOME entirely. With HOME also absent, expanduser
