...
class GetSpillConfigTests(unittest.TestCase):
    ...
    def test_user_overrides_are_respected(self):
        ...
        with patch("agentic_os_cli.config.load_config", return_value=user_cfg):
            cfg = hos.get_spill_config()
    ...
    def test_load_config_exception_is_swallowed(self):
        with patch("agentic_os_cli.config.load_config", side_effect=RuntimeError("bad")):
            cfg = hos.get_spill_config()
        self.assertEqual(cfg["max_chars"], hos.DEFAULT_MAX_CHARS)
        self.assertTrue(cfg["enabled"])
...
class SpillIfOversizedTests(unittest.TestCase):
    ...
    def test_default_directory_uses_agentic_os_home(self):
        """When no directory override, spill under AGENTIC_OS_HOME/hook_outputs."""
        test_home = tempfile.mkdtemp(prefix="hermes-home-")
        try:
            with patch.dict(os.environ, {"AGENTIC_OS_HOME": test_home}):
            ...
            # ~/.agentic-os/hook_outputs depending on get_agentic_os_home behaviour.
            candidates = [
                Path(test_home) / "hook_outputs" / "sess",
                Path(os.path.expanduser("~/.agentic-os/hook_outputs/sess")),
            ]
