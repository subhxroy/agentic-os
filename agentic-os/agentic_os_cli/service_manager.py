...
the existing module-level functions in agentic_os_cli.gateway and
agentic_os_cli.gateway_windows directly. This protocol is a thin facade
...
# in ``agentic_os_cli.gateway`` (systemd/launchd) and ``agentic_os_cli.gateway_windows``
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in agentic_os_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from agentic_os_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from agentic_os_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from agentic_os_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from agentic_os_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in agentic_os_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from agentic_os_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from agentic_os_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from agentic_os_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from agentic_os_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``agentic_os_cli.gateway_windows`` (Scheduled Task /
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's AGENTIC_OS_HOME blindly;
    derive the shared profile root from the current AGENTIC_OS_HOME and map the
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources AGENTIC_OS_HOME (and any extra env) via with-contenv —
             so e.g. ``-e AGENTIC_OS_HOME=/data/hermes`` is honored at run
        ...
        root AGENTIC_OS_HOME profile" (the implicit profile that exists at
        the top of $AGENTIC_OS_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``agentic_os_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$AGENTIC_OS_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``AGENTIC_OS_HOME``,
        ...
        allocator in ``agentic_os_cli.profiles._allocate_gateway_port``).
        ...
        # profile's AGENTIC_OS_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``hermes gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-AGENTIC_OS_HOME PID lock first;
        ...
        # Each profile is scoped to its own AGENTIC_OS_HOME and s6 guarantees a
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${AGENTIC_OS_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the AGENTIC_OS_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e AGENTIC_OS_HOME=/data/hermes``
        ...
        return (
            ...
            f': "${{AGENTIC_OS_HOME:=/opt/data}}"\n'
            f'log_dir="$AGENTIC_OS_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            ...
            f'chown hermes:hermes "$AGENTIC_OS_HOME/logs/gateways" 2>/dev/null || true\n'
