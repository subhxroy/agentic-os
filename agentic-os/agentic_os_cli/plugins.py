...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with agentic-os;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.agentic-os/plugins/<name>/``
...
# stderr in addition to ~/.agentic-os/logs/agent.log. Aimed at plugin authors
...
VALID_HOOKS: Set[str] = {
    ...
    # Kanban task lifecycle hooks. Fired by agentic_os_cli.kanban_db when a task
...
class PluginManifest:
    ...
    #              in ~/.agentic-os/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        ...
        Derived from ``AGENTIC_OS_HOME`` via
        :func:`agentic_os_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.agentic-os/profiles/<name>``, or ``"custom"`` when
        ``AGENTIC_OS_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`agentic_os_cli.dashboard_auth.DashboardAuthProvider`. Used by
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        # Lazy import to avoid circular: agentic_os_cli.main imports plugins indirectly
        from agentic_os_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.agentic-os/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to agentic_os_cli/. Two layouts are
        ...
        # 2. User plugins (~/.agentic-os/plugins/)
