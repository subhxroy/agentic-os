...
from agentic_os_cli.urllib_security import open_credentialed_url
...
def _codex_curated_models() -> list[str]:
    ...
    """
    from agentic_os_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $AGENTIC_OS_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $AGENTIC_OS_HOME/models_dev_cache.json directly (no network) so this
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_agentic_os_home() / "cache" / "nous_recommended_cache.json"
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$AGENTIC_OS_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from agentic_os_cli.model_catalog import get_default_model_from_cache
...
# ``get_recommended_default_model`` in agentic_os_cli/web_server.py and
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    agentic-os is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from agentic_os_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — agentic-os
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from agentic_os_cli.model_catalog import get_curated_nous_models
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from agentic_os_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from agentic_os_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``agentic_os_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         when the env var is set in ``~/.agentic-os/.env``.
    ...
    """
    try:
        from agentic_os_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        ...
        from agentic_os_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from agentic_os_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
#
# Cache strategy:
#   - One JSON file at $AGENTIC_OS_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from agentic_os_constants import get_agentic_os_home
    return get_agentic_os_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$AGENTIC_OS_HOME/auth.json`` and external
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from agentic_os_constants import get_agentic_os_home
    return get_agentic_os_home() / "ollama_cloud_models_cache.json"
