...
def apply_managed_overlay(config: dict) -> dict:
    ...
    (rather than going through agentic_os_cli.config.load_config) to honor managed
    scope. Mirrors agentic_os_cli.config._load_config_impl's managed merge exactly:
    ...
    try:
        ...
        from agentic_os_cli.config import _deep_merge, _expand_env_vars, _normalize_root_model_keys
