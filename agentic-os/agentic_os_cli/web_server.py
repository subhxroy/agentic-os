"""
Agentic OS — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m agentic_os_cli.main web          # Start on http://127.0.0.1:9119
    python -m agentic_os_cli.main web --port 8080
"""
...
from agentic_os_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    real gateway on the same AGENTIC_OS_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
def _warm_gateway_module() -> None:
    try:
        import agentic_os_cli.gateway  # noqa: F401
    except Exception:
        pass
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from agentic_os_cli.gateway import _get_restart_drain_timeout
...
async def _lifespan(app: "FastAPI"):
    ...
    # Fire agentic_os_cli.gateway import into a background thread so the event
...
app = FastAPI(title="Agentic OS", version=__version__, lifespan=_lifespan)

# Memory-provider OAuth connect routes live in the memory layer, not here.
from agentic_os_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
#
# This list is defined in ``agentic_os_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from agentic_os_cli.dashboard_auth.public_paths import (
...
async def _dashboard_auth_gate(request: Request, call_next):
    from agentic_os_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from agentic_os_cli.dashboard_auth.token_auth import token_auth_middleware
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    from agentic_os_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from agentic_os_cli.model_normalize import normalize_model_for_provider
    from agentic_os_cli.providers import resolve_custom_provider, resolve_user_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from agentic_os_cli.models import _AGGREGATOR_PROVIDERS
...
def _count_status_active_sessions() -> int:
    ...
    """
    from agentic_os_state import DEFAULT_DB_PATH, SessionDB
...
# AGENTIC_OS_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to AGENTIC_OS_HOME.
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no AGENTIC_OS_HOME resolution, so it blocks these trees wherever
...
def _default_hermes_root_is_opt_data() -> bool:
    raw = os.environ.get("AGENTIC_OS_HOME", "").strip()
    if not raw:
        return False
    try:
...
def _dashboard_local_update_managed_externally() -> bool:
    ...
    if _default_hermes_root_is_opt_data():
        return True
    try:
        from agentic_os_constants import is_container
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    ``AGENTIC_OS_HOME/images/`` — the same directory ``clipboard.paste`` /
...
# gate + path hardening as /api/fs). Logic lives in ``agentic_os_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from agentic_os_cli import web_git as _web_git  # noqa: E402
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``gateway_mode`` — ``"multiplex"`` when the default gateway serves
    ...
    """
    try:
        from agentic_os_cli.profiles import _check_gateway_running, profiles_to_serve
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        # Windows install the first import of agentic_os_cli.gateway blocks the
        ...
        try:
            from agentic_os_cli.dashboard_auth import list_providers as _list_providers
        ...
        try:
            from agentic_os_cli.auth import get_nous_session_validity
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from agentic_os_cli.auth import get_nous_auth_status
    ...
    features = []
    try:
        from agentic_os_cli.nous_subscription import get_nous_subscription_features
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from agentic_os_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.agentic-os/logs/<action>.log`` so
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    Uses the running interpreter's ``agentic_os_cli.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "agentic_os_cli.main", *subcommand]
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.agentic-os/config.yaml`` under ``tts.``.
    """
...
# in agentic_os_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_recommended_default_model(provider: str = ""):
    ...
    try:
        from agentic_os_cli.inventory import build_models_payload, load_picker_context
        from agentic_os_cli.models import pick_silent_default_model
...
def get_moa_models(profile: Optional[str] = None):
    ...
    try:
        from agentic_os_cli.moa_config import normalize_moa_config
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    try:
        from agentic_os_cli.moa_config import normalize_moa_config, validate_moa_payload
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.agentic-os/config.yaml`` — applies to **new** sessions only.
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (agentic_os_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from agentic_os_cli.nous_subscription import apply_nous_managed_defaults
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from agentic_os_cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    try:
        from agentic_os_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Agentic OS").strip() or "Agentic OS"
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # all resolve against the requested profile's AGENTIC_OS_HOME.
...
def _multiplex_port_binding_conflict(
    ...
    *enabling* is blocked; disabling/clearing stays allowed so users can
    ...
    if not requested or requested.lower() == "current":
        ...
        # The dashboard's own profile. "custom" (an unrecognized AGENTIC_OS_HOME)
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.agentic-os/.anthropic_oauth.json`` — Hermes-managed PKCE flow (what
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from agentic_os_cli import auth as hauth
        if provider_id == "nous":
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        try:
            from agentic_os_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
#          → persists to ~/.agentic-os/.anthropic_oauth.json AND credential pool
...
async def _start_device_code_flow(
    ...
    if provider_id == "xai-oauth":
        from agentic_os_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from agentic_os_cli.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from agentic_os_cli.auth import _save_codex_tokens
...
async def get_logs(
    ...
    from agentic_os_cli.logs import _read_tail, LOG_FILES
...
def _cron_default_profile() -> str:
    ...
    A desktop pool backend runs one process per profile (AGENTIC_OS_HOME already
    ...
    fallback would write a non-default profile's job into ``~/.agentic-os``.
    Resolve the process's own profile instead. ``custom`` (an unrecognized
    AGENTIC_OS_HOME outside the profiles tree) has no profile-dir equivalent, so
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, AGENTIC_OS_HOME)."""
    from agentic_os_cli import profiles as profiles_mod
...
#
# Wraps the same config data layer the CLI uses (agentic_os_cli.mcp_config), so
...
def _normalize_mcp_server_create(
    ...
    """
    from agentic_os_cli.mcp_config import (
...
async def replace_mcp_servers(body: MCPServersReplace, profile: Optional[str] = None):
    ...
    """
    from agentic_os_cli.mcp_config import _replace_mcp_servers
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from agentic_os_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    def _probe_scoped():
        ...
        # The probe touches no skills globals; it only needs the AGENTIC_OS_HOME
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from agentic_os_cli.dashboard_auth.prefix import prefix_from_request, resolve_public_url
...
async def list_mcp_catalog(profile: Optional[str] = None):
    ...
    """
    try:
        from agentic_os_cli import mcp_catalog
    except Exception as exc:
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds AGENTIC_OS_HOME-derived paths in the child.
...
#
# Wraps the same JSON store the CLI uses (agentic_os_cli.webhook); the webhook
...
async def list_webhooks():
    import agentic_os_cli.webhook as wh
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    from agentic_os_cli.auth import suppress_credential_source
...
async def list_hooks():
    ...
    try:
        from agentic_os_cli.plugins import VALID_HOOKS
...
async def preview_skill_hub(identifier: str = "", profile: Optional[str] = None):
    ...
    def _run():
        from agentic_os_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
class ProfileCreate(BaseModel):
    ...
    # because skills_hub.SKILLS_DIR is import-time-bound and the AGENTIC_OS_HOME
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_agentic_os_home()
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local AGENTIC_OS_HOME override so the write lands in the target
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local AGENTIC_OS_HOME override (same mechanism as
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the AGENTIC_OS_HOME override. Returns the
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's AGENTIC_OS_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    the running dashboard/gateway is scoped to (derived from AGENTIC_OS_HOME).
    """
    from agentic_os_cli import profiles as profiles_mod
    try:
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the AGENTIC_OS_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    _resolve_profile_dir(name)
    try:
        from agentic_os_cli import profile_describer
...
def _profile_scope(profile: Optional[str]):
    ...
    imported the modules before a AGENTIC_OS_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
async def get_toolsets(profile: Optional[str] = None):
    ...
    from agentic_os_cli.platforms import platform_label
...
async def select_toolset_provider(
    ...
    from agentic_os_cli.nous_subscription import (
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.agentic-os/.env`` via ``save_env_value`` —
...
async def run_toolset_post_setup(
    ...
    write per-profile state must see the same AGENTIC_OS_HOME the rest of the
...
if sys.platform.startswith("win"):
    try:
        from agentic_os_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
...
from agentic_os_cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from agentic_os_cli.dashboard_auth.audit import AuditEvent, audit_log
...
def _resolve_chat_argv(
    ...
    Session resume is propagated via the ``AGENTIC_OS_TUI_RESUME`` env var —
    matching what ``agentic_os_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``AGENTIC_OS_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``AGENTIC_OS_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `active_session_file` (when set) is forwarded as
    ``AGENTIC_OS_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``AGENTIC_OS_HOME`` at the profile dir in the child env. Every spawned
    ...
    ``AGENTIC_OS_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from agentic_os_cli.main import PROJECT_ROOT, _apply_tui_python_env, _make_tui_argv
    ...
    env.setdefault("AGENTIC_OS_TUI_DISABLE_MOUSE", "1")
    env.setdefault("AGENTIC_OS_TUI_INLINE", "1")
    ...
    env["AGENTIC_OS_TUI_DASHBOARD"] = "1"

    if profile_dir is not None:
        env["AGENTIC_OS_HOME"] = str(profile_dir)
    ...
    if resume:
        ...
        env["AGENTIC_OS_TUI_RESUME"] = resume

    if sidecar_url:
        env["AGENTIC_OS_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile AGENTIC_OS_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["AGENTIC_OS_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from agentic_os_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from agentic_os_cli.console_engine import HermesConsoleEngine
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if raw_resume and env:
        registry_resume = env.get("AGENTIC_OS_TUI_RESUME") or raw_resume
...
# AGENTIC_OS_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`agentic_os_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from agentic_os_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    def _serve_index(prefix: str = ""):
        ...
        # (``AGENTIC_OS_HOME/dashboard-themes/<name>.yaml``), inject a minimal
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Agentic OS",         "description": "Monochrome black — the canonical Agentic OS look"},
    {"name": "default-large", "label": "Agentic OS (Large)", "description": "Agentic OS with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.agentic-os/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.agentic-os/dashboard-themes/*.yaml for user-created themes.
    ...
    live under the server's own ``AGENTIC_OS_HOME``.
    """
...
async def get_dashboard_themes():
    ...
    from `~/.agentic-os/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as agentic_os_cli.plugins):
    1. User plugins:    ~/.agentic-os/plugins/<name>/dashboard/manifest.json
    ...
    from agentic_os_cli.plugins import get_bundled_plugins_dir
    ...
    # AGENTIC_OS_HOME override (e.g. embedded /chat under --open-profile).
    search_dirs = [
    ...
    # ``agentic_os_cli/plugins.py`` and the documented user contract.
...
async def get_dashboard_plugins():
    ...
    try:
        from agentic_os_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
        enabled_set = _get_enabled_set()
        disabled_set = _get_disabled_set()
    except Exception:
        enabled_set = set()
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from agentic_os_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from agentic_os_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from agentic_os_cli.plugins_cmd import dashboard_remove_user_plugin
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.agentic-os/plugins/ if you trust it)",
...
from agentic_os_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    try:
        from agentic_os_cli.nous_auth_keepalive import start_nous_auth_keepalive
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # Each provider plugin that ships with Agentic OS exposes a
            ...
            try:
                ...
                from agentic_os_cli.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
