"""``hermes debug`` debug tools for Agentic OS.
...
Currently supports:
    hermes debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.agentic-os/logs/*.log`` are not leaked into
...
def _pending_file() -> Path:
    """Path to ``~/.agentic-os/pastes/pending.json``.
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "agentic-os/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    The replacement is stateless: we append to ``~/.agentic-os/pastes/pending.json``
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "agentic-os/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    ...
    suggests ``--local``.
    """
    from agentic_os_cli.diagnostics_upload import share_to_nous
