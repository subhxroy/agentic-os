...
from agentic_os_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE

HERMES_AGENT_LOGO = """[bold #FFFFFF]██████╗ ██████╗ ███████╗██████╗  █████╗  ██████╗ ███████╗████████╗[/]
[bold #FFFFFF]██╔══██╗██╔══██╗██╔════╝██╔══██╗██╔══██╗██╔════╝ ██╔════╝╚══██╔══╝[/]
[#BBBBBB]███████║██████╔╝█████╗  ██████╔╝███████║██║  ███╗█████╗     ██║[/]
[#BBBBBB]██╔══██║██╔══██╗██╔══╝  ██╔══██╗██╔══██║██║   ██║██╔══╝     ██║[/]
[#888888]██║  ██║██║  ██║███████╗██║  ██║██║  ██║╚██████╔╝███████╗   ██║[/]
...
def check_for_updates() -> Optional[int]:
    ...
    try:
        from agentic_os_cli.config import detect_install_method, get_project_root
    ...
    if embedded_rev:
        ...
        # $AGENTIC_OS_HOME/agentic-os/ may be a stale copy from --clone-all;
...
def _resolve_repo_dir() -> Optional[Path]:
    ...
    because ``$AGENTIC_OS_HOME/agentic-os/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``agentic_os_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from agentic_os_cli.build_info import get_build_sha
...
def format_banner_version_label() -> str:
    ...
    base = f"Agentic OS v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        from agentic_os_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from agentic_os_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
