...
Diagnoses issues with Agentic OS setup.
"""
...
_DHH = display_agentic_os_home()  # user-facing display path (e.g. ~/.agentic-os or ~/.agentic-os/profiles/coder)
...
from agentic_os_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.agentic-os/.env contains provider auth/base URL settings."""
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from agentic_os_cli.auth import get_minimax_oauth_auth_status
            return bool((get_minimax_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "xai":
        try:
            from agentic_os_cli.auth import get_xai_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches agentic_os_cli.__version__.
    ...
    """
    try:
        from agentic_os_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != agentic_os_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'hermes update', or set "
            "agentic_os_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from agentic_os_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    os.environ.setdefault("AGENTIC_OS_INTERACTIVE", "1")
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.agentic-os/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    # Detect drift between pyproject.toml and agentic_os_cli/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.agentic-os/.env (primary location for user config)
    ...
    # Check ~/.agentic-os/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from agentic_os_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from agentic_os_cli.providers import (
            ...
            if isinstance(user_providers, dict):
                from agentic_os_cli.config import is_provider_enabled
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from agentic_os_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.agentic-os/.env or run 'hermes setup')",
                            (
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from agentic_os_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        ...
        try:
            import yaml
            from agentic_os_cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
        ...
        try:
            ...
            from agentic_os_cli.config import load_env as _load_env_depr
        ...
        try:
            from agentic_os_cli.config import validate_config_structure
    ...
    if soul_path.exists():
        ...
        if should_fix:
            ...
            soul_path.write_text(
                "# Agentic OS Persona\n\n"
                "<!-- Edit this file to customize how the agent communicates. -->\n\n"
                "You are Agentic OS, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    if state_db_path.exists():
        try:
            ...
            from agentic_os_state import _db_opens_cleanly, repair_state_db_schema
            ...
            from agentic_os_state import is_malformed_db_error, repair_state_db_schema
    ...
    try:
        from agentic_os_constants import is_container as _is_container
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable AGENTIC_OS_HOME mirror
    ...
    _section("Skills Hub")
    hub_dir = AGENTIC_OS_HOME / "skills" / ".hub"
    if hub_dir.exists():
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = AGENTIC_OS_HOME / "config.yaml"
    ...
    try:
        from agentic_os_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
