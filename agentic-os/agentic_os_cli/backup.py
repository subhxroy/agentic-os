...
`hermes backup` creates a zip archive of the entire ~/.agentic-os/ directory
(excluding the agentic-os repo and transient files).

`hermes import` restores from a backup zip, overlaying onto the current
AGENTIC_OS_HOME root.
"""
...
# ``agentic-os`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/agentic-os/``
...
# AGENTIC_OS_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    ...
    # Python dependency trees (plugin / MCP-server venvs under AGENTIC_OS_HOME) —
...
# Reserved archive subtree for provider state that lives OUTSIDE AGENTIC_OS_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside AGENTIC_OS_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    ...
    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``agentic-os`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/agentic-os/`` — must be preserved.
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside AGENTIC_OS_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Agentic OS home directory."""
    hermes_root = get_default_agentic_os_root()

    if not hermes_root.is_dir():
        print(f"Error: Agentic OS home directory not found at {hermes_root}")
        sys.exit(1)
    ...
    for dirpath, dirnames, filenames in os.walk(hermes_root, followlinks=False):
        ...
        # ``agentic-os`` is only pruned at the root level; nested dirs
    ...
    # outside AGENTIC_OS_HOME, so the walk above never sees it. Ask the active
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like an Agentic OS backup.
    ...
    if not found:
        return False, (
            "zip does not appear to be an Agentic OS backup "
...
def run_import(args) -> None:
    """Restore an Agentic OS backup from a zip file."""
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Agentic OS configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under AGENTIC_OS_HOME.
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # agentic_os_cli.profiles might not be available (fresh install)
        ...
        if not (hermes_root / "agentic-os").is_dir():
            print("Note: The agentic-os codebase was not included in the backup.")
        ...
        print("Done. Your Agentic OS configuration has been restored.")
...
# Critical state files to include in quick snapshots (relative to AGENTIC_OS_HOME).
...
_QUICK_STATE_FILES = (
    ...
    # are at $AGENTIC_OS_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside AGENTIC_OS_HOME and the entry is silently
...
# Relative path of the cron job database inside AGENTIC_OS_HOME. Kept in sync with
...
def create_pre_update_backup(
    ...
    but writes to ``<AGENTIC_OS_HOME>/backups/pre-update-<timestamp>.zip`` and
...
def create_pre_migration_backup(
    ...
    ``<AGENTIC_OS_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
