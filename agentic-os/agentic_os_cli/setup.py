"""
Interactive setup wizard for Agentic OS.
...
from agentic_os_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from agentic_os_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("Agentic OS Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
...
def is_noninteractive() -> bool:
    ...
    ``HERMES_NONINTERACTIVE=1`` (see ``agentic_os_cli/web_server.py``). In that
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from agentic_os_cli.curses_ui import curses_checklist
...
def _print_setup_summary(config: dict, agentic_os_home):
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from agentic_os_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    from agentic_os_cli.main import select_provider_and_model
    try:
...
def _setup_telegram_auto_result():
    ...
    try:
        from agentic_os_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
...
def _setup_qqbot():
    ...
    from agentic_os_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def setup_gateway(config: dict):
    ...
    from agentic_os_cli.gateway import _all_platforms, _platform_status, _configure_platform
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``agentic_os_cli.auth`` — lists every supported
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from agentic_os_cli.config
    ...
    if section_key == "model":
        ...
        from agentic_os_cli.gateway import _all_platforms, _platform_status
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    print(color("│     Agentic OS Setup — Nous Portal (one-shot)            │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    try:
        from agentic_os_cli.main import _model_flow_nous
...
def run_setup_wizard(args):
    ...
    """
    from agentic_os_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     Agentic OS Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             Agentic OS Setup Wizard                     │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Agentic OS installation.      │", Colors.MAGENTA
        )
    )
    print(
        color(
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    try:
        ...
        from agentic_os_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
