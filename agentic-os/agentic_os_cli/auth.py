"""
Multi-provider authentication system for Agentic OS.
...
from agentic_os_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from agentic_os_cli import __version__ as _agentic_os_cli_VERSION
except Exception:  # pragma: no cover - version import should always succeed
    _agentic_os_cli_VERSION = "unknown"
CODEX_OAUTH_USER_AGENT = f"hermes-cli/{_agentic_os_cli_VERSION}"
...
def login_spotify_command(args) -> None:
    ...
    print()
    print("Open this URL to authorize Agentic OS:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"Agentic OS is waiting for the OAuth callback on {redirect_uri}")
...
def resolve_nous_access_token(
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        if not state:
            raise AuthError(
                "Agentic OS is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        if not state:
            raise AuthError("Agentic OS is not logged into Nous Portal.",
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    try:
        from agentic_os_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on Agentic OS' "
...
def _prompt_model_selection(
    ...
    """
    from agentic_os_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from agentic_os_cli.config import save_config, load_config
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in Agentic OS auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Agentic OS will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                print("Agentic OS will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — Agentic OS gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Agentic OS creates its own session — won't affect Codex CLI or VS Code)")
    print()
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in Agentic OS auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(Agentic OS creates its own local OAuth session)")
    print()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Agentic OS login via MiniMax ({region}) OAuth...")
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting Agentic OS login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("Agentic OS will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `agentic-os model` or configure an API key to use Agentic OS.")
        else:
