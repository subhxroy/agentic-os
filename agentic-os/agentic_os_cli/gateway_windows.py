...
_TASK_DESCRIPTION = "Agentic OS Gateway - Messaging Platform Integration"
...
def _current_profile_cli_args() -> list[str]:
    ...
    from agentic_os_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "agentic_os_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def get_task_name() -> str:
    ...
    """
    _assert_windows()
    # Local import to avoid circular module initialization during agentic_os_cli boot.
    from agentic_os_cli.gateway import _profile_suffix
...
def _build_gateway_cmd_script(
    ...
    prog_args = [pythonw_path, "-m", "agentic_os_cli.main"]
    if profile_arg:
...
def windowless_gateway_restart_spec(
    ...
    ``agentic_os_cli`` package without the venv launcher's site config.

    Returns ``(new_argv, working_dir, env_overlay)``.  ``new_argv``
    preserves every argument after the interpreter (``-m agentic_os_cli.main
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m agentic_os_cli.main gateway run``
...
def _prompt_install_choices(
    ...
    from agentic_os_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    from agentic_os_cli.gateway import find_gateway_pids, _profile_arg
