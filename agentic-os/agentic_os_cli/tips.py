...
TIPS = [
    ...
    "agentic_os_timeZONE overrides the server timezone with any IANA timezone string.",
    ...
    "Checkpoints use shadow repos in ~/.agentic-os/checkpoints/ — your project's .git is never touched.",
    ...
    "Cron scripts live in ~/.agentic-os/scripts/ and run before the agent — perfect for data collection pipelines.",
    ...
    "The skills audit log at ~/.agentic-os/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Profiles scope Hermes state via AGENTIC_OS_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "AGENTIC_OS_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in AGENTIC_OS_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.agentic-os/interrupt_debug.log with timestamps.",
    ...
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.agentic-os/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.agentic-os/skills/.hub/quarantine/ holds skills pending security review.",
    ...
    '/reload reloads ~/.agentic-os/.env into the running session — pick up new API keys without restarting.',
    ...
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.agentic-os/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.agentic-os/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',
    ...
    # --- TUI & Dashboard ---
    'AGENTIC_OS_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "AGENTIC_OS_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'Drop a YAML in ~/.agentic-os/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.agentic-os/dashboard-plugins/ — no npm build required.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.agentic-os/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.agentic-os/skills/ so drop-in skills appear without restarting the session.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.agentic-os/dashboard-plugins/.',
]
