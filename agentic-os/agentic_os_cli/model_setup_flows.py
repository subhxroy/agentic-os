...
Extracted from ``agentic_os_cli/main.py`` as part of the god-file decomposition
campaign (``~/.agentic-os/plans/god-file-decomposition.md``, Phase 2 — splitting
...
in main.py re-imports them (``from agentic_os_cli.model_setup_flows import *``-style
explicit import) so existing call sites — and test monkeypatches that target
``agentic_os_cli.main._model_flow_*`` — keep resolving against main.py's namespace.
...
imported lazily inside the flows (``from agentic_os_cli.main import ...`` resolves at
call time, when main.py is fully loaded) so this module never imports
``agentic_os_cli.main`` at import time -> no import cycle.
"""
...
from agentic_os_cli.config import clear_model_endpoint_credentials
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from agentic_os_cli.main import _prompt_api_key
    ...
    # in-flow (K/R/C) instead of having to edit ~/.agentic-os/.env by hand. The
    ...
    from agentic_os_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_moa(config, current_model=""):
    ...
    """
    from agentic_os_cli.auth import _save_model_choice, deactivate_provider
    from agentic_os_cli.config import load_config, save_config
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from agentic_os_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier:
        try:
            from agentic_os_cli.nous_account import (
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from agentic_os_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from agentic_os_cli.main import _DEFAULT_QWEN_PORTAL_MODELS
    from agentic_os_cli.auth import (
    ...
    from agentic_os_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from agentic_os_cli.main import _auto_provider_name, _prompt_custom_api_mode_selection, _save_custom_provider
    from agentic_os_cli.auth import _save_model_choice, deactivate_provider
    from agentic_os_cli.config import get_env_value, load_config, save_config
    ...
    from agentic_os_cli.models import probe_api_models
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from agentic_os_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from agentic_os_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from agentic_os_cli import azure_detect
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from agentic_os_cli.main import _custom_provider_api_key_config_value, _custom_provider_base_url_config_value, _save_custom_provider
    from agentic_os_cli.auth import _save_model_choice, deactivate_provider
    from agentic_os_cli.config import load_config, save_config
    from agentic_os_cli.models import fetch_api_models
...
def _model_flow_copilot(config, current_model=""):
    ...
    from agentic_os_cli.main import _current_reasoning_effort, _prompt_reasoning_effort_selection, _set_reasoning_effort
    from agentic_os_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from agentic_os_cli.config import save_env_value, load_config, save_config
    from agentic_os_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from agentic_os_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from agentic_os_cli.copilot_auth import validate_copilot_token
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from agentic_os_cli.main import _infer_stepfun_region, _prompt_api_key, _prompt_provider_choice, _stepfun_base_url_for_region
    from agentic_os_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from agentic_os_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_vertex(config, current_model=""):
    ...
    *path* lives in .env (VERTEX_CREDENTIALS_PATH / GOOGLE_APPLICATION_CREDENTIALS);
    ...
    from agentic_os_cli.config import load_config, save_config, get_env_value
    from agentic_os_cli.models import _PROVIDER_MODELS
    ...
    if sa_path:
        ...
        print("      • set VERTEX_CREDENTIALS_PATH in ~/.agentic-os/.env to a service account JSON")
    print()
...
def _select_zai_endpoint(current_base: str) -> str:
    ...
    sourced from ``ZAI_ENDPOINTS`` in ``agentic_os_cli.auth`` so it stays in
    sync with the probe list.

    Returns the selected base URL.  Falls back to *current_base* on cancel
    or error.
    """
    from agentic_os_cli.main import _prompt_provider_choice
    from agentic_os_cli.auth import ZAI_ENDPOINTS
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "lmstudio":
        ...
        from agentic_os_cli.models import fetch_lmstudio_models
        ...
        from agentic_os_cli.models import fetch_ollama_cloud_models
