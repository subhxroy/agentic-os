...
from agentic_os_cli import kanban_swarm as ks
...
def _check_dispatcher_presence() -> tuple[bool, str]:
    ...
    - ``running=True``: a gateway is alive for this AGENTIC_OS_HOME and its
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description=(
            ...
            "See https://agentic-os.nousresearch.com/docs/user-guide/features/kanban "
    ...
    p_asg = sub.add_parser(
        ...
        help="List known profiles + per-profile task counts "
             "(union of ~/.agentic-os/profiles/ and current assignees on the board)",
    )
...
def kanban_command(args: argparse.Namespace) -> int:
    ...
    # AGENTIC_OS_HOME. Previously only `init` and `daemon` triggered
...
def _dispatch_boards(args: argparse.Namespace) -> int:
    ...
    DB, so a fresh AGENTIC_OS_HOME that has never called ``kanban init``
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    if profiles:
        ...
        print("No profiles found under ~/.agentic-os/profiles/.")
        ...
        for tid in ids:
            ...
            if task and task.goal_mode:
                ...
                try:
                    ...
                    pass
                if judge_available:
                    from agentic_os_cli.goals import judge_goal
                    verdict = "done"
                    reason = ""
                    try:
                        # judge_goal returns (verdict, reason, parse_failed,
                        # wait_directive, transport_failed) — see
                        # agentic_os_cli/goals.py. Unpacking fewer raises
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from agentic_os_cli import kanban_specify as spec
...
def _cmd_decompose(args: argparse.Namespace) -> int:
    ...
    from agentic_os_cli import kanban_decompose as decomp
