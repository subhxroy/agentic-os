...
transport, discovers their tools, and registers them into the agentic-os
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.agentic-os/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.agentic-os/logs/mcp-stderr.log), tagged with the
...
class MCPServerTask:
    ...
    async def _run_stdio(self, config: dict):
        ...
        # ~/.agentic-os/logs/mcp-stderr.log.
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"file under ~/.agentic-os/mcp-tokens/ and restart). Do NOT retry "
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local AGENTIC_OS_HOME override into ``coro``.
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local AGENTIC_OS_HOME override onto the MCP loop.
...
def _load_mcp_config() -> Dict[str, dict]:
    ...
    ``os.environ`` (which includes ``~/.agentic-os/.env`` loaded at startup).
    """
    try:
