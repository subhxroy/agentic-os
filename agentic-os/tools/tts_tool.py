...
# Base URL now resolved via agentic_os_cli.models.deepinfra_base_url (shared).
...
def _get_default_output_dir() -> str:
    ...
    return str(get_agentic_os_dir("cache/audio", "audio_cache"))
...
def _load_tts_config() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.debug("agentic_os_cli.config not available, using default TTS config")
        return {}
    except Exception as e:
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    the shared ``agentic_os_cli.models`` helpers so every DeepInfra surface
...
def _generate_gemini_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if urlparse(base_url).hostname == "generativelanguage.googleapis.com":
        try:
            import agentic_os_cli as _agentic_os_cli

            _hermes_version = str(_agentic_os_cli.__version__)
        except Exception:
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_agentic_os_home()}/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
