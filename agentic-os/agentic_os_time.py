...
Resolution order:
  1. ``agentic_os_timeZONE`` environment variable
  2. ``timezone`` key in ``~/.agentic-os/config.yaml``
...
from agentic_os_constants import get_config_path
...
def _resolve_timezone_name() -> str:
    ...
    tz_env = os.getenv("agentic_os_timeZONE", "").strip()
    if tz_env:
        return tz_env
...
def reset_cache() -> None:
    ...
    config edit or ``agentic_os_timeZONE`` update) to force ``get_timezone()`` /
