...
Usage:
    ...
    hermes uninstall           Uninstall Agentic OS
...
# IMPORTANT: agentic_os_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``agentic_os_bootstrap`` is a
...
# new code references ``agentic_os_bootstrap`` but the editable install's
...
# that run *before* agentic_os_cli.config is importable. Mirrors the explicit
...
def _print_fast_version_info() -> None:
    from agentic_os_cli import __release_date__, __version__

    print(f"Agentic OS v{__version__} ({__release_date__})")
...
from agentic_os_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from agentic_os_cli.subcommands.cron import build_cron_parser
from agentic_os_cli.subcommands.gateway import build_gateway_parser
from agentic_os_cli.subcommands.profile import build_profile_parser
from agentic_os_cli.subcommands.model import build_model_parser
from agentic_os_cli.subcommands.setup import build_setup_parser
from agentic_os_cli.subcommands.postinstall import build_postinstall_parser
from agentic_os_cli.subcommands.whatsapp import build_whatsapp_parser
from agentic_os_cli.subcommands.slack import build_slack_parser
from agentic_os_cli.subcommands.login import build_login_parser
from agentic_os_cli.subcommands.logout import build_logout_parser
from agentic_os_cli.subcommands.auth import build_auth_parser
from agentic_os_cli.subcommands.status import build_status_parser
from agentic_os_cli.subcommands.webhook import build_webhook_parser
from agentic_os_cli.subcommands.hooks import build_hooks_parser
from agentic_os_cli.subcommands.doctor import build_doctor_parser
from agentic_os_cli.subcommands.security import build_security_parser
from agentic_os_cli.subcommands.dump import build_dump_parser
from agentic_os_cli.subcommands.debug import build_debug_parser
from agentic_os_cli.subcommands.backup import build_backup_parser
from agentic_os_cli.subcommands.import_cmd import build_import_cmd_parser
from agentic_os_cli.subcommands.config import build_config_parser
from agentic_os_cli.subcommands.console import build_console_parser
from agentic_os_cli.subcommands.version import build_version_parser
from agentic_os_cli.subcommands.update import build_update_parser
...
from agentic_os_cli.subcommands.gui import build_gui_parser
from agentic_os_cli.subcommands.logs import build_logs_parser
from agentic_os_cli.subcommands.prompt_size import build_prompt_size_parser
from agentic_os_cli.subcommands.memory import build_memory_parser
from agentic_os_cli.subcommands.acp import build_acp_parser
from agentic_os_cli.subcommands.tools import build_tools_parser
from agentic_os_cli.subcommands.insights import build_insights_parser
from agentic_os_cli.subcommands.skills import build_skills_parser
from agentic_os_cli.subcommands.pairing import build_pairing_parser
from agentic_os_cli.subcommands.plugins import build_plugins_parser
from agentic_os_cli.subcommands.mcp import build_mcp_parser
from agentic_os_cli.subcommands.claw import build_claw_parser
...
def _apply_profile_override() -> None:
    ...
    # Mirrors agentic_os_cli.profiles._PROFILE_ID_RE so we never call
...
try:
    from agentic_os_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from agentic_os_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if agentic_os_constants not importable yet
...
# Provider model-selection wizard flows extracted to agentic_os_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (agentic_os_cli.main._model_flow_*) keep resolving unchanged.
from agentic_os_cli.model_setup_flows import (
...
def _find_bundled_tui(agentic_os_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if agentic_os_cli_dir is None:
        agentic_os_cli_dir = Path(__file__).parent
    bundled = agentic_os_cli_dir / "tui_dist" / "entry.js"
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    def _node_bin(bin: str) -> str:
        ...
        if not path and bin == "node":
            try:
                from agentic_os_cli.dep_ensure import ensure_dependency
    ...
    # ships agentic_os_cli/tui_dist/entry.js in the wheel but never ships ui-tui/
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from agentic_os_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    if code == 42:
        from agentic_os_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    ...
    if os.environ.get("HERMES_KANBAN_BOARD"):
        return
    try:
        from agentic_os_cli.kanban_db import get_current_board
...
def cmd_chat(args):
    ...
    try:
        from agentic_os_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from agentic_os_cli.config import load_config as _load_config_for_xai_check
...
def cmd_gateway(args):
    ...
    from agentic_os_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from agentic_os_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from agentic_os_constants import find_node_executable, with_hermes_node_path
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with '▸ Agentic OS'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  hermes gateway")
...
def cmd_whatsapp_cloud(args):
    ...
    ``agentic_os_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from agentic_os_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from agentic_os_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    print("▸ Agentic OS post-install bootstrap")
    print()
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from agentic_os_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    # PROVIDER_GROUPS in agentic_os_cli/models.py). A multi-member group shows one
...
def _clear_stale_openai_base_url():
    ...
    """
    from agentic_os_cli.config import get_env_value, save_env_value, load_config
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`agentic_os_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from agentic_os_cli.plugins import get_plugin_auxiliary_tasks
...
def _aux_flow_provider_model(
    ...
    from agentic_os_cli.auth import _prompt_model_selection
    from agentic_os_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from agentic_os_cli.setup import _curses_prompt_choice
...
# downstream call sites read `agentic_os_cli.main._PROVIDER_MODELS` directly,
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from agentic_os_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    ...
    """
    from agentic_os_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    from agentic_os_cli.env_loader import format_secret_source_suffix
...
def cmd_login(args):
    ...
    from agentic_os_cli.auth import login_command
...
def cmd_auth(args):
    ...
    from agentic_os_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from agentic_os_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from agentic_os_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from agentic_os_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    if sub == "manifest":
        from agentic_os_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from agentic_os_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from agentic_os_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from agentic_os_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from agentic_os_cli.doctor import run_doctor
...
def cmd_security(args):
    ...
    if sub in ("audit", None):
        from agentic_os_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from agentic_os_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from agentic_os_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from agentic_os_cli.config import config_command
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        from agentic_os_cli.backup import run_quick_backup
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    from agentic_os_cli.banner import format_banner_version_label
...
def cmd_uninstall(args):
    """Uninstall Agentic OS (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from agentic_os_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from agentic_os_cli.uninstall import run_gui_uninstall
    ...
    from agentic_os_cli.uninstall import run_uninstall
...
_UPDATE_CRITICAL_FILES = (
    "agentic_os_cli/main.py",
    "agentic_os_cli/config.py",
    "agentic_os_cli/__init__.py",
    "agentic_os_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
    "agentic_os_constants.py",
)
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``agentic_os_cli/web_dist/`` (per vite.config.ts
    outDir: "../agentic_os_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "agentic_os_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``hermes`` launcher present but ``agentic_os_cli`` not importable.
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    # build path usable on a stock `py -m agentic_os_cli.main web` invocation.
    ...
    from agentic_os_constants import with_hermes_node_path
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from agentic_os_logging import setup_logging as _setup_logging_gui
...
def _find_stale_dashboard_pids(
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    patterns = [
        "hermes dashboard",
        "agentic_os_cli.main dashboard",
        "agentic_os_cli/main.py dashboard",
        ...
        "hermes serve",
        "agentic_os_cli.main serve",
        "agentic_os_cli/main.py serve",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # with `agentic_os_cli.gateway._scan_gateway_pids` and avoids the
...
def _update_via_zip(args):
    """Update Agentic OS by downloading a ZIP archive.
    ...
    from agentic_os_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        from agentic_os_cli.model_catalog import seed_cache_from_checkout
    ...
    *,
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m agentic_os_cli.main <command>"
        )
    else:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from agentic_os_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _resolve_node_runtime_npm() -> str | None:
    ...
    """
    from agentic_os_constants import find_node_executable
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from agentic_os_constants import is_wsl
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    if method == "docker":
        ...
        from agentic_os_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from agentic_os_cli.config import recommended_update_command
        from agentic_os_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    path_comment = (
        "# Agentic OS — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    Windows: the Desktop app's backend (``python.exe -m agentic_os_cli.main
    ...
    for proc in proc_iter:
        ...
        # this venv's path, or a `-m agentic_os_cli.main ...` invocation tied to
        # this install (install root in the cmdline or as the working dir).
        if not is_holder and venv_prefix in cmdline_low:
            is_holder = True
        if not is_holder and "agentic_os_cli.main" in cmdline_low:
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m agentic_os_cli.main
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    try:
        from agentic_os_cli import gateway_windows
        from agentic_os_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def cmd_update(args):
    """Update Agentic OS to the latest version.
    ...
    if is_managed():
        managed_error("update Agentic OS")
        return
...
def _cmd_update_pip(args):
    ...
    from agentic_os_cli.config import is_uv_tool_install
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # orphan merge-conflict markers in agentic_os_cli/config.py bricked
        ...
        if (desktop_dir / "package.json").exists() and _resolve_node_runtime_npm() and has_desktop_app:
            ...
            _desktop_build_cmd = [sys.executable, "-m", "agentic_os_cli.main", "desktop", "--build-only"]
        ...
        try:
            from agentic_os_cli.profiles import backfill_profile_envs
        ...
        try:
            ...
            if (
                ...
                from agentic_os_cli.tools_config import install_cua_driver
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        from agentic_os_cli import profiles as _profiles_mod
        ...
        from agentic_os_cli import profile_describer as _pd
        ...
        from agentic_os_cli.profiles import profile_exists, validate_alias_name
        ...
        from agentic_os_cli.profiles import rename_profile
        ...
        from agentic_os_cli.profiles import export_profile
        ...
        from agentic_os_cli.profiles import import_profile
        ...
        import tempfile
        from agentic_os_cli.profile_distribution import (
            plan_install,
        ...
        from agentic_os_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        from agentic_os_cli.profile_distribution import describe_distribution, DistributionError
...
def _render_distribution_plan(plan) -> None:
    ...
    from agentic_os_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    try:
        from agentic_os_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    try:
        from agentic_os_cli.config import load_config, save_config
        from agentic_os_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
...
def cmd_dashboard(args):
    ...
    if (
        ...
        reexec_argv = [
            sys.executable, "-m", "agentic_os_cli.main",
            "-p", "default",
    ...
    if _headless_backend:
        ...
        _dist_root = (
            ...
            else PROJECT_ROOT / "agentic_os_cli" / "web_dist"
        )
    ...
    try:
        from agentic_os_cli.mcp_startup import start_background_mcp_discovery
    ...
    from agentic_os_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from agentic_os_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from agentic_os_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from agentic_os_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from agentic_os_cli.prompt_size import cmd_prompt_size as _impl
...
def cmd_logs(args):
    ...
    from agentic_os_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    ...
    from agentic_os_cli.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from agentic_os_cli.models import CANONICAL_PROVIDERS as _cp
...
# the top-level flags declared in ``agentic_os_cli/_parser.py``.
#
...
def cmd_acp(args):
    """Launch Agentic OS as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from agentic_os_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from agentic_os_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from agentic_os_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from agentic_os_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from agentic_os_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from agentic_os_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from agentic_os_cli.plugins_cmd import plugins_command
...
def cmd_claw(args):
    from agentic_os_cli.claw import claw_command
...
def main():
    ...
    try:
        from agentic_os_cli.stdio import configure_windows_stdio
    ...
    # =========================================================================
    # model command  (parser built in agentic_os_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from agentic_os_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from agentic_os_cli.fallback_cmd import cmd_fallback
    ...
    from agentic_os_cli import secrets_cli as _secrets_cli
    from agentic_os_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    # migrate command
    # =========================================================================
    from agentic_os_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in agentic_os_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in agentic_os_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in agentic_os_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in agentic_os_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    # =========================================================================
    # slack command  (parser built in agentic_os_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from agentic_os_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in agentic_os_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in agentic_os_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in agentic_os_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in agentic_os_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in agentic_os_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in agentic_os_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from agentic_os_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from agentic_os_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from agentic_os_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in agentic_os_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in agentic_os_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in agentic_os_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in agentic_os_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in agentic_os_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in agentic_os_cli/subcommands/backup.py)
    # =========================================================================
    ...
    from agentic_os_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in agentic_os_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in agentic_os_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # console command  (parser built in agentic_os_cli/subcommands/console.py)
    # =========================================================================
    build_console_parser(subparsers, cmd_console=cmd_console)

    # =========================================================================
    # pairing command  (parser built in agentic_os_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in agentic_os_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from agentic_os_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in agentic_os_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from agentic_os_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from agentic_os_cli.curator import register_cli as _register_curator_cli
    ...
    try:
        from agentic_os_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from agentic_os_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in agentic_os_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in agentic_os_cli/subcommands/tools.py)
    # =========================================================================
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                version = ""
                try:
                    from agentic_os_cli.tools_config import _cua_driver_env
    ...
    # =========================================================================
    # mcp command  (parser built in agentic_os_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            from agentic_os_state import workspace_key as _ws_key
            ...
            def _redact(data):
                if not args.redact or data is None:
                    return data
                from agentic_os_cli.session_export_md import redact_session_data
    ...
    # =========================================================================
    # insights command  (parser built in agentic_os_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in agentic_os_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in agentic_os_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in agentic_os_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in agentic_os_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in agentic_os_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in agentic_os_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in agentic_os_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # =========================================================================
    # gui command  (parser built in agentic_os_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in agentic_os_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in agentic_os_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    from agentic_os_cli.config import get_container_exec_info
