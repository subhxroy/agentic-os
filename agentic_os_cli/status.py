...
Shows the status of all Agentic OS components.
"""
...
from agentic_os_cli.auth import AuthError, resolve_provider
from agentic_os_cli.colors import Colors, color
from agentic_os_cli.config import get_env_path, get_env_value, get_agentic_os_home, load_config
from agentic_os_cli.models import provider_label
...
from agentic_os_cli.runtime_provider import resolve_requested_provider
...
def show_status(args):
    """Show status of all Agentic OS components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 Agentic OS Status                      │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if _effective_provider_label() == "LM Studio":
        from agentic_os_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from agentic_os_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    jobs_file = get_agentic_os_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
