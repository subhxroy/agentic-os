...
- ``agentic_os_cli.providers``        -- canonical provider identity + overlays
- ``agentic_os_cli.model_normalize``  -- per-provider name formatting
...
_HERMES_MODEL_WARNING = (
    ...
    "for use with Agentic OS. They lack the tool-calling capabilities "
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from agentic_os_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        from agentic_os_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from agentic_os_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from agentic_os_cli.config import load_config
            from agentic_os_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from agentic_os_cli.providers import resolve_user_provider as _ruser
    ...
    # agentic_os_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    if target_provider in {"opencode-zen", "opencode-go"} and isinstance(base_url, str):
        from agentic_os_cli.models import normalize_opencode_base_url
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from agentic_os_cli.config import normalize_extra_headers
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from agentic_os_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from agentic_os_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from agentic_os_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # https://agentic-os.nousresearch.com/docs/api/model-catalog.json so
    ...
    from agentic_os_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from agentic_os_cli.models import _PROVIDER_ALIASES as _CANON_ALIASES
    from agentic_os_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        from agentic_os_cli.auth import is_runtime_provider_routable
    ...
    from agentic_os_cli.auth import PROVIDER_REGISTRY as _auth_registry
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                from agentic_os_cli.auth import get_provider_auth_state as _nous_state
    ...
    # with `hermes model`).
    try:
        from agentic_os_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # run_agent.py, agentic_os_cli/config.py) already consume that dict.
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from agentic_os_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`agentic_os_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from agentic_os_cli.models import fetch_openrouter_models
