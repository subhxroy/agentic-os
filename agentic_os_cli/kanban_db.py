...
from agentic_os_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional link to a first-class Project (agentic_os_cli/projects_db). When set,
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See agentic_os_state._WAL_INCOMPAT_MARKERS for detection logic.
                from agentic_os_state import apply_wal_with_fallback
                apply_wal_with_fallback(conn, db_label=f"kanban.db ({path.name})")
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from agentic_os_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    if project_id:
        try:
            from agentic_os_cli import projects_db as _pdb
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    if not rows:
        return False
    try:
        from agentic_os_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def _dispatch_once_locked(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from agentic_os_cli.profiles import profile_exists as _pe
    ...
    for row in review_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        try:
            from agentic_os_cli.profiles import profile_exists
        except Exception:
            profile_exists = None  # type: ignore[assignment]
...
def _module_hermes_argv() -> list[str]:
    """Return the interpreter-bound Agentic OS CLI invocation."""
    # ``agentic_os_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``hermes`` package — there is no
    # ``hermes`` package to import.
    return [sys.executable, "-m", "agentic_os_cli.main"]
...
def _resolve_hermes_argv() -> list[str]:
    ...
    3. ``sys.executable -m agentic_os_cli.main`` — fallback for setups where
    ...
    local (not imported from gateway) because ``agentic_os_cli`` sits below
    ...
    *,
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Install Agentic OS or activate its venv before running the kanban dispatcher."
        )
...
def list_profiles_on_disk() -> list[str]:
    ...
    ``agentic_os_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from agentic_os_constants import get_default_hermes_root
