...
sidebar_position: 9
title: "Matrix"
description: "将 Agentic OS 设置为 Matrix 机器人"
---

# Matrix 设置

Agentic OS 与 Matrix 集成，Matrix 是一种开放的联邦消息协议。Matrix 允许你运行自己的 homeserver，也可以使用 matrix.org 等公共 homeserver——无论哪种方式，你都保持对通信的控制权。机器人通过 `mautrix` Python SDK 连接，通过 Agentic OS 管道（包括工具调用、记忆和推理）处理消息，并实时响应。它支持文本、文件附件、图片、音频、视频，以及可选的端对端加密（E2EE）。
...
Agentic OS 使用你的 Matrix 用户 ID 来控制谁可以与机器人交互。Matrix 用户 ID 的格式为 `@username:server`。
...
有关保护 Agentic OS 部署的更多信息，请参阅[安全指南](../security.md)。
