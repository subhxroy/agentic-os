"""
Tests for timezone support (agentic_os_time module + integration points).
...
def _reset_agentic_os_time_cache():
    """Reset the agentic_os_time module cache (replacement for removed reset_cache)."""
    agentic_os_time._cached_tz = None
    agentic_os_time._cached_tz_name = None
    agentic_os_time._cache_resolved = False


# =========================================================================
# agentic_os_time.now() — core helper
# =========================================================================
...
class TestHermesTimeNow:
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["agentic_os_timeZONE"] = "Asia/Kolkata"
        result = agentic_os_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["agentic_os_timeZONE"] = "UTC"
        result = agentic_os_time.now()
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["agentic_os_timeZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="agentic_os_time"):
            result = agentic_os_time.now()
    ...
    def test_cache_invalidation(self):
        ...
        os.environ["agentic_os_timeZONE"] = "UTC"
        _reset_agentic_os_time_cache()
        r1 = agentic_os_time.now()
        assert r1.utcoffset() == timedelta(0)

        os.environ["agentic_os_timeZONE"] = "Asia/Kolkata"
        _reset_agentic_os_time_cache()
        r2 = agentic_os_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["agentic_os_timeZONE"] = "Europe/London"
        tz = agentic_os_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["agentic_os_timeZONE"] = "Not/A/Timezone"
        tz = agentic_os_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When agentic_os_timeZONE is set, child process sees TZ env var.
        ...
        #   2) agentic_os_timeZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("agentic_os_timeZONE=" + os.environ.get("agentic_os_timeZONE", "NOT_SET"))'
        )
        ...
        assert "agentic_os_timeZONE=NOT_SET" in result["output"], (
            "agentic_os_timeZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When agentic_os_timeZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("agentic_os_timeZONE", None)
...
class TestCronTimezone:
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        ...
        os.environ["agentic_os_timeZONE"] = "Pacific/Midway"  # UTC-11
        _reset_agentic_os_time_cache()
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["agentic_os_timeZONE"] = "US/Eastern"
        _reset_agentic_os_time_cache()
