# Agentic OS Security Policy

This document describes Agentic OS's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/subhxroy/agentic-os/security/advisories/new)
or **security@nousresearch.com**. Do not open public issues for
security vulnerabilities. **Agentic OS does not operate a bug
bounty program.**
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Agentic OS access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Agentic OS's documentation
...
Agentic OS supports two OS-level isolation postures. They address
...
Agentic OS supports this in two ways:

- **Agentic OS's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Agentic OS's in-process heuristics
...
Agentic OS filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Agentic OS
itself. Bugs in Agentic OS's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Agentic OS does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
   what this policy, Agentic OS's own documentation, or reasonable
  operator expectations would predict — including cases where
   Agentic OS has documented a stance about how its output should
...
- **Community-contributed skills and plugins.** Third-party skills
  (including the community skills repository) and third-party
   plugins are in the operator's review surface, not Agentic OS's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
   reviewed, not a vulnerability in Agentic OS. Bugs in Hermes
...
- Agentic OS includes supply-chain guards for MCP server
