#!/usr/bin/env python3
"""
SQLite State Store for Agentic OS.
...
# agentic_os_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``agentic_os_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see agentic_os_cli/kanban_db.py) would
...
class SessionDB:
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            # ``agentic_os_state._set_last_init_error(None)`` explicitly.
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        if repair_alternation and messages:
            # Lazy import: agentic_os_state already depends on agent.* (see
