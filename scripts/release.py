...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"agentic-os[acp]=={semver}"
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/subhxroy/agentic-os",
    ...
    lines.append(f"# Agentic OS v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Agentic OS. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Agentic OS Release Tool")
    ...
    print(f"{'='*60}")
    print("  Agentic OS Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Agentic OS v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Agentic OS v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Agentic OS v{new_version} ({calver_date})' "
